...
async def test_link_button_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: چون حدس اول (button_link تو‌درتو) با
    INVALID_INPUT رد شد، این نسخه چند تا ساختار محتمل مختلف رو یکی‌یکی به API
    واقعی روبیکا می‌فرسته و می‌بینه سرور کدوم رو قبول می‌کنه (بدون خطا) - تا
    به‌جای حدس‌زدن کور، تجربی مشخص بشه ساختار درست کدومه."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    url = "https://google.com"
    candidates = [
        ("A: url تخت (حالت فعلی/باگ‌دار)", {"id": "cA", "type": "Link", "button_text": "🅰️ A", "url": url}),
        ("B: button_link{type:url, link_url}", {"id": "cB", "type": "Link", "button_text": "🅱️ B", "button_link": {"type": "url", "link_url": url}}),
        ("C: button_link{type:Url, link_url}", {"id": "cC", "type": "Link", "button_text": "🇨 C", "button_link": {"type": "Url", "link_url": url}}),
        ("D: link_url تخت", {"id": "cD", "type": "Link", "button_text": "🇩 D", "link_url": url}),
        ("E: button_link{link_url} بدون type", {"id": "cE", "type": "Link", "button_text": "🇪 E", "button_link": {"link_url": url}}),
        ("F: link{type:url, url}", {"id": "cF", "type": "Link", "button_text": "🇫 F", "link": {"type": "url", "url": url}}),
        ("G: button_link{type:URL, link_url}", {"id": "cG", "type": "Link", "button_text": "🇬 G", "button_link": {"type": "URL", "link_url": url}}),
    ]
    results = []
    for label, btn in candidates:
        kp = make_link_keypad(btn)
        try:
            await _rx(message.reply_inline(f"تست {label}", kp))
            results.append(f"✅ پذیرفته شد (بدون خطا): {label}")
        except Exception as e:
            results.append(f"❌ رد شد: {label}\n   → {type(e).__name__}: {e}")
    summary = "📊 نتیجه‌ی همه‌ی تست‌ها:\n\n" + "\n\n".join(results)
    summary += "\n\n⚠️ هر پیامی که بالاش اومد رو هم لمس کن؛ حتی اگه سرور 'پذیرفت'، ممکنه بازم لمسش کار نکنه. بگو کدوم دکمه واقعاً باز شد."
    await _rx(message.reply(summary[:3900]))
...
def build_invite_message(chat_id):
    """متن کوتاه و دکمه‌ی شیشه‌ای (لینکی) مشترک برای دعوت دوستان - هم توی پیام
    اتمام سقف رایگان چت هوش مصنوعی استفاده میشه، هم توی /invite. با زدن دکمه،
    کاربر مستقیم به لینک اختصاصی (صفحه‌ی ربات توی روبیکا) می‌ره تا از همونجا
    برای هرکسی که خواست بفرستدش."""
    ...
    keypad = None
    if link:
        # لینک به‌صورت متن خام هم توی پیام میاد تا روبیکا خودش آبی/لمس‌پذیرش کنه
        # (توجه: چون خودِ متنه، قابل انتخاب و کپی هم هست؛ روبیکا امکان لینک
        # با متن‌نمایشیِ جدا از URL (مثل تلگرام) رو پشتیبانی نمی‌کنه، پس تنها
        # حالت واقعاً غیرقابل‌کپی همون دکمه‌ی شیشه‌ای پایینه)
        text += f"\n\n🔗 {link}"
        keypad = make_link_keypad(make_link_button("do_invite", "🤝 دعوت کردن", link))
    else:
