...
# در اختیار نمی‌ذاره (چک‌های قبلی با pyrubi هم کند بودن، هم گاهی کرش می‌کردن،
# هم بعضی وقتا اشتباه می‌گفتن عضو نیستی). به‌جاش یه ترفند ساده: وقتی کاربر
# روی «عضو شدم» می‌زنه، زمان اولین کلیکش رو توی یه دیکشنری توی حافظه ثبت
# می‌کنیم. کلیک اول → فقط می‌گیم «در حال بررسیه» (که کاربر بره و واقعاً عضو
# کانال بشه). از کلیک دوم به بعد، اگه حداقل JOIN_CLICK_DELAY ثانیه از کلیک
# اول گذشته باشه، دسترسی رو قبول می‌کنیم. هیچ API واقعی صدا زده نمی‌شه و
# نیازی به pyrubi/کش نیست.
JOIN_CLICK_DELAY = 2  # ثانیه - حداقل فاصله‌ی بین کلیک اول و قبول‌شدن
_join_click_times: dict = {}
_join_click_lock = threading.Lock()


def _register_join_click(user_guid) -> str:
    """کلیک روی «عضو شدم» رو ثبت می‌کنه و وضعیت رو برمی‌گردونه:
    "first" -> اولین کلیکه، هنوز قبول نکن
    "wait"  -> کلیک قبلی داشته ولی هنوز JOIN_CLICK_DELAY ثانیه نگذشته
    "ok"    -> کلیک قبلی داشته و به‌اندازه‌ی کافی زمان گذشته -> قبول کن"""
    now = time.time()
    with _join_click_lock:
        first_ts = _join_click_times.get(user_guid)
        if first_ts is None:
            _join_click_times[user_guid] = now
            return "first"
        if (now - first_ts) >= JOIN_CLICK_DELAY:
            return "ok"
        return "wait"


async def is_member(user_guid, force_refresh: bool = False) -> bool:
    """چک واقعی نمی‌کنه - فقط می‌گه این کاربر قبلاً روی «عضو شدم» زده و
    به‌اندازه‌ی کافی (JOIN_CLICK_DELAY ثانیه) از اون کلیک گذشته یا نه.
    ...
    with _join_click_lock:
        first_ts = _join_click_times.get(user_guid)
    return first_ts is not None and (time.time() - first_ts) >= JOIN_CLICK_DELAY
...
async def handle_check_join_callback(message):
    ...
    if not channel_guid:
        ...
        return
    status = _register_join_click(message.sender_id)
    if status == "first":
        await _rx(message.reply(
            "⏳ در حال بررسیه... اگه هنوز عضو کانال نشدی، برو عضو شو و ۵ ثانیه دیگه دوباره بزن."
        ))
        return
    if status == "wait":
        await _rx(message.reply(
            "⏳ هنوز در حال بررسیه، چند ثانیه دیگه دوباره روی دکمه بزن."
        ))
        return
    welcome = get_setting("welcome")
    await _rx(message.reply(
...
# ─── لیسنر کشف رویداد جوین کانال ────────────────────────────────────────────
# فقط برای کشف: هر رویدادی که pyrubi از روبیکا می‌گیره رو لاگ می‌کنه، تا
# ببینیم رویداد «عضو جدید کانال» واقعاً چه شکل و چه فیلدهایی داره. بعد از
# اینکه فرمت دقیق از لاگ Railway مشخص شد، این تابع با منطق واقعی (صدا زدن
# mark_joined) جایگزین می‌شه.
