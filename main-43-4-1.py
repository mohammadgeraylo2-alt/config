...
# ─── اجبار به IPv4 برای همه‌ی درخواست‌های خروجی ──────────────────────────────
# چون هم آپلود خام (با retry و بدون keep-alive) هم reply_music خودِ rubka با
# همون خطای «Connection aborted / write operation timed out» شکست می‌خوردن،
# مشکل از سمتِ اتصالِ استفاده‌شده (pooled/تازه) نبود - یعنی اتصال واقعاً
# برقرار میشه (سه‌مرحله‌ای TCP کامل میشه) ولی نوشتنِ دیتای واقعی روش گیر
# می‌کنه. این دقیقاً علامتِ کلاسیکِ یه مسیر IPv6 خراب/blackhole‌ست: هندشیک
# اولیه از یه مسیر رد میشه ولی پکت‌های دیتا (که بزرگ‌ترن) یه جای دیگه گم
# میشن یا drop میشن، بدون اینکه هیچ خطای فوری برگرده - فقط سمت ما منتظر
# می‌مونیم تا timeout. با این monkeypatch، urllib3 (زیرساخت requests) رو
# مجبور می‌کنیم فقط IPv4 امتحان کنه، که این کلاس از مشکلات رو کامل حذف می‌کنه.
import socket
...
# ─── محدودکردن MSS برای دورزدن PMTU black hole ──────────────────────────────
# فورس‌کردن IPv4 به‌تنهایی مشکل رو حل نکرد چون علتِ واقعی IPv6 نبود: الگوی لاگ
# (اتصال فوری برقرار میشه، ولی نوشتنِ بدنه‌ی فایل - نه JSONهای کوچیک - هنگ
# می‌کنه تا timeout، حتی بعد از retry/backoff و بدون هیچ Session/keep-alive‌ای)
# دقیقاً امضای کلاسیک یه PMTU black hole‌ست: یه روتر توی مسیر بین Railway و
# messengerg2f1.rubika.ir پکت‌های بزرگ‌تر از MTU واقعیِ مسیر رو drop می‌کنه
# بدون اینکه ICMP «Fragmentation Needed» برگردونه (این پیام معمولاً یه‌جای
# دیگه‌ی مسیر - یا خودِ فایروال Railway - فیلتر میشه)، پس سمتِ ما هیچ‌وقت
# نمی‌فهمه باید پکت رو کوچیک‌تر بفرسته. درخواست‌های کوچیک (زیر ~1500 بایت)
# هیچ‌وقت این مشکل رو نشون نمی‌دن، برای همین requestSendFile/sendFile همیشه
# سریع جواب می‌دن ولی بدنه‌ی چندسگمنتیِ آپلود فایل گیر می‌کنه - و چون این
# مشکل ربطی به کانکشن reuse نداره، retry/backoff/upload_url تازه هم دقیقاً
# همون نتیجه رو تکرار می‌کنن. راه‌حل: با محدودکردنِ MSS از همون اول، هیچ‌وقت
# پکتی نمی‌فرستیم که نیاز به fragment‌شدن داشته باشه - یعنی اصلاً وابسته به
# کارکردنِ PMTU discovery/ICMP نیستیم. این تنظیم روی urllib3.connection.
# HTTPConnection سرتاسری اعمال میشه (چون requests از urllib3 استفاده می‌کنه
# و HTTPSConnection هم همین default_socket_options رو ارث می‌بره)، برای همه‌ی
# درخواست‌ها (چه HTTP_SESSION مشترک، چه requests.post خام توی آپلود) - روی
# درخواست‌های کوچیک تاثیر محسوسی نداره، فقط برای آپلودهای بزرگ حیاتیه.
import urllib3.connection as _urllib3_conn

_urllib3_conn.HTTPConnection.default_socket_options = (
    list(_urllib3_conn.HTTPConnection.default_socket_options)
    + [(socket.IPPROTO_TCP, socket.TCP_MAXSEG, 1200)]
)
...
# فال‌بک تشخیص آهنگ وقتی Shazam نتونه چیزی پیدا کنه (کاور/ریمیکس/کیفیت پایین/قطعی سرور شزم).
# با ثبت‌نام رایگان تو dashboard.audd.io بدون کارت بانکی ۳۰۰ درخواست رایگان می‌گیری.
# اگه اصلاً چیزی ست نشه، توکن تستی "test" استفاده میشه که سقفش فقط ۱۰ درخواست در روزه
# (فقط برای تست اولیه کافیه، برای استفاده‌ی واقعی حتماً توکن خودت رو تو Railway ست کن).
AUDD_API_TOKEN = os.environ.get("AUDD_API_TOKEN", "test")
# فال‌بک «جستجوی متنی/شعری»: وقتی کاربر به‌جای اسم آهنگ یه تیکه از شعرشو می‌فرسته
# و search_songs (که فقط رو کاتالوگ Deezer/YouTube Music سرچ می‌کنه) چیزی پیدا
# نمی‌کنه، با Google Custom Search همون شعر رو تو وب سرچ می‌کنیم - سایت‌های دانلود
# آهنگ فارسی معمولاً تو تیتر صفحه‌شون فرمت «دانلود آهنگ [خواننده] [اسم آهنگ]» رو
# دارن، پس از تیتر بهترین نتیجه اسم واقعی آهنگ رو استخراج و دوباره تو کاتالوگ
# سرچ می‌کنیم. رایگان تا ۱۰۰ درخواست در روز - از console.cloud.google.com یه
# API key و از programmablesearchengine.google.com یه Search Engine ID (cx) بگیر.
# فال‌بک «جستجوی متنی/شعری» با کتابخونه‌ی ddgs (سرچ رو DuckDuckGo) - کاملاً
# رایگان، بدون نیاز به API key/ثبت‌نام/کارت بانکی. اگه نصب نشده باشه، پایین‌تر
# با یه try/except بی‌صدا غیرفعال میشه و ربات مثل قبل کار می‌کنه.
# ترفند دور زدن خطای «Sign in to confirm you're not a bot» یوتیوب که رو IP سرورهای
# ابری (Railway و مشابه) خیلی زیاد پیش میاد: به‌جای کلاینت وب پیش‌فرض yt-dlp، از
# کلاینت اندروید/iOS تقلید می‌کنیم که این چک رو نداره. اگه کوکی export‌شده از یه
# اکانت لاگین‌شده داری، مسیرش رو تو متغیر محیطی YTDLP_COOKIES_FILE بذار (اختیاری،
# اثرش قوی‌تر از ترفند بالاست ولی کوکی معمولاً بعد چند هفته expire میشه).
...
@bot.on_message(commands=["pyrubi_debug"])
async def pyrubi_debug_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: متدهای channel/member کلاینت pyrubi رو روی
    channel_guid تنظیم‌شده صدا می‌زنه و متن دقیق خطا (اگه بود) رو نشون میده."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
        await _rx(message.reply("❌ اول باید channel_guid توی تنظیمات ست بشه."))
        return
    if not os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi"):
        await _rx(message.reply("❌ سشن pyrubi لاگین نیست."))
        return
    await _rx(message.reply("⏳ در حال تست متدهای pyrubi..."))
    try:
        text = await asyncio.to_thread(_pyrubi_debug_blocking, channel_guid)
        for i in range(0, len(text), 3500):
            await _rx(message.reply(text[i:i + 3500]))
    except Exception as e:
        logger.exception("خطا در pyrubi_debug")
        await _rx(message.reply(f"❌ خطای کلی: {type(e).__name__}: {e}"))


def _pyrubi_debug2_blocking(channel_guid: str) -> str:
    """تست چند ترکیب مختلف از پارامترهای get_all_members/get_admin_members
    تا مشخص بشه کدوم ترکیب دقیق باعث InvalidInput میشه و کدوم جواب میده."""
    from pyrubi import Client
    client = Client(PYRUBI_SESSION_NAME)
    out = []

    attempts = [
        ("get_all_members(guid)", lambda: client.get_all_members(channel_guid)),
        ("get_all_members(guid, search_text='')", lambda: client.get_all_members(channel_guid, search_text="")),
        ("get_all_members(guid, start_id=None)", lambda: client.get_all_members(channel_guid, start_id=None)),
        ("get_all_members(guid, just_get_guids=True)", lambda: client.get_all_members(channel_guid, just_get_guids=True)),
        ("get_all_members(guid, search_text='', start_id=None, just_get_guids=True)",
         lambda: client.get_all_members(channel_guid, search_text="", start_id=None, just_get_guids=True)),
        ("get_admin_members(guid)", lambda: client.get_admin_members(channel_guid)),
        ("get_admin_members(guid, just_get_guids=True)", lambda: client.get_admin_members(channel_guid, just_get_guids=True)),
    ]
    ...
    for label, fn in attempts:
        try:
            ...
            detail = getattr(e, "args", None)
            out.append(f"❌ {label}: {type(e).__name__}: {e}\n   args={detail!r}")

    return "\n\n".join(out)


@bot.on_message(commands=["pyrubi_debug2"])
async def pyrubi_debug2_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: چند ترکیب مختلف پارامتر get_all_members رو
    تست می‌کنه تا مشخص بشه دقیقاً کدوم پارامتر باعث InvalidInput میشه."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
        await _rx(message.reply("❌ اول باید channel_guid توی تنظیمات ست بشه."))
        return
    await _rx(message.reply("⏳ در حال تست ترکیب‌های مختلف پارامتر..."))
    try:
        text = await asyncio.to_thread(_pyrubi_debug2_blocking, channel_guid)
        for i in range(0, len(text), 3500):
            await _rx(message.reply(text[i:i + 3500]))
    except Exception as e:
        logger.exception("خطا در pyrubi_debug2")
        await _rx(message.reply(f"❌ خطای کلی: {type(e).__name__}: {e}"))


def _pyrubi_resolve_username_blocking(username: str) -> str:
    """یوزرنیم کانال رو مستقیم با check_channel_username (که قبلاً موفق
    اجرا شد) به pyrubi می‌ده تا GUID واقعی از دید خودِ pyrubi رو برگردونه -
    برای مقایسه با channel_guid ذخیره‌شده (که با rubka فوروارد استخراج شده)."""
    from pyrubi import Client
    client = Client(PYRUBI_SESSION_NAME)
    result = client.check_channel_username(username)
    return repr(result)
...
@bot.on_message(commands=["pyrubi_resolve"])
async def pyrubi_resolve_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: /pyrubi_resolve instasavexx
    GUID واقعی کانال از دید pyrubi رو برمی‌گردونه تا با channel_guid فعلی
    (که با فوروارد rubka ست شده) مقایسه کنیم."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) < 2:
        await _rx(message.reply("فرمت: /pyrubi_resolve instasavexx (بدون @)"))
        return
    username = parts[1].strip().lstrip("@")
    try:
        text = await asyncio.to_thread(_pyrubi_resolve_username_blocking, username)
        current = get_setting("channel_guid") or "-"
        await _rx(message.reply(
            f"📣 خروجی pyrubi برای '{username}':\n{text}\n\n"
            f"📌 channel_guid فعلاً ذخیره‌شده (از rubka): {current}"
        ))
    except Exception as e:
        logger.exception("خطا در pyrubi_resolve")
        await _rx(message.reply(f"❌ خطا: {type(e).__name__}: {e}"))


@bot.on_message(commands=["check_member"])
async def check_member_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: مستقیم _pyrubi_is_member_blocking رو برای یک
    user_guid صدا می‌زنه و یا نتیجه (True/False) یا متن کامل خطا رو نشون میده."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    parts = (message.text or "").split()
    if len(parts) < 2:
        await _rx(message.reply("فرمت: /check_member uXXXXXXXXXXXXXXXXXXXXXXXX"))
        return
    ...
    if not channel_guid:
        ...
        return
    try:
        result = await asyncio.to_thread(_pyrubi_is_member_blocking, channel_guid, user_guid)
        await _rx(message.reply(f"نتیجه‌ی چک زنده برای {user_guid}: {'✅ عضوه' if result else '❌ عضو نیست'}"))
    except Exception as e:
        logger.exception("خطا در check_member")
        await _rx(message.reply(f"❌ چک زنده Exception داد: {type(e).__name__}: {e}"))
...
def init_db():
    with db() as con:
        ...
        """)
        con.execute("""
        CREATE TABLE IF NOT EXISTS sender_real_guid_map (
...
def get_cached_real_guid(sender_id: str):
    """GUID واقعی حساب (که قبلاً از message.author_info دراومده) رو اگه از
    قبل ذخیره شده باشه برمی‌گردونه، وگرنه None."""
    with db() as con:
        row = con.execute(
            "SELECT real_guid FROM sender_real_guid_map WHERE sender_id=?", (sender_id,)
        ).fetchone()
    return row[0] if row else None


def save_real_guid(sender_id: str, real_guid: str):
    with db() as con:
        con.execute(
            "INSERT INTO sender_real_guid_map (sender_id, real_guid, resolved_at) VALUES (?, ?, ?) "
            "ON CONFLICT(sender_id) DO UPDATE SET real_guid=excluded.real_guid, resolved_at=excluded.resolved_at",
            (sender_id, real_guid, datetime.now().isoformat()),
        )
...
def is_vip_active(u: dict) -> bool:
    """چک می‌کنه رکورد کاربر (خروجی get_user) الان VIP فعال حساب میشه یا نه
    - هم فلگ is_vip باید ست باشه، هم (اگه تاریخ انقضا داره) هنوز نگذشته باشه."""
    if not u or not u.get("is_vip"):
        return False
    return not u.get("vip_until") or u["vip_until"] >= datetime.now().isoformat()
...
def get_new_users_count(since: datetime) -> int:
    """تعداد کاربرانی که از تاریخ/زمان `since` به بعد عضو ربات شدن (برای
    گزارش‌های دوره‌ای روزانه/هفتگی)."""
    with db() as con:
        return con.execute(
            "SELECT COUNT(*) FROM users WHERE joined_at >= ?", (since.isoformat(),)
        ).fetchone()[0]


def build_report_text(period_label: str, since: datetime) -> str:
    """متنِ گزارش آماری رو برای یه بازه‌ی زمانی (روزانه/هفتگی/دستی) می‌سازه."""
    s = get_stats()
    new_users = get_new_users_count(since)
    today = datetime.now().date().isoformat()
    downloads_today = s.get("today_total", 0) if s.get("today_date") == today else 0
    return (
        f"📈 گزارش {period_label}\n\n"
        f"🆕 کاربران جدید: {new_users}\n"
        f"👥 کل کاربران: {s.get('total_users', 0)}\n"
        f"💎 VIP فعال: {s.get('vip_count', 0)}\n"
...
def check_cooldown(chat_id) -> bool:
    """آنتی‌اسپم: جلوی درخواست‌های خیلی پشت‌سرهم رو می‌گیره (مثلاً کاربری که
    ۱۰ لینک رو تو ۲ ثانیه می‌فرسته)، جدا از سقفِ روزانه‌ی دانلود/چت. VIP فعال
    و ادمین از این محدودیت معافن. برخلاف check_limit، اینجا پیامی به کاربر
    نمی‌فرستیم (silent ignore) تا خودِ پیامِ هشدار هم اسپم روی اسپم نشه."""
    if ADMIN_CHAT_ID and str(chat_id) == str(ADMIN_CHAT_ID):
        return True
    u = get_user(chat_id)
    if u and is_vip_active(u):
        return True
    cooldown = float(get_setting("cooldown_seconds", "3") or "3")
    if cooldown <= 0:
        return True
    now = time.monotonic()
    last = _last_request_at.get(chat_id)
    _last_request_at[chat_id] = now
    if last is not None and (now - last) < cooldown:
        logger.info(f"🧊 آنتی‌اسپم: درخواست {chat_id} به خاطر فاصله‌ی کم ({now - last:.1f}s) نادیده گرفته شد")
        return False
    return True
...
# ─── جوین اجباری کانال (چک واقعی با سلف‌بات pyrubi) ──────────────────────────
# متدهای واقعیِ کارکننده‌ی pyrubi همونایی هستن که توی /list_members استفاده
# شدن: client.get_all_members(channel_guid, start_id=...) و
# client.get_admin_members(channel_guid, start_id=...). این تابع دقیقاً همون
# الگو رو برای گشتن دنبال یک user_guid خاص به کار می‌بره. چون گشتن کل لیست
# اعضا برای هر چک تک‌کاربره سنگینه (و ریسک فلگ‌شدن سلف‌بات رو بالا می‌بره)،
# کل لیست GUIDها رو برای مدت کوتاهی (MEMBERS_CACHE_TTL) توی حافظه کش می‌کنیم
# و همه‌ی چک‌ها توی همون بازه از روی کش انجام می‌شن.

MEMBERS_CACHE_TTL = 90  # ثانیه - بعد از این مدت، لیست اعضا دوباره از pyrubi گرفته میشه
_members_cache = {"channel_guid": None, "guids": None, "fetched_at": 0.0}
_members_cache_lock = threading.Lock()


def _pyrubi_fetch_all_member_guids_blocking(channel_guid: str) -> set:
    """کل GUIDهای اعضای عادی + ادمین/سازنده‌ی کانال رو با pyrubi می‌گیره."""
    from pyrubi import Client
    client = Client(PYRUBI_SESSION_NAME)
    guids = set()
...
def _pyrubi_is_member_blocking(channel_guid: str, user_guid: str) -> bool:
    """چک واقعی عضویت یک کاربر، با کش کوتاه‌مدت لیست اعضا. اگه سلف‌بات
    لاگین نباشه یا خطا بده، Exception رو بالا می‌فرسته تا caller بتونه
    fallback مناسب رو انتخاب کنه (مثلاً همون روش فوروارد قبلی).

    نکته: کش فقط برای جواب «عضوه» قابل اعتماده (چون توی این ۹۰ ثانیه بعیده
    کسی همون لحظه لف بده). جواب «عضو نیست» از کش هیچ‌وقت مستقیم قبول
    نمی‌شه - چون دقیقاً همین حالته که باعث میشه کسی تازه جوین‌شده، بخاطر
    لیست کش‌شده‌ی قدیمی، «عضو نیستی» بشنوه. برای جواب منفی، همیشه یه بار
    لیست تازه (بدون کش) گرفته میشه تا مطمئن بشیم واقعاً عضو نیست."""
    now = time.time()
...
def _dig_username(value):
    """مثل _dig_guid ولی برای پیدا کردن username داخل دیکشنری‌های تو در تو
    (author_info که rubka برمی‌گردونه، username رو زیر data.chat.username
    قایم می‌کنه، نه توی سطح بالا)."""
...
def _pyrubi_resolve_user_by_username_blocking(username: str):
    """تنها روش قابل‌اعتماد کشف‌شده برای رسیدن از یوزرنیم به GUID واقعیِ
    فضای pyrubi: get_chat_info_by_username. (author_info بات‌پلتفرم فقط
    sender_id خودِ بات‌پلتفرم رو می‌ده که با GUID واقعی pyrubi فرق داره -
    این دو فضا هیچ ارتباط مستقیمی ندارن مگر از طریق username عمومی کاربر.)"""
    from pyrubi import Client
    client = Client(PYRUBI_SESSION_NAME)
...
async def _resolve_real_sender_guid(message: "Message"):
    """اول از author_info/author_name (پراپرتی‌های async رو‌بکا) دنبال یه
    username می‌گرده - چون GUID خودِ author_info همیشه برابر sender_id
    بات‌پلتفرمه، نه GUID واقعی. اگه username پیدا شد، با pyrubi
    (get_chat_info_by_username) GUID واقعی حساب رو resolve می‌کنه. اگه
    کاربر اصلاً username نداشته باشه، resolve فنی ممکن نیست (محدودیت
    پلتفرم) و None برمی‌گرده تا caller به فلگ has_joined/فوروارد fallback کنه."""
    username = None
    for attr in _REAL_GUID_ATTR_CANDIDATES:
        ...
        if inspect.iscoroutine(val):
            try:
                ...
                logger.exception(f"❌ خطا در await کردن message.{attr}")
                continue
        guid = _dig_guid(val)
        if guid:
            return guid
        ...
        if val not in (None, "", [], {}):
            logger.info(f"🔎 دیباگ {attr}: {repr(val)[:400]}")
    ...
    if username and os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi"):
        try:
            ...
            logger.exception(f"❌ resolve یوزرنیم '{username}' با pyrubi خطا داد")
            return None
        if real_guid:
            logger.info(f"✅ GUID واقعی یوزرنیم '{username}' پیدا شد: {real_guid}")
        return real_guid
    return None


async def check_membership_live(user_guid) -> bool:
    """اول سعی می‌کنه GUID واقعی حساب فرستنده رو (از کش دیتابیس یا با
    await‌کردن message.author_info) پیدا کنه و مستقیم با pyrubi چک واقعی
    عضویت بزنه - این دقیق‌ترین روشه چون دیگه مقایسه‌ی دو فضای شناسه‌ی
    متفاوت نیست. اگه GUID واقعی پیدا نشد یا pyrubi لاگین نبود/خطا داد،
    fallback می‌کنه به فلگ has_joined (که با تایید فوروارد پست کانال ثبت
    می‌شه) تا ربات کلاً قفل نشه."""
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
        return True  # ادمین هنوز کانال تنظیم نکرده
    return has_joined(user_guid)


async def check_membership_live_with_message(message: "Message") -> bool:
    """نسخه‌ی کامل‌تر check_membership_live که خودِ آبجکت message رو داره و
    می‌تونه GUID واقعی رو resolve کنه. handle_text از این استفاده می‌کنه."""
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
        return True

    sender_id = message.sender_id
    if get_setting("realcheck_enabled", "0") == "1" and os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi"):
        real_guid = get_cached_real_guid(sender_id)
        if not real_guid:
            real_guid = await _resolve_real_sender_guid(message)
            if real_guid:
                save_real_guid(sender_id, real_guid)
        if real_guid:
            try:
                live_result = await asyncio.to_thread(_pyrubi_is_member_blocking, channel_guid, real_guid)
            except Exception:
                logger.exception("❌ چک زنده‌ی عضویت با pyrubi خطا داد - fallback به فلگ has_joined")
            else:
                if live_result:
                    if mark_joined(sender_id):
                        _spawn(notify_referrer_on_join(sender_id))
                    return True
                return False  # چک زنده قطعاً میگه عضو نیست

    return has_joined(sender_id)


async def is_member(user_guid, force_refresh: bool = False) -> bool:
    """نقطه‌ی ورودی قدیمی برای چک عضویت - فقط GUID رو داره، نه کل پیام،
    برای همین نمی‌تونه real_guid رو resolve کنه و مستقیم سراغ فلگ
    has_joined می‌ره. جاهایی که خودِ message در دسترسه (مثل handle_text)
    باید به‌جاش check_membership_live_with_message صدا زده بشه."""
    return await check_membership_live(user_guid)


# ─── تایید عضویت با فوروارد پست کانال (fallback وقتی realcheck جواب نده) ────
# چون sender_id (فضای بات‌پلتفرم) و GUID واقعی حساب (فضای pyrubi) در حالت
# کلی قابل مقایسه‌ی مستقیم نیستن، به‌عنوان یه راه پشتیبان، عضویت رو از راه
# فوروارد هم می‌شه تایید کرد: از کاربر می‌خوایم یه پست از خودِ کانال رو به
# این چت فوروارد کنه. GUID مبدأ فوروارد با channel_guid تنظیم‌شده مقایسه
# می‌شه - این مقایسه چون هر دو طرف از فضای عمومی/بات‌پلتفرم کانال هستن (نه
# GUID شخصی کاربر)، قابل اعتماده.
...
def _dig_guid(value):
    """از یه دیکشنری/شیء یا خودِ رشته، دنبال چیزی که شبیه GUID باشه می‌گرده
    (پیشوند c0/g0/b0 برای کانال/گروه/بات، u0 برای کاربر)."""
    if value is None:
        return None
...
def _safe_getattr_skip_coroutine(obj, attr):
    """مثل getattr عادی، ولی اگه مقدار یه کوروتینِ await‌نشده باشه (که rubka
    برای بعضی پراپرتی‌ها مثل author_info/username داره)، به‌جای رها کردنش
    (که باعث RuntimeWarning و لیک می‌شه) می‌بندتش و None برمی‌گردونه - چون
    توی یه اسکن عمومی/سینک نمی‌خوایم کوروتین‌های ناشناس رو کورکورانه await
    کنیم."""
    try:
...
def _extract_forward_channel_guid(message: "Message"):
    """اگه پیام فوروارد‌شده باشه، GUID مبدأ (کانال اصلی) رو در میاره. چون
    اسم دقیق فیلد توی rubka مستند نیست، چند تا اسم رایج رو امتحان می‌کنه؛
    اگه هیچ‌کدوم جواب نداد، دنبال هر اتریبیوتی که اسمش 'forward' داره
    می‌گرده (همون الگوی _extract_incoming_file)."""
...
def _debug_forward_dump(message: "Message") -> str:
    """فقط برای دیباگ: هر اتریبیوتی که اسمش 'forward' داره رو با مقدارش
    برمی‌گردونه، تا اگه حدس‌های بالا جواب ندادن، توی لاگ ریلوی دیده بشه و
    بشه اسم واقعی فیلد رو پیدا کرد. کوروتین‌های await‌نشده رو می‌بنده تا
    RuntimeWarning نده."""
    found = {}
    try:
        for attr in dir(message):
            if attr.startswith("_") or "forward" not in attr.lower():
                continue
            v = _safe_getattr_skip_coroutine(message, attr)
            if v not in (None, "", [], {}):
                found[attr] = repr(v)[:300]
    except Exception:
        pass
    return repr(found)
...
def not_joined_keypad():
    b = InlineBuilder()
    b = b.row(b.button_simple("check_join", "✅ بررسی عضویت"))
    return b.build()


async def send_not_joined(message=None):
    link = get_setting("channel_link")
    can_auto_check = False
    if message is not None and get_setting("realcheck_enabled", "0") == "1" and os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi"):
        real_guid = get_cached_real_guid(message.sender_id) or await _resolve_real_sender_guid(message)
        can_auto_check = bool(real_guid)

    if can_auto_check:
        text = "🔒 برای استفاده از ربات، اول عضو کانال شو، بعد دکمه‌ی زیر رو بزن."
    else:
        text = "🔒 برای استفاده از ربات، اول عضو کانال شو، بعد یه پست ازش فوروارد کن."
    if link:
        text += f"\n🔗 {link}"
    await _rx(message.reply_inline(text, not_joined_keypad()))


async def handle_check_join_callback(message):
    """دکمه‌ی «بررسی عضویت»: اول سعی می‌کنه با GUID واقعی (از author_info)
    یه چک زنده‌ی درست با pyrubi بزنه؛ اگه نشد، فلگ has_joined (که با
    فوروارد پست کانال هم ثبت می‌شه) رو چک می‌کنه."""
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
    ...
    await _rx(message.reply("🔎 در حال بررسی عضویت..."))
    ...
    if member:
        ...
        await _rx(message.reply(
            welcome or (
                "✅ عضویت شما تایید شد!\n\n"
                "🔗 لینک پست روبینو، اینستاگرام، تیک‌تاک یا پینترست بفرست تا دانلودش کنم.\n"
                "🎵 اسم یک آهنگ بنویس تا برات پیدا و دانلودش کنم.\n"
                "🎬 یا خودِ ویدیو/ویس رو بفرست، آهنگش رو برات پیدا می‌کنم.\n\n"
                "/help — راهنما"
            )
        ))
        return
...
# وقتی صدای ویدیو دقیقاً با نسخه‌ی اصلی مچ نشه (کیفیت پایین، افکت پس‌زمینه، یا
# اینکه دقیقاً همون تیکه از یه ریمیکس/مدلی آپلود شده)، Shazam/AudD بجای آهنگ
# تکی، به یه فایل «Medley»/«Mix» (مجموعه‌ی چندتا آهنگ تو یه فایل، از یه DJ)
# مچ می‌کنن که کاملاً نتیجه‌ی غلطیه - این‌جور نتایج رو رد می‌کنیم تا به‌جاش
# آفست بعدی یا فال‌بک بعدی امتحان بشه.
_MEDLEY_TITLE_PATTERNS = [
    "medley", "megamix", "mega mix", "nonstop", "non-stop", "non stop",
    "mixtape", "dj mix", "mix vol", "vol.", " vol ", "remix collection",
    "بی‌کلام مدلی", "میکس آهنگ", "میکس بی وقفه",
]


def _is_medley_result(title: str) -> bool:
    t = (title or "").lower()
    return any(p in t for p in _MEDLEY_TITLE_PATTERNS)
        ...
        try:
            ...
            if os.path.exists(tmp_audio) and os.path.getsize(tmp_audio) > 1000:
                ...
                if track:
                    ...
                    if _is_medley_result(title):
                        # این آفست به یه فایل «Medley/Mix» مچ شده، نه آهنگ تکی -
                        # قبولش نمی‌کنیم و می‌ریم سراغ آفست بعدی، شاید اونجا
                        # مچ تمیزتری پیدا بشه.
                        logger.warning(f"shazam ss={ss} یه نتیجه‌ی Medley/Mix داد، رد شد: {title}")
                        continue
                    return {"title": title, "artist": track.get("subtitle", "")}
        except Exception as e:
...
def _audd_detect_sync(input_path):
    """فال‌بک تشخیص آهنگ با AudD - وقتی Shazam چیزی پیدا نکرد صدا زده میشه.
    برخلاف shazamio (که غیررسمی و گاهی به‌خاطر تغییرات سرور شزم قطع میشه)،
    AudD یه API رسمیه که با api_token کار می‌کنه (رایگان تا ۳۰۰ درخواست)."""
    try:
        probe = subprocess.run(
            ["ffprobe", "-v", "error", "-show_entries", "format=duration",
             "-of", "default=noprint_wrappers=1:nokey=1", input_path],
    ...
    # AudD حداکثر ۲۵ ثانیه/۱۰ مگابایت قبول می‌کنه؛ برای همین کلیپ کوتاه می‌گیریم.
    offsets = [0] if duration <= 20 else [0, int(duration / 2)]
    for ss in offsets:
        tmp_audio = f"{input_path}_audd_{ss}.wav"
        try:
            subprocess.run(
                [_FFMPEG_EXE, "-y", "-i", input_path, "-ss", str(ss), "-t", "18",
                 "-vn", "-ar", "44100", "-ac", "2", "-f", "wav", tmp_audio],
                capture_output=True, timeout=30,
            )
            if not (os.path.exists(tmp_audio) and os.path.getsize(tmp_audio) > 1000):
                continue
            ...
            if data.get("status") == "success" and data.get("result"):
                ...
                if _is_medley_result(title):
                    logger.warning(f"audd ss={ss} یه نتیجه‌ی Medley/Mix داد، رد شد: {title}")
                    continue
                return {"title": title, "artist": result.get("artist", "")}
            elif data.get("status") == "error":
                # کد ۹۰۱ یعنی سقف توکن تستی/رایگان تموم شده - این یه خطای واقعی نیست،
                # فقط لاگ می‌کنیم و دیگه بار دیگه امتحان نمی‌کنیم.
                logger.warning(f"audd error: {data.get('error')}")
                return None
        except Exception as e:
            logger.warning(f"audd offset ss={ss} error: {e}")
        finally:
            if os.path.exists(tmp_audio):
                try:
                    os.remove(tmp_audio)
...
def detect_song_sync(input_path):
    """اجرای همزمان (sync) تشخیص آهنگ: اول Shazam، اگه چیزی پیدا نکرد فال‌بک AudD.
    هر thread حلقه‌ی asyncio خودش رو برای Shazam می‌سازه."""
    try:
        result = asyncio.run(_shazam_detect_async(input_path))
        if result:
            return result
    except Exception as e:
        logger.warning(f"shazam detect_song_sync error: {e}")

    try:
        return _audd_detect_sync(input_path)
    except Exception as e:
        logger.warning(f"audd detect_song_sync error: {e}")
        return None
...
def _rubika_upload_file_with_retries(file_type: str, file_path: str, mime_type: str,
                                      upload_url: str = None, file_name: str = None,
                                      max_attempts: int = 3) -> str:
    """مرحله‌ی آپلود واقعی فایل (نه requestSendFile، نه sendFile) - جدا شده
    چون این دقیقاً همون جایی بود که خطای «Connection aborted / write operation
    timed out» می‌داد، حتی بعد از یه بار retry دستی، و حتی توی reply_music
    خودِ rubka. علت اصلی: این آپلودها از HTTP_SESSION مشترک (با
    pool_connections/pool_maxsize و keep-alive) استفاده می‌کردن. یه اتصال
    pooled که یه مدت بیکار مونده ممکنه سمت سرور/شبکه بسته شده باشه بدون
    اینکه سمت ما خبردار بشیم؛ دفعه‌ی بعد که همون اتصال برای یه آپلود (که طول
    می‌کشه) دوباره استفاده بشه، نتیجه‌اش دقیقاً همینه: اتصال «باز» به نظر
    میاد ولی نوشتن روش هنگ می‌کنه تا timeout. برای همین اینجا (فقط اینجا،
    نه بقیه‌ی جاهای HTTP_SESSION که JSON کوچیک و سریعن) عمداً:
      ۱) یه request مستقل با requests.post (بدون Session مشترک) می‌سازیم،
         به‌علاوه‌ی هدر Connection: close تا هیچ keep-alive‌ای درکار نباشه.
      ۲) تا max_attempts بار با backoff (0/3/8 ثانیه) و upload_url تازه در
         هر تلاش، دوباره امتحان می‌کنیم - چون یه تلاش فوری دوباره روی همون
         مسیر شبکه‌ی شلوغ/خراب معمولاً همون نتیجه رو میده."""
    attempt_timeouts = [(20, 90), (20, 150), (20, 240)][:max_attempts]
    backoffs = [0, 3, 8][:max_attempts]
    name = file_name or os.path.basename(file_path)
    last_err = None
    for i, timeout in enumerate(attempt_timeouts):
        if backoffs[i]:
            time.sleep(backoffs[i])
        if not upload_url:
            try:
                upload_url = _rubika_request_upload_url_blocking(file_type)
            except Exception as e:
                last_err = e
                logger.warning(f"گرفتن upload_url تازه ({file_type}, تلاش {i+1}) شکست خورد: {e}")
                continue
        try:
            host = urllib.parse.urlparse(upload_url).netloc
            ...
            last_err = e
            logger.warning(f"آپلود {file_type} تلاش {i+1}/{len(attempt_timeouts)} به هاست {host} شکست خورد: {e}")
            upload_url = None  # تلاش بعدی حتماً یه upload_url تازه بگیره
    raise RuntimeError(f"آپلود {file_type} بعد از {len(attempt_timeouts)} تلاش شکست خورد: {last_err}")
...
def _rubika_send_video_blocking(chat_id: str, file_path: str, caption: str = "", file_name: str = None, inline_keypad: dict = None, upload_url: str = None) -> dict:
    # ۱+۲) گرفتن upload_url (اگه از قبل موازی با دانلود گرفته نشده) و آپلود
    # خودِ فایل - با retry/backoff و بدون keep-alive (توضیح کامل بالای تابع
    # _rubika_upload_file_with_retries).
    file_id = _rubika_upload_file_with_retries("Video", file_path, "video/mp4",
...
def _retry_download_keypad():
    b = InlineBuilder()
    b = b.row(b.button_simple("retry_download", "🔁 امتحان دوباره"))
    return b.build()


async def _fail_with_retry(status, message: Message, fail_text: str):
    """پیام خطا رو نشون میده و یه دکمه‌ی «امتحان دوباره» زیرش می‌ذاره، تا کاربر
    مجبور نباشه لینک رو از اول کپی/پیست کنه (مثلاً وقتی سرور مقصد یا API
    واسط موقتاً قطع بوده و با یه بار دیگه امتحان‌کردن درست میشه)."""
    await _rx(status.edit(fail_text))
    await _rx(message.reply_inline("می‌خوای دوباره امتحان کنم؟", _retry_download_keypad()))


async def handle_retry_download_callback(message: Message):
    """کلیک روی دکمه‌ی «🔁 امتحان دوباره» زیر پیام خطای دانلود - همون
    درخواست قبلی (اینستا/تیک‌تاک/پینترست/روبینو) رو دوباره اجرا می‌کنه."""
    chat_id = message.chat_id
    req = _last_download_request.get(chat_id)
    if not req:
        await _rx(message.reply("❌ درخواستی برای امتحان دوباره پیدا نشد؛ لینک رو دوباره بفرست."))
        return
    kind, text = req
    if kind == "instagram":
...
async def handle_instagram(message: Message, text: str):
    chat_id = message.chat_id
    _last_download_request[chat_id] = ("instagram", text)
    ...
    try:
        ...
        if downloaded:
            ...
            upload_url_task.cancel()
            await _fail_with_retry(status, message, "❌ دانلود ممکن نشد. ممکنه پست خصوصی باشه یا لینک اشتباه باشه.")
    except Exception as e:
        ...
        try:
            ...
            pass
        try:
            await _fail_with_retry(status, message, f"❌ خطا: {e}")
        except Exception:
            pass
    finally:
        if os.path.exists(video_path):
...
async def handle_tiktok(message: Message, text: str):
    chat_id = message.chat_id
    _last_download_request[chat_id] = ("tiktok", text)
    ...
    try:
        ...
        if downloaded:
            ...
            upload_url_task.cancel()
            await _fail_with_retry(status, message, "❌ دانلود از تیک‌تاک ممکن نشد. لینک رو چک کن یا بعداً امتحان کن.")
    except Exception as e:
...
async def handle_pinterest(message: Message, text: str):
    chat_id = message.chat_id
    _last_download_request[chat_id] = ("pinterest", text)
    ...
    try:
        ...
        if file_path:
            ...
            await _fail_with_retry(status, message, "❌ دانلود پینترست ممکن نشد، دوباره امتحان کن.")
    except Exception as e:
...
async def handle_rubino(message: Message, text: str):
    chat_id = message.chat_id
    _last_download_request[chat_id] = ("rubino", text)
    ...
    try:
        ...
        if file_path:
            ...
            await _fail_with_retry(status, message, "❌ دانلود از روبینو ممکن نشد. شاید پست خصوصیه یا ساختار صفحه فرق داره.")
    except Exception as e:
...
try:
    ...
    def _text_score(query, target):
        if not query or not target:
            return 0.0
        tsr = _fuzz.token_set_ratio(query, target)
        pr = _fuzz.partial_ratio(query, target)
        # کوئری‌های کوتاه (اسم مستعار مثل «تتلو») با partial_ratio راحت با یه
        # تیکه‌ی کاملاً نامرتبط از متن هدف امتیاز بالا می‌گیرن، چون
        # partial_ratio فقط دنبال بهترین زیررشته‌ست، نه شباهت کلی عبارت.
        # هرچی کوئری کوتاه‌تر باشه وزن partial_ratio رو کمتر و وزن
        # token_set_ratio (که کل عبارت رو می‌سنجه) رو بیشتر می‌کنیم.
        if len(query) <= 6:
            return tsr * 0.85 + pr * 0.15
        return tsr * 0.6 + pr * 0.4
except Exception as _e:
    import difflib
...
_JUNK_TITLE_PATTERNS = [
    ...
    # جستجوی آرتیست‌های فارسی/کمترشناخته با اسم فارسی معمولاً توی کاتالوگ
    # رسمی (Deezer/YTMusic) نتیجه نمیده و می‌افته روی جستجوی خام یوتیوب؛
    # اونجا پر از کاورهای هوش‌مصنوعی/صداسازی جعلی از خواننده‌ست که اصلاً
    # آهنگ اصلی نیست - باید کاملاً حذف بشن، نه فقط جریمه امتیاز.
    r"هوش\s*مصنوعی", r"\bai\s*cover\b", r"\bai\s*voice\b", r"\bvoice\s*clone\b",
    r"\bdeep\s*fake\b", r"دیپ\s*فیک", r"\bfake\s*voice\b", r"شبیه\s*ساز",
]
...
def _has_persian(text):
    """تشخیص اینکه عبارت شامل حروف فارسی/عربی هست یا نه."""
    return bool(re.search(r"[\u0600-\u06FF]", text or ""))


# کانال‌های عمومی/آگرگاتور که نباید به‌عنوان «کانال رسمی آرتیست» انتخاب بشن
# (میکس‌های ژانری، کانال‌های بدون کپی‌رایت، کامپایلیشن و ...).
_GENERIC_CHANNEL_PATTERNS = [
    r"various\s*artists", r"music\s*mix", r"no\s*copyright", r"\bncs\b",
    r"lyrics?\s*video", r"full\s*album", r"compilation", r"playlist",
    r"^topic$",
]


def _resolve_artist_channel(query):
    """وقتی جستجوی کاتالوگی (Deezer/YTMusic) با اسم فارسی/کمترشناخته چیزی
    پیدا نمی‌کنه یا نتایج بی‌ربط میده: مستقیم توی یوتیوب (که تشخیص متن
    فارسی‌ش خیلی بهتره) عبارت رو سرچ می‌کنیم و از بین نتایج، کانالی که
    بیشترین تکرار رو داره به‌عنوان «کانال رسمی آرتیست» انتخاب می‌کنیم -
    دقیقاً همون کاری که یه آدم می‌کرد: تو گوگل/یوتیوب اسم فارسی رو می‌زنه،
    می‌بینه کانال واقعیش (که اسمش معمولاً انگلیسیه) کدومه."""
    try:
        with yt_dlp.YoutubeDL({"quiet": True, "extract_flat": True, "no_warnings": True}) as ydl:
            info = ydl.extract_info(f"ytsearch20:{query}", download=False)
            entries = info.get("entries") or []
    except Exception as e:
        logger.warning(f"artist channel search error: {e}")
        return None

    counts = {}  # channel_url -> {"name":, "count":}
    for entry in entries:
        title = entry.get("title", "")
        if _title_has_any(title, _JUNK_TITLE_PATTERNS):
            continue
        uploader = entry.get("channel") or entry.get("uploader") or ""
        if not uploader or _title_has_any(uploader, _GENERIC_CHANNEL_PATTERNS):
            continue
        channel_id = entry.get("channel_id")
        channel_url = (f"https://www.youtube.com/channel/{channel_id}" if channel_id
                       else entry.get("channel_url") or entry.get("uploader_url"))
        if not channel_url:
            continue
        item = counts.setdefault(channel_url, {"name": uploader, "count": 0})
        item["count"] += 1

    if not counts:
        return None
    best_url = max(counts, key=lambda u: counts[u]["count"])
    return {"name": counts[best_url]["name"], "channel_url": best_url}


def _get_channel_songs_by_views(channel_url, artist_name, limit=15):
    """لیست ویدیوهای یه کانال، مرتب‌شده بر اساس تعداد بازدید (نزدیک‌ترین
    معیار در دسترس به «محبوبیت» بدون نیاز به کاتالوگ رسمی) - دقیقاً همون
    چیزی که کاربر خواسته: به‌جای اسم فارسیِ سرچ‌شده، آهنگ‌های واقعیِ خودِ
    آرتیست از کانال خودش، مرتب بر اساس ویو.

    بعضی کانال‌ها (مخصوصاً کانال‌های خودکارِ «Topic» که یوتیوب برای
    آرتیست‌ها می‌سازه) اصلاً تب «Videos» ندارن و مستقیم رفتن به .../videos
    ارور «This channel does not have a videos tab» می‌ده. راه مطمئن‌تر:
    هر کانالی، صرف‌نظر از اینکه تب videos داره یا نه، یه پلی‌لیست خودکار
    «uploads» داره که با تعویض پیشوند UC->UU توی channel_id به‌دست میاد -
    این رو اول امتحان می‌کنیم؛ اگه channel_id قابل استخراج نبود یا نتیجه
    نداد، به روش قبلی (تب videos) برمی‌گردیم."""
    ydl_opts = {"quiet": True, "extract_flat": True, "no_warnings": True, "playlistend": limit * 4}

    def _extract(url):
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info = ydl.extract_info(url, download=False)
            entries = info.get("entries") or []
            if len(entries) == 1 and entries[0].get("entries"):
    ...
    if m:
        uploads_playlist = "UU" + m.group(1)[2:]
        try:
            entries = _extract(f"https://www.youtube.com/playlist?list={uploads_playlist}")
        except Exception as e:
            logger.warning(f"channel uploads-playlist fetch error: {e}")
            entries = []
    ...
    if not entries:
        ...
        if not target.endswith("/videos"):
            target += "/videos"
        try:
            entries = _extract(target)
        except Exception as e:
            logger.warning(f"channel videos fetch error: {e}")
            return []
    ...
    for entry in entries:
        ...
        if duration and duration > 900:  # >۱۵ دقیقه یعنی احتمالاً آهنگ تکی نیست
            continue
        out.append({
            "title": title,
            "artist": artist_name,
...
def search_songs(query):
    ...
    # نتایج ضعیف‌تر از این حد اصلاً نمایش داده نشن؛ بهتره بگه «پیدا نشد» تا
    # اینکه یه چیز کاملاً نامرتبط (که فقط چندتا حرف مشترک با کوئری داشته)
    # به‌جای آهنگ نشون بده.
    ...
    # نتایج ضعیف/بی‌ربط بودن (مثلاً وقتی اسم فارسی سرچ شده و کاتالوگ رسمی
    # چیزی نداشت و فقط جستجوی خام یوتیوب پر شده) یعنی باید کانال واقعی
    # آرتیست رو پیدا کنیم و آهنگ‌های خودش رو مرتب بر اساس ویو جایگزین کنیم.
    strong_enough = bool(results) and results[0]["score"] >= 65
    if not strong_enough:
        try:
            channel = _resolve_artist_channel(query)
        except Exception as e:
            logger.warning(f"artist channel resolve error: {e}")
            channel = None
        if channel:
            try:
                channel_songs = _get_channel_songs_by_views(
                    channel["channel_url"], channel["name"], limit=15)
            except Exception as e:
                logger.warning(f"channel songs fetch error: {e}")
                channel_songs = []
            if channel_songs:
                results = [
                    {"title": s["title"], "artist": s["artist"], "score": 80,
                     "source": "channel", "video_id": s["video_id"]}
                    for s in channel_songs
                ]
                return results[:10], channel["channel_url"], channel["name"], "channel"

    return results[:10], top_artist_id, top_artist_name, top_artist_source


def get_artist_tracks(artist_id, source="deezer", artist_name=None):
    ...
    آرتیست به‌عنوان جایگزین top-tracks استفاده میشه؛ برای آرتیست‌هایی که از
    روی کانال یوتیوبشون شناسایی شدن (اسم فارسی که کاتالوگ رسمی نداشت)،
    مستقیم ویدیوهای کانال خودش رو بر اساس ویو مرتب می‌کنیم."""
    if source == "channel":
        # قبلاً ۴۰ بود؛ چون حالا لیست صفحه‌بندی‌شده ارسال میشه (به‌جای یه پیام
        # غول‌پیکر که سمت روبیکا رد می‌شد)، می‌تونیم آهنگ بیشتری برگردونیم تا
        # واقعاً «همه‌ی» آهنگ‌های موجود از کانال آرتیست رو پوشش بده.
        return _get_channel_songs_by_views(artist_id, artist_name or "", limit=80)
    if source == "ytmusic":
        try:
            yt = _get_ytmusic()
...
def download_song_file(title, artist, chat_id, video_id=None):
    ...
    ydl_opts = {
        ...
        "fragment_retries": 2,
        "extractor_retries": 2,
        # کلاینت اندروید/iOS رو به یوتیوب معرفی می‌کنیم تا خطای «Sign in to confirm
        # you're not a bot» که مخصوص کلاینت وب پیش‌فرضه (و رو IP سرورهای Railway
        # خیلی زیاد رخ می‌ده) دور زده بشه. اگه این هم کافی نبود، کوکی از
        # YTDLP_COOKIES_FILE اضافه میشه که قوی‌تره.
        # از اواخر ۲۰۲۵/اوایل ۲۰۲۶ به بعد، جعل کلاینت به‌تنهایی دیگه رو IPهای
        # دیتاسنتر کافی نیست (یوتیوب SABR-only rollout). به‌عنوان یه لایه‌ی
        # fallback رایگانِ اضافه (بدون نیاز به کوکی)، پلاگین yt-dlp-invidious
        # نصب شده - این پلاگین خودکار وقتی همون خطای «Sign in to confirm...»
        # از یوتیوب برگرده، درخواست رو از طریق یه سرور عمومی Invidious دوباره
        # امتحان می‌کنه. اینجا فقط max_retries/retry_interval رو تنظیم
        # می‌کنیم تا سر یه سرور Invidious کند/بی‌جواب زیادی معطل نمونه.
        "extractor_args": {
            "youtube": {"player_client": ["android", "ios"]},
            "invidious": {"max_retries": ["3"], "retry_interval": ["3"]},
        },
    }
...
def _dedupe_artist(title, artist):
    """اگه اسم آرتیست از قبل تو خودِ عنوان باشه (که برای آهنگ‌های گرفته‌شده
    از کانال یوتیوب آرتیست خیلی پیش میاد، چون خودِ آپلودکننده تایتل رو
    «آرتیست - آهنگ» می‌ذاره)، دوباره در آخر اضافه‌ش نمی‌کنیم تا اسم آرتیست
    دوبار تکرار نشه - نه توی برچسب دکمه، نه توی اسم فایلِ ارسالی."""
    title = title or ""
    artist = artist or ""
...
def _song_label(title, artist, max_len=42):
    """برچسب دکمه‌ی «عنوان - آرتیست» با حذف تکرار اسم آرتیست."""
    return _dedupe_artist(title, artist)[:max_len]


# ─── ارسال صفحه‌بندی‌شده‌ی لیست آهنگ‌ها ────────────────────────────────────
# باگ اصلیِ «دکمه‌ی همه‌ی آهنگ‌های آرتیست هیچی نمیاره»: وقتی یه آرتیست ۴۰-۵۰
# تا آهنگ داشت، همه‌شون یه‌جا توی یه inline_keypad با ۴۰+ ردیف دکمه فرستاده
# می‌شدن. روبیکا برای تعداد زیاد دکمه توی یه پیام ارور می‌ده (send fail)،
# و چون خطا فقط لاگ می‌شد و به کاربر نشون داده نمی‌شد، نتیجه‌ش این بود که
# کاربر هیچ پیام/دکمه‌ای نمی‌دید - انگار ربات "چیزی نیاورده". راه‌حل: لیست
# رو به تکه‌های کوچیک (هر پیام حداکثر CHUNK_SIZE دکمه) تقسیم می‌کنیم و هر
# تکه رو توی یه پیام جدا می‌فرستیم، دقیقاً همون سایزی که همیشه سالم کار
# کرده (نتایج جستجوی معمولی هم حداکثر ۱۰ دکمه‌ن).
CHUNK_SIZE = 10


async def _send_song_list_paginated(message: Message, tracks: list, header: str, extra_row_builder=None):
    """tracks رو به بخش‌های CHUNK_SIZE‌تایی تقسیم می‌کنه و هر بخش رو توی یه
    پیام جدا با دکمه‌های dl_<index-اصلی-توی-tracks> می‌فرسته. extra_row_builder
    (اختیاری) یه تابعه که builder رو می‌گیره و ردیف(های) اضافه (مثل دکمه‌ی
    جستجوی دوباره) رو فقط به آخرین صفحه اضافه می‌کنه."""
    total = len(tracks)
    total_pages = max(1, (total + CHUNK_SIZE - 1) // CHUNK_SIZE)
    for page in range(total_pages):
        ...
        for offset, t in enumerate(chunk):
            i = start + offset
            label = _song_label(t["title"], t["artist"])
            builder = builder.row(builder.button_simple(f"dl_{i}", label))
        is_last_page = (page == total_pages - 1)
        if is_last_page and extra_row_builder:
            builder = extra_row_builder(builder)
        inline_keypad = builder.build()
        page_label = f"{header} ({page + 1}/{total_pages}):" if total_pages > 1 else f"{header}:"
        try:
            await _rx(message.reply_inline(page_label, inline_keypad))
        except Exception:
            logger.exception(f"خطا در ارسال صفحه‌ی {page + 1} از {total_pages} لیست آهنگ‌ها")
            if page == 0:
                # حتی صفحه‌ی اول هم نرفت - کاربر باید بفهمه، نه اینکه سکوت کنه.
                await _rx(message.reply("❌ خطا در ارسال لیست آهنگ‌ها، دوباره امتحان کن."))
            return


_LYRIC_TITLE_NOISE_PREFIXES = [
    "دانلود آهنگ جدید", "دانلود اهنگ جدید", "دانلود آهنگ", "دانلود اهنگ",
    "دانلود موزیک جدید", "دانلود موزیک", "متن و دانلود آهنگ", "متن آهنگ", "دانلود",
]
_LYRIC_TITLE_NOISE_WORDS = [
    "با کیفیت 320", "با کیفیت 128", "کیفیت 320", "کیفیت 128", "کیفیت عالی",
    "+ متن کامل", "+ متن", "متن کامل", "متن ترانه", "پخش آنلاین", "به همراه متن",
    "با لینک مستقیم", "لینک مستقیم", "MP3", "mp3",
]
_LATIN_JUNK_WORDS = {
    "mp3", "official", "video", "music", "lyrics", "hd", "new", "full",
    "download", "song", "audio", "high", "quality", "link", "direct", "text",
}


def _clean_lyrics_search_title(title: str):
    """از تیتر صفحه‌ی سایت‌های دانلود آهنگ (که معمولاً «دانلود آهنگ [خواننده] [اسم]»
    هستن) نویزهای رایج رو حذف می‌کنه تا یه کوئری تمیز برای search_songs بمونه."""
    if not title:
        return None
    t = title.strip()
    for p in _LYRIC_TITLE_NOISE_PREFIXES:
        if t.startswith(p):
            t = t[len(p):].strip()
            break
    for sep in [" | ", "|", " – ", " — ", " - "]:
        if sep in t:
            ...
            # اگه بخش بعد از جداکننده کوتاه بود، احتمالاً اسم سایته نه بخشی از اسم آهنگ
            if len(tail.strip()) <= 25:
                t = head.strip()
    for w in _LYRIC_TITLE_NOISE_WORDS:
        t = t.replace(w, "")
...
_PERSIAN_STOPWORDS = {
    "من", "تو", "او", "ما", "شما", "اونا", "اونها", "که", "به", "از", "با", "تا",
    "را", "رو", "هم", "یا", "و", "در", "این", "اون", "آن", "یه", "یک", "نه",
    "بی", "چی", "چه", "هر", "همه", "فقط", "دیگه", "بازم", "الان", "حالا",
    "ها", "های", "بود", "هست", "نیست", "اگه", "اگر", "برای", "روی", "زیر",
    "کنم", "کنی", "کنه", "شد", "شده", "میشه", "بشه", "چون", "پس", "اما", "ولی",
}


def _persian_relevance_ratio(query: str, text: str) -> float:
    """نسبت کلمات فارسیِ *معنادار* شعرِ کاربر که تو یه نتیجه‌ی سرچ هم اومدن.
    قبلاً یه شمارش خام (بدون فیلتر) استفاده می‌شد که برای شعرهای کوتاه (۴-۵
    کلمه) خیلی راحت با کلمات پرکاربرد/بی‌معنی مثل «تو»، «که»، «رو» ارضا می‌شد
    و صفحات نسبتاً بی‌ربط رو هم قبول می‌کرد. حالا حروف اضافه/ضمایر رو حساب
    نمی‌کنیم و به‌جای عدد مطلق، نسبت رو می‌سنجیم تا برای شعرهای کوتاه هم
    استاندارد به همون اندازه سخت‌گیرانه بمونه."""
    def meaningful_words(s):
        return set(
            w for w in re.findall(r"[\u0600-\u06FF]+", s or "")
            if len(w) >= 2 and w not in _PERSIAN_STOPWORDS
        )
    qw = meaningful_words(query)
    if not qw:
        return 0.0
    return len(qw & meaningful_words(text)) / len(qw)



def _extract_latin_song_name(text: str):
    """خیلی از سایت‌های دانلود آهنگ، تیترشونو عین متن شعر (فارسی) می‌ذارن که تو
    کاتالوگ Deezer/YouTube Music مچ نمی‌شه. ولی بعضی‌هاشون (مثلاً ساخا‌موزیک)
    اسم فینگلیش/لاتین واقعی آهنگ («Masoud Sadeghloo Roozaye Sakht») رو هم تو
    تیتر یا توضیحات میذارن - این دقیقاً همون فرمتیه که کاتالوگ باهاش خوب مچ
    میشه (خودِ نتایج کاتالوگ همیشه لاتین/فینگلیش‌ن). این تابع بلندترین تیکه‌ی
    معتبر لاتین (حداقل ۲ کلمه، نه فقط کلمات نویز مثل mp3/official) رو پیدا می‌کنه."""
    if not text:
        return None
    candidates = re.findall(r"[A-Za-z][A-Za-z0-9()&'\-]*(?:\s+[A-Za-z0-9()&'\-]+)*", text)
    best = None
    for c in candidates:
        words = [w for w in c.split() if w]
        if len(words) < 2:
            continue
        if all(w.strip("()'-").lower() in _LATIN_JUNK_WORDS for w in words):
            continue
...
def google_lyrics_search(query: str):
    """سرچ عمومی وب (با DuckDuckGo از طریق ddgs) برای وقتی کاربر یه تیکه از شعر
    آهنگ رو فرستاده، نه اسمش. برخلاف Google CSE، نیازی به API key نداره - برای
    همین اگه پکیج ddgs نصب نشده باشه، بی‌صدا None برمی‌گردونه تا جریان عادی
    (پیام «آهنگی پیدا نشد») ادامه پیدا کنه.
    فقط نتایجی که حداقل نصف کلمات معنادار شعر رو داشته باشن (بدون احتساب حروف
    اضافه/ضمایر) در نظر
    گرفته میشن (وگرنه نتایج کاملاً بی‌ربط که ddgs گاهی برمی‌گردونه، به اشتباه
    قبول میشدن). از بین نتایج مرتبط، اولویت با تیکه‌ی لاتین/فینگلیش‌شونه چون
    خیلی دقیق‌تر از تیتر خام فارسی با کاتالوگ مچ میشه."""
    try:
        from ddgs import DDGS
    except Exception as import_err:
        logger.warning(f"ddgs نصب نیست، فال‌بک جستجوی شعر غیرفعاله: {import_err}")
        return None
    try:
        with DDGS() as ddgs:
            items = ddgs.text(f"{query} آهنگ", region="ir-fa", max_results=8) or []
    except Exception as e:
        logger.warning(f"ddgs lyric search error: {e}")
        return None
    relevant = [
        item for item in items
        if _persian_relevance_ratio(query, item.get("title", "") + " " + item.get("body", "")) >= 0.5
    ]
    if not relevant:
        return None
    for item in relevant:
        latin = _extract_latin_song_name(item.get("title", "")) or _extract_latin_song_name(item.get("body", ""))
        if latin:
            return latin
    for item in relevant:
        cleaned = _clean_lyrics_search_title(item.get("title", ""))
        if cleaned:
            return cleaned
    return None


def _looks_like_lyrics(query: str) -> bool:
    """تشخیص اینکه کوئری احتمالاً یه تیکه از شعره، نه اسم کوتاه آهنگ/خواننده.
    اسم آهنگ‌ها معمولاً ۱ تا ۴ کلمه‌ن؛ یه جمله‌ی کامل از شعر معمولاً طولانی‌تره."""
    q = (query or "").strip()
    return len(q.split()) >= 5 or len(q) >= 28
...
async def handle_music_search(message: Message, query: str):
    ...
    # اگه کوئری شبیه یه تیکه از شعره، جستجوی کاتالوگ (Deezer/YouTube Music) روی
    # کل جمله معمولاً گمراه میشه و به‌جای آهنگ دقیق، لیست آهنگای پرطرفدار همون
    # آرتیست رو برمی‌گردونه (نه نتیجه‌ی خالی، برای همین چک «if not results»
    # به‌تنهایی کافی نبود). برای همین همیشه برای کوئری‌های شعرمانند، اول با
    # ddgs شعر رو سرچ می‌کنیم تا اسم واقعی آهنگ رو پیدا کنیم و نتیجه‌ی
    # دقیق‌تر رو جایگزین نتیجه‌ی حدسی می‌کنیم.
    if _looks_like_lyrics(query):
        lyric_query = await asyncio.to_thread(google_lyrics_search, query)
        if lyric_query and lyric_query.strip().lower() != query.strip().lower():
            lyric_results, l_artist_id, l_artist_name, l_artist_source = await asyncio.to_thread(
                search_songs, lyric_query)
            if lyric_results:
                results, artist_id, artist_name, artist_source = (
                    lyric_results, l_artist_id, l_artist_name, l_artist_source)
    if not results:
    ...
    builder = builder.row(builder.button_simple("music_research", "🔄 جستجوی دوباره"))
    inline_keypad = builder.build()
...
async def handle_all_songs_callback(message: Message):
    ...
    try:
        tracks = await asyncio.to_thread(
            get_artist_tracks, artist_data["id"], artist_data.get("source", "deezer"), artist_data.get("name"))
    except Exception:
        # قبلاً اگه گرفتن لیست آهنگ‌ها هر خطایی می‌داد، این exception بی‌صدا
        # بالا می‌رفت و توی _handle_inline_click فقط لاگ می‌شد - کاربر هیچی
        # نمی‌دید. حالا صریح می‌گیریمش و به کاربر خبر می‌دیم.
        logger.exception("خطا در گرفتن لیست آهنگ‌های آرتیست")
        await _rx(status.edit("❌ خطا در گرفتن لیست آهنگ‌ها، دوباره امتحان کن."))
        return
    if not tracks:
        await _rx(status.edit("آهنگی پیدا نشد"))
        return
    user_search_results[chat_id] = tracks
    await _rx(status.edit(f"🎵 {len(tracks)} آهنگ از {artist_data['name']} پیدا شد:"))

    def _add_research_row(builder):
        return builder.row(builder.button_simple("music_research", "🔄 جستجوی دوباره"))

    await _send_song_list_paginated(
        message, tracks, f"🎵 همه آهنگهای {artist_data['name']}", extra_row_builder=_add_research_row)


async def handle_music_research_callback(message: Message):
    """کلیک روی دکمه‌ی «🔄 جستجوی دوباره» زیر نتایج آهنگ."""
    await _rx(message.reply("🔎 اسم آهنگ یا خواننده‌ی بعدی رو بفرست."))


async def handle_back_to_results_callback(message: Message):
    """کلیک روی دکمه‌ی «🔙 برگشت به نتایج» - وقتی دانلود یه آهنگ خاص ناموفق بود،
    به‌جای مجبورکردن کاربر به سرچ دوباره از صفر، همون لیست نتایج قبلی رو
    (که توی user_search_results ذخیره‌ست) دوباره نشون می‌ده."""
    chat_id = message.chat_id
    results = user_search_results.get(chat_id)
    if not results:
        await _rx(message.reply("❌ نتیجه‌ای برای برگشت وجود نداره، دوباره اسم آهنگ رو بفرست."))
        return
    artist_data = user_artist_data.get(chat_id)

    def _add_extra_rows(builder):
        if artist_data:
            builder = builder.row(builder.button_simple("all_songs", f"همه آهنگهای {artist_data['name']}"))
        return builder.row(builder.button_simple("music_research", "🔄 جستجوی دوباره"))

    await _send_song_list_paginated(message, results, "🎵 نتایج قبلی", extra_row_builder=_add_extra_rows)
...
def _rename_for_send(src_path: str, title: str, artist: str) -> str:
    ...
    display_name = _safe_filename(_dedupe_artist(title, artist))
...
def _music_retry_keypad():
    b = InlineBuilder()
    b = b.row(b.button_simple("back_to_results", "🔙 برگشت به نتایج"))
    b = b.row(b.button_simple("music_research", "🔄 جستجوی دوباره"))
    return b.build()
...
async def handle_download_callback(message: Message, index: int):
    ...
    display = _dedupe_artist(title, artist)
    status = await _rx(message.reply(f"⬇️ در حال دریافت... (۱/۲)\n{display}"))
    ...
    if not path or not os.path.exists(path):
        ...
        await _rx(message.reply_inline("می‌خوای چیکار کنی؟", _music_retry_keypad()))
        return
    try:
        path = _rename_for_send(path, title, artist)
        await _rx(status.edit(f"📤 در حال ارسال... (۲/۲)\n{display}"))
        await send_music_native(message, path, build_caption())
        add_download(chat_id, "music")
        await _rx(status.edit(f"✅ ارسال شد.\n{display}"))
        # اگه آهنگ اشتباه بود (مثلاً کاور/نسخه‌ی غلط)، کاربر بدون اسکرول به بالا
        # بتونه سریع یه گزینه‌ی دیگه از همون نتایج رو امتحان کنه یا دوباره سرچ کنه.
        await _rx(message.reply_inline("اگه آهنگ درستی نبود:", _music_retry_keypad()))
    except Exception as e:
...
_last_request_at = {}     # chat_id -> آخرین زمانِ یک درخواست (برای آنتی‌اسپم)
_last_download_request = {}  # chat_id -> (kind, text) - برای دکمه‌ی «امتحان دوباره» وقتی دانلود خراب میشه
...
def _extract_incoming_file(message: "Message"):
    ...
    # نکته: rubka بعضی پراپرتی‌ها (مثل author_info/username) رو به‌صورت
    # async تعریف کرده - getattr روی اونا یه کوروتین برمی‌گردونه، نه مقدار
    # واقعی؛ برای همین از _safe_getattr_skip_coroutine استفاده می‌کنیم تا هم
    # کوروتین ناخواسته await نشه (RuntimeWarning) و هم به‌غلط file_id ازش
    # استخراج نشه.
    try:
        for attr in [a for a in dir(message) if not a.startswith("_")]:
            f = _safe_getattr_skip_coroutine(message, attr)
            if f is None or callable(f) or f in ("", [], {}):
                continue
            ...
            if song_path and os.path.exists(song_path):
                ...
                for f in glob.glob(f"song_{chat_id}.*") + [song_path]:
                    try:
                        ...
                        pass
                # تشخیص خودکار (Shazam) گاهی اشتباه می‌کنه (مثلاً کاور/ریمیکس)؛
                # یه راه سریع برای سرچ دستی می‌ذاریم تا لازم نباشه از اول کل
                # فایل رو دوباره بفرسته.
                b = InlineBuilder()
                b = b.row(b.button_simple("music_research", "🔍 این آهنگ درست نبود؟ جستجوی دستی"))
                await _rx(message.reply_inline("اگه تشخیص اشتباه بود:", b.build()))
            else:
                b = InlineBuilder()
                b = b.row(b.button_simple("music_research", "🔍 جستجوی دستی"))
                await _rx(status.edit("🎵 آهنگ شناسایی شد ولی دانلودش ممکن نشد."))
                await _rx(message.reply_inline("می‌تونی اسم آهنگ رو دستی هم بفرستی:", b.build()))
        else:
...
def admin_main_keypad():
    ...
    b = b.row(b.button_simple("adm_report_now", "📈 گزارش الان"))
...
async def handle_admin_callback(message: Message, btn_id: str):
    ...
    if btn_id == "adm_back":
        admin_pending.pop(chat_id, None)
        await _rx(message.reply_inline(admin_main_text(), admin_main_keypad()))
    ...
    elif btn_id == "adm_report_now":
        since = datetime.now() - timedelta(days=1)
        text = build_report_text("دستی (۲۴ ساعت اخیر)", since)
        await _rx(message.reply_inline(text, back_keypad()))

    elif btn_id == "adm_daily_report_toggle":
        new_val = "0" if get_setting("daily_report_enabled", "1") == "1" else "1"
        set_setting("daily_report_enabled", new_val)
        await _rx(message.reply_inline(admin_main_text(), admin_main_keypad()))

    elif btn_id == "adm_weekly_report_toggle":
        new_val = "0" if get_setting("weekly_report_enabled", "1") == "1" else "1"
        set_setting("weekly_report_enabled", new_val)
        await _rx(message.reply_inline(admin_main_text(), admin_main_keypad()))

    elif btn_id == "adm_realcheck_toggle":
        if not os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi"):
            await _rx(message.reply_inline(
                "❌ اول باید سلف‌بات لاگین باشه.\nتوی چت خصوصی بات بزن:\n/pyrubi_login 989xxxxxxxxx",
                settings_keypad(),
            ))
            return
        new_val = "0" if get_setting("realcheck_enabled", "0") == "1" else "1"
        set_setting("realcheck_enabled", new_val)
        with _members_cache_lock:
            _members_cache["guids"] = None  # با تغییر حالت، کش قبلی بی‌اعتبار میشه
        status = "🟢 روشن" if new_val == "1" else "🔴 خاموش"
        await _rx(message.reply_inline(f"✅ چک واقعی عضویت (سلف‌بات): {status}", settings_keypad()))
        ...
        text = (
            ...
            f"⏱ فاصله بین درخواست‌ها: {get_setting('cooldown_seconds', '3')} ثانیه\n"
            f"📅 گزارش روزانه: {'🟢 روشن' if get_setting('daily_report_enabled', '1') == '1' else '🔴 خاموش'}\n"
            f"🗓 گزارش هفتگی: {'🟢 روشن' if get_setting('weekly_report_enabled', '1') == '1' else '🔴 خاموش'}\n"
            f"🔍 چک واقعی عضویت (سلف‌بات): "
            f"{'🟢 روشن' if get_setting('realcheck_enabled', '0') == '1' else '🔴 خاموش'}"
            f"{' (سلف‌بات لاگین نیست ⚠️)' if not os.path.exists(PYRUBI_SESSION_NAME + '.pyrubi') else ''}\n"
        )
        ...
        "adm_set_caption", "adm_set_bot_username", "adm_set_referral_target", "adm_set_cooldown",
    ):
        prompts = {
            ...
            "adm_set_cooldown": "⏱ فاصله‌ی زمانی (ثانیه) بین دو درخواست هر کاربر رو بفرست (برای آنتی‌اسپم، مثلاً 3):",
        }
...
def settings_keypad():
    ...
    b = b.row(b.button_simple("adm_set_cooldown", "⏱ فاصله بین درخواست‌ها (آنتی‌اسپم)"))
    b = b.row(b.button_simple("adm_daily_report_toggle", "📅 گزارش روزانه (روشن/خاموش)"))
    b = b.row(b.button_simple("adm_weekly_report_toggle", "🗓 گزارش هفتگی (روشن/خاموش)"))
    b = b.row(b.button_simple("adm_realcheck_toggle", "🔍 چک واقعی عضویت با سلف‌بات (روشن/خاموش)"))
    b = b.row(b.button_simple("adm_back", "🔙 برگشت"))
    return b.build()
...
async def handle_admin_text(message: Message, text: str):
    ...
    elif action == "adm_set_cooldown":
        try:
            val = float(text.strip())
            set_setting("cooldown_seconds", str(val))
            await _rx(message.reply(f"✅ فاصله‌ی بین درخواست‌ها به {val} ثانیه تغییر کرد."))
        except Exception:
            await _rx(message.reply("❌ یک عدد وارد کن (مثلاً 3 یا 2.5)."))
...
@bot.on_message(commands=["cancel"])
async def cancel_cmd(bot: Robot, message: Message):
    """دستور عمومی برای بیرون اومدن از هر حالتِ در انتظار (چت با AI، یا
    منتظر ورودیِ یه اکشن پنل ادمین)، تا کاربر/ادمین هیچ‌وقت وسط یه فرآیند
    گیر نکنه و مجبور به شروع از صفر نشه."""
    chat_id = message.chat_id
    did_something = False
    if chat_id in chat_mode_users:
        chat_mode_users.discard(chat_id)
        did_something = True
    if ADMIN_CHAT_ID and message.sender_id == ADMIN_CHAT_ID and chat_id in admin_pending:
        admin_pending.pop(chat_id, None)
        did_something = True
    if did_something:
        await _rx(message.reply("✅ لغو شد. می‌تونی از اول شروع کنی."))
    else:
        await _rx(message.reply("چیزی برای لغو کردن در جریان نیست."))
...
@bot.on_message(commands=["music"])
async def music_cmd(bot: Robot, message: Message):
    """دستور /music - هم برای نمایش توی منوی دستورات (با آیکون 🎵)، هم اگه
    کاربر بعد از دستور مستقیم اسم آهنگ رو هم نوشته باشه (/music تتلو گدایی)
    بلافاصله جستجوش می‌کنه؛ وگرنه فقط راهنمای کوتاه می‌ده."""
    try:
        register_user(message.chat_id)
        if is_banned(message.chat_id):
            await _rx(message.reply("⛔️ شما مسدود شده‌اید."))
        ...
        parts = (message.text or "").split(maxsplit=1)
        if len(parts) > 1 and parts[1].strip():
            _spawn(handle_music_search(message, parts[1].strip()))
            return
        await _rx(message.reply(
            "🎵 جستجو و دانلود آهنگ\n\n"
            "کافیه اسم آهنگ یا خواننده رو بفرستی، خودم پیداش می‌کنم و برات می‌فرستم.\n"
            "مثال: تتلو گدایی"
        ))
    except Exception:
        logger.exception("خطا در /music")


@bot.on_message(commands=["whoami"])
async def whoami_cmd(bot: Robot, message: Message):
    """دستور عمومی (برای همه، نه فقط ادمین): sender_id بات‌پلتفرم و (اگه
    قابل resolve باشه) GUID واقعی حساب رو نشون می‌ده، و می‌گه از نظر جوین
    اجباری تاییدشده حساب می‌شه یا نه (چک زنده)."""
    channel_guid = get_setting("channel_guid")
    text = f"🆔 sender_id (بات‌پلتفرم): {message.sender_id}"
    real_guid = get_cached_real_guid(message.sender_id)
    if not real_guid:
        real_guid = await _resolve_real_sender_guid(message)
        if real_guid:
            save_real_guid(message.sender_id, real_guid)
    if real_guid:
        text += f"\n🪪 GUID واقعی حساب: {real_guid}"
    else:
        text += "\n🪪 GUID واقعی حساب: پیدا نشد (لاگ ریلوی رو برای جزئیات چک کن)"
    if channel_guid:
        found = await check_membership_live_with_message(message)
...
def start_menu_keypad():
    ...
    b = InlineBuilder()
    b = b.row(b.button_simple("start_download_info", "⬇️ دانلود روبینو/اینستا/تیک‌تاک/پینترست + موزیک"))
...
async def start_menu_download_info_callback(message):
    ...
    await _rx(message.reply(
        ...
        "🎬 یا خودِ ویدیو/ویس رو بفرست یا فوروارد کن، آهنگش رو برات پیدا می‌کنم\n\n"
...
async def start(bot: Robot, message: Message):
    ...
    try:
        ...
        await _rx(message.reply_inline(
            welcome or (
                "سلام 👋\n\n"
                "لینک روبینو، اینستاگرام، تیک‌تاک یا پینترست رو بفرست — دانلودش می‌کنم.\n"
                "اسم یه آهنگ هم بفرستی، پیداش می‌کنم.\n"
                "ویدیو یا ویس هم بفرستی، آهنگشو برات پیدا می‌کنم.\n\n"
                "راهنما: /help"
            ),
            start_menu_keypad(),
        ))
...
async def help_cmd(bot: Robot, message: Message):
    ...
    try:
        await _rx(message.reply(
            ...
            "• لینک پینترست بفرست\n"
            "• اسم آهنگ یا خواننده بنویس یا /music بزن\n"
            "• ویدیو یا ویس بفرست یا فوروارد کن تا آهنگش رو پیدا کنم\n"
            "• /chat — چت با هوش مصنوعی\n"
            "• /clearchat — پاک کردن تاریخچه‌ی چت و خروج از حالت چت\n"
            "• /cancel — لغو هر حالتِ در انتظار (مثلاً وسط چت گیر کردی)\n"
            "• /invite — دریافت لینک دعوت و دریافت پاداش VIP\n"
            "• /vip — وضعیت اشتراک VIP خودت"
        ))
    except Exception:
...
@bot.on_message(commands=["vip"])
async def vip_cmd(bot: Robot, message: Message):
    """وضعیت VIP خودِ کاربر - بدون نیاز به پنل ادمین."""
    logger.info(f"📩 /vip از {message.sender_id}")
    try:
        register_user(message.chat_id)
        u = get_user(message.chat_id) or {}
        if is_vip_active(u):
            until = (u.get("vip_until") or "")[:10]
            text = "💎 وضعیت اشتراک: VIP فعال\n"
            text += f"📅 تا تاریخ: {until}" if until else "📅 بدون تاریخ انقضا (نامحدود)"
        else:
            target = int(get_setting("referral_target", "2") or "2")
            reward_days = int(get_setting("referral_reward_days", "30") or "30")
            free_limit = get_setting("free_limit") or "10"
            text = (
                "💎 وضعیت اشتراک: عادی (VIP نیستی)\n\n"
                f"با دعوت {target} نفر از دوستات، {reward_days} روز VIP رایگان بگیر:\n"
                "/invite\n\n"
                f"کاربر عادی: تا {free_limit} دانلود رایگان در روز\n"
                "کاربر VIP: بدون محدودیت روزانه"
            )
        builder = InlineBuilder()
        builder = builder.row(builder.button_simple("start_invite", "🤝 دعوت دوستان"))
        await _rx(message.reply_inline(text, builder.build()))
    except Exception:
        logger.exception("خطا در هندلر /vip")
...
async def handle_text(bot: Robot, message: Message):
    ...
    try:
        ...
        # ─── تایید عضویت با فوروارد پست کانال (جایگزین چک زنده‌ی نامعتبر) ───
        channel_guid = get_setting("channel_guid")
        if channel_guid and not has_joined(message.sender_id):
            fwd_guid = _extract_forward_channel_guid(message)
            logger.info(
                f"🔎 دیباگ فوروارد از {message.sender_id}: guid_found={fwd_guid} "
                f"attrs={_debug_forward_dump(message)}"
            )
            if fwd_guid and fwd_guid == channel_guid:
                if mark_joined(message.sender_id):
                    _spawn(notify_referrer_on_join(message.sender_id))
                await _rx(message.reply("✅ عضویت تایید شد! حالا لینک یا اسم آهنگت رو بفرست."))
                return

        if not await check_membership_live_with_message(message):
            await send_not_joined(message)
            return
        if not check_cooldown(message.chat_id):
            return
...
async def on_callback(bot: Robot, message: Message):
    ...
    try:
        ...
        elif btn_id == "music_research":
            _spawn(handle_music_research_callback(message))
        elif btn_id == "back_to_results":
            _spawn(handle_back_to_results_callback(message))
        elif btn_id == "retry_download":
            _spawn(handle_retry_download_callback(message))
...
def _rubika_send_music_blocking(chat_id: str, file_path: str, caption: str = "") -> dict:
    # آپلود با retry/backoff و بدون keep-alive (توضیح کامل بالای تابع
    # _rubika_upload_file_with_retries) - همون علتی که آپلود ویدیو رو هم
    # گرفته بود، برای آهنگ هم صدق می‌کرد.
    file_id = _rubika_upload_file_with_retries("Music", file_path, "audio/mpeg")
...
async def _handle_inline_click(inline: dict):
    ...
    try:
        if btn_id.startswith("dl_"):
            ...
            await handle_music_research_callback(msg)
        elif btn_id == "back_to_results":
            await handle_back_to_results_callback(msg)
        elif btn_id == "retry_download":
            await handle_retry_download_callback(msg)
        elif btn_id == "check_join":
...
def _scheduled_reports_loop():
    """هر ساعت یه‌بار چک می‌کنه که وقتِ گزارش روزانه (هر روز ساعت ۹) یا
    گزارش هفتگی (هر دوشنبه ساعت ۹) رسیده یا نه، و اگه از تنظیمات فعال باشه
    برای ADMIN_CHAT_ID می‌فرسته. عمداً توی یه ترد کاملاً جدا و با
    _rubika_send_message_blocking (نه از طریق bot async) اجرا میشه - دقیقاً
    به همون دلیلی که وبهوک دکمه‌های شیشه‌ای هم جدا اجرا میشه: تا هیچ event
    loopِ جدیدی داخل ترد اصلی ربات ساخته نشه و به مشکل قبلیِ
    InvalidTokenError/Event loop is closed برنخوریم."""
    REPORT_HOUR = 9
        ...
        try:
            if ADMIN_CHAT_ID:
                ...
                if now.hour == REPORT_HOUR:
                    if get_setting("daily_report_enabled", "1") == "1" and last_daily_sent != today:
                        since = datetime.combine(today, datetime.min.time())
                        _rubika_send_message_blocking(ADMIN_CHAT_ID, build_report_text("روزانه 📅", since))
                        last_daily_sent = today
                    if (
                        get_setting("weekly_report_enabled", "1") == "1"
                        and today.weekday() == 0  # دوشنبه
                        and last_weekly_sent != today
                    ):
                        since = datetime.combine(today - timedelta(days=7), datetime.min.time())
                        _rubika_send_message_blocking(ADMIN_CHAT_ID, build_report_text("هفتگی 🗓", since))
                        last_weekly_sent = today
        except Exception:
            logger.exception("خطا در ارسال گزارش خودکار آماری")
        time.sleep(3600)


def start_scheduled_reports_thread():
    t = threading.Thread(target=_scheduled_reports_loop, daemon=True, name="scheduled-reports")
    t.start()
...
def _restore_ytdlp_cookies():
    """اگه YTDLP_COOKIES_B64 توی Railway ست شده باشه، فایل کوکیِ یوتیوب رو
    روی دیسک بازسازی و YTDLP_COOKIES_FILE رو بهش اشاره می‌ده.

    چرا این اضافه شد: تا همین‌جای کد (خط ۱۰۰ به بالا) قبلاً فرض بر این بود
    که جعل کلاینت اندروید/iOS (player_client) به‌تنهایی کافیه تا خطای
    «Sign in to confirm you're not a bot» دور زده بشه. این فرض دیگه درست
    نیست: یوتیوب اواخر ۲۰۲۵/اوایل ۲۰۲۶ یه محدودیت جدید (تجربه‌ی SABR-only +
    نیاز به GVS PO Token برای کلاینت‌های اندروید/iOS) رو گسترش داد که رو
    IPهای دیتاسنتر (مثل Railway) تقریباً برای *همه‌ی* ویدیوها همین خطا رو
    برمی‌گردونه، فارغ از اینکه کدوم player_client جعل بشه. تنها راه واقعاً
    قابل‌اعتماد الان کوکیِ یه اکانت یوتیوب واقعی و لاگین‌شده‌ست.

    راهنمای گرفتن کوکی:
      ۱) با اکستنشن مرورگر «Get cookies.txt LOCALLY» وقتی به یوتیوب لاگین
         هستی، فایل cookies.txt (فرمت Netscape) رو export کن.
      ۲) محتوای فایل رو base64 کن (مثلاً `base64 -w0 cookies.txt`).
      ۳) نتیجه رو توی Railway به‌عنوان متغیر محیطی YTDLP_COOKIES_B64 ذخیره کن.
    اکانتی که کوکیش استفاده میشه بهتره یه اکانت فرعی/تستی باشه (نه اکانت
    شخصی اصلی)، چون استفاده‌ی خودکار و مداوم از یه کوکی همیشه یه ریسک محدودشدن
    داره. کوکی هم بعد چند هفته/ماه ممکنه expire بشه و لازم باشه دوباره
    export و جایگزین بشه."""
    global YTDLP_COOKIES_FILE
    b64 = os.environ.get("YTDLP_COOKIES_B64", "")
    if not b64:
        return
    cookies_path = "ytdlp_cookies.txt"
    try:
        raw = base64.b64decode(b64.strip())
        # اعتبارسنجی: اگه رشته‌ی base64 موقع کپی/پیست (مخصوصاً رو موبایل) یه
        # جاش خراب/بریده شده باشه، دیکدش یه سری بایتِ بی‌معنیه که اصلاً متن
        # UTF-8 معتبر نیست و باعث میشه yt-dlp سرِ *هر* تلاش دانلود با یه خطای
        # کاملاً نامرتبط (UnicodeDecodeError) شکست بخوره - که گیج‌کننده‌ست و
        # از نبودِ کوکی هم بدتر عمل می‌کنه (چون قبلش fallbackهای دیگه اصلاً
        # فرصت اجرا شدن پیدا نمی‌کردن). برای همین قبل از قبول کردنِ فایل،
        # چک می‌کنیم واقعاً متنِ UTF-8 معتبره و شکلِ یه فایل کوکی نت‌اسکیپ
        # رو داره؛ اگه نه، بی‌خیالِ کوکی میشیم و می‌ذاریم بات مثل قبل (بدون
        # کوکی) کار کنه، به‌جای اینکه با یه فایل خراب همه‌چیز رو خراب کنه.
        text = raw.decode("utf-8")
        if not (text.lstrip().startswith("# Netscape HTTP Cookie File") or "\t" in text):
            raise ValueError("محتوای دیکدشده شکل یه فایل کوکی نتscape رو نداره")
        with open(cookies_path, "wb") as f:
            f.write(raw)
        YTDLP_COOKIES_FILE = cookies_path
        logger.info("✅ فایل کوکی یوتیوب از روی YTDLP_COOKIES_B64 بازسازی و اعتبارسنجی شد.")
    except Exception as e:
        logger.warning(
            f"⚠️ YTDLP_COOKIES_B64 خراب/ناقصه (احتمالاً موقع کپی/پیست بریده شده) - "
            f"از کوکی صرف‌نظر شد و بات بدون کوکی ادامه میده: {e}"
        )


def _check_invidious_plugin():
    """فقط یه چک تشخیصی: لاگ می‌کنه که پلاگین yt-dlp-invidious واقعاً نصب و
    شناسایی شده یا نه، تا اگه فقط تو کد فعالش کردیم ولی تو requirements.txt/
    Dockerfile پکیجش اضافه نشده باشه، به‌جای شکست خاموش، تو لاگ Railway
    واضح معلوم باشه."""
    try:
        from yt_dlp.extractor import list_extractors
        names = [ie.ie_key() for ie in list_extractors()]
        if any("Invidious" in n for n in names):
            logger.info("✅ پلاگین yt-dlp-invidious نصب و شناسایی شد (fallback بدون کوکی فعاله).")
        else:
            logger.warning(
                "⚠️ پلاگین yt-dlp-invidious نصب نیست - برای فعال‌شدنش باید "
                "'yt-dlp-invidious' به requirements.txt/Dockerfile اضافه و ری‌دیپلوی بشه."
            )
    except Exception as e:
        logger.warning(f"⚠️ چک پلاگین yt-dlp-invidious ناموفق بود: {e}")
...
if __name__ == "__main__":
    ...
    _restore_ytdlp_cookies()
    _check_invidious_plugin()
    # این ترد قبلاً برای تشخیص خودکار جوین از روی رویدادهای خام pyrubi بود،
    # ولی چون همیشه یه اتصال persistent به همون فایل session باز نگه می‌داره،
    # با چک زنده‌ی عضویت (که برای هر چک یه Client جدید با همون session باز
    # می‌کنه) قفل/تداخل ایجاد می‌کنه و باعث میشه چک زنده Exception بخوره و
    # به فلگ قدیمیِ has_joined (که هیچ‌وقت پاک نمیشه) fallback کنه - یعنی
    # دقیقاً همون باگ «بعد از لف‌دادن هنوز دسترسی داره». غیرفعالش می‌کنیم
    # چون الان چک زنده خودش این کار رو (بهتر و لحظه‌ای‌تر) انجام می‌ده.
    # start_pyrubi_event_logger_thread()
    start_inline_webhook_thread()
    start_scheduled_reports_thread()
    # نکته: قبلاً اینجا با یه asyncio.run() جدا bot.set_commands() صدا زده
    # می‌شد و بعدش با یه asyncio.run() دیگه (داخل bot.run()) ربات اجرا
    # می‌شد. دو تا event loop جدا از هم باعث می‌شد بعد از بسته‌شدن loop
    # اول، session/توکن ربات دیگه با loop دوم کار نکنه و ربات با ارور
    # InvalidTokenError / "Event loop is closed" کرش کنه. فعلاً این بخش
    # غیرفعال شده تا ربات پایدار بالا بیاد؛ بعداً می‌شه set_commands رو
    # داخل همون event loop اصلی bot.run() (یا یه هوک استارتاپ مناسب در
    # rubka) صدا زد.
    bot.run()
