...
# فاصله‌ی حداقلی بین درخواست‌های پشت‌سرهم به Gemini (ثانیه). چون چند کاربر
# ممکنه هم‌زمان لینک بفرستن و هر کدوم چند فریم چک بشه، بدون این فاصله سریع
# rate limit (429) می‌خوریم. این یه throttle سراسریه، نه per-request.
_GEMINI_MIN_INTERVAL = 1.2
_gemini_throttle_lock = threading.Lock()
_gemini_last_call_at = 0.0


def _gemini_throttle_blocking() -> None:
...
def _check_images_nsfw_blocking(image_paths: list) -> bool:
    ...
    try:
        ...
        # اگه Gemini با 429 (rate limit) جواب بده، به‌جای تسلیم فوری، چندبار
        # با فاصله دوباره امتحان می‌کنیم چون معمولاً موقتیه. قبل از هر تلاش
        # هم throttle سراسری رو رعایت می‌کنیم تا خودمون باعث burst نشیم.
        r = None
        for attempt in range(3):
            _gemini_throttle_blocking()
            r = HTTP_SESSION.post(
            ...
            if r.status_code != 429:
                break
            wait_s = 2 * (attempt + 1)
            logger.warning(f"Gemini NSFW check: 429 rate limit، {wait_s} ثانیه صبر و retry ({attempt + 1}/3)")
            time.sleep(wait_s)

        if r.status_code == 429:
            # بعد از ۳ تلاش هم rate limit بود - نتونستیم واقعاً چک کنیم. این
            # حالت معمولاً یعنی سهمیه‌ی Gemini موقتاً پر شده، نه اینکه محتوا
            # واقعاً مشکل داره؛ fail-closed کردنش باعث می‌شد تقریباً همه‌ی
            # پست‌ها رد بشن. برای همین fail-open عمل می‌کنیم.
            logger.warning("Gemini NSFW check: بعد از retry هم 429 - fail-open, اجازه‌ی ارسال داده میشه")
            return False
