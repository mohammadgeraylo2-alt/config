...
# ─── فیلتر محتوای نامناسب (NSFW) برای پست‌های اینستاگرام/پینترست ────────────
# چون روبیکا تابع قوانینه، قبل از فرستادن هر عکس/ویدیوی دانلودشده از اینستا
# یا پینترست، یه فریم ازش (یا خودِ عکس) رو به مدل بینایی Gemini می‌دیم و
# می‌پرسیم محتوای پورنوگرافی/خیلی جنسی داره یا نه. اگه GEMINI_KEY تنظیم نشده
# باشه یا سرویس Gemini موقتاً خطا بده، به‌صورت fail-open عمل می‌کنیم (یعنی
# اجازه‌ی ارسال داده میشه) تا یه مشکل موقتی روی Gemini کل ربات رو از کار
# نندازه؛ این یعنی این فیلتر یه لایه‌ی کمکیه، نه تضمین ۱۰۰٪.
NSFW_CHECK_PROMPT = (
    "به تصویر(های) زیر با دقت نگاه کن. اگه توی هر کدوم از تصویرها، هر کدوم "
    "از این موارد رو دیدی، فقط بنویس: NSFW\n"
    "- نوک سینه یا هاله‌ی سینه (نیپل/آرئولا) به هر دلیلی نمایانه - حتی اگه "
    "زمینه‌ش شیردهی، هنری، یا غیرجنسی باشه\n"
    "- برهنگی کامل یا نسبی (کل بدن یا اندام تناسلی/باسن بدون پوشش کامل)\n"
    "- پورنوگرافی یا محتوای جنسی صریح\n"
    "- ژست بدنی که آشکارا برای نمایش جنسی بدن طراحی شده - مثل خم‌شدن با "
    "برجسته‌کردن باسن رو به دوربین، یا بالازدن لباس برای نشون‌دادن اندام\n"
    "- دکلته‌ی خیلی زیاد که بخش قابل‌توجهی از سینه (نه فقط استخوان "
    "ترقوه) نمایان باشه\n"
    "اگه هیچ‌کدوم از تصویرها این موارد رو نداشتن (لباس اسپرت/شنا/تاپ/یقه‌باز "
    "معمولی با ژست عادی و روزمره)، فقط بنویس: SAFE\n"
    "فقط یکی از این دو کلمه رو بنویس، بدون هیچ توضیح اضافه."
)

# فاصله‌ی حداقلی بین درخواست‌های پشت‌سرهم به Gemini (ثانیه). سهمیه‌ی رایگان
# Gemini معمولاً حدود ۱۵ درخواست در دقیقه‌ست؛ قبلاً این فاصله ۱.۲ ثانیه بود
# که کافی نبود و مدام 429 می‌خوردیم. الان هم فاصله رو زیاد کردیم هم این قفل
# کل مدت درخواست (نه فقط قبلش) رو نگه می‌داره تا درخواست‌های هم‌زمانِ چند
# کاربر روی هم نیفتن و باعث burst نشن.
...
class _GeminiThrottle:
    """context manager: کل تماس با Gemini (نه فقط فاصله‌ی قبلش) رو سریالایز
    می‌کنه تا هیچ‌وقت دو درخواست هم‌زمان به Gemini نره."""
    def __enter__(self):
        global _gemini_last_call_at
        _gemini_call_lock.acquire()
        now = time.monotonic()
        wait = _gemini_last_call_at + _GEMINI_MIN_INTERVAL - now
        if wait > 0:
            time.sleep(wait)
        return self
...
def _check_images_nsfw_blocking(image_paths: list) -> bool:
    """یک یا چند عکس (مثلاً چند فریم از یک ویدیو) رو توی یه درخواست واحد به
    gemini-flash-latest می‌ده و True برمی‌گردونه اگه توی هر کدوم از تصویرها
    محتوای نامناسب تشخیص داد. قبلاً هر فریم یه درخواست جدا بود که هم کند بود
    هم زود rate limit می‌خورد؛ الان همه‌ی فریم‌ها با هم توی یه درخواست چک
    می‌شن."""
    if not GEMINI_KEY or not image_paths:
        return False
    try:
        parts = [{"text": NSFW_CHECK_PROMPT}]
        ...
        # اگه Gemini با 429 (rate limit) جواب بده، به‌جای تسلیم فوری بعد از
        # چندتا تلاش کوتاه، این‌بار جدی‌تر و با فاصله‌ی بیشتر retry می‌کنیم
        # (چون قبلاً retry کوتاه بود و مدام به fail-open می‌رسیدیم که باعث
        # میشد محتوای نامناسب هم رد بشه). با throttle سراسری هم دیگه خودمون
        # باعث burst نمیشیم.
        backoffs = [3, 5, 8, 12, 17]
        r = None
        for attempt, wait_s in enumerate([0] + backoffs):
            if wait_s:
                time.sleep(wait_s)
            with _GeminiThrottle():
                r = HTTP_SESSION.post(
            ...
            if r.status_code != 429:
                break
            logger.warning(f"Gemini NSFW check: 429 rate limit، retry ({attempt + 1}/{len(backoffs) + 1})")

        if r.status_code == 429:
            # بعد از حدود ۴۵ ثانیه retry هم بازم 429 بود - این دیگه یه
            # مشکل واقعی سهمیه‌ست، نه یه لحظه‌ی موقتیِ کوتاه. چون نمی‌تونیم
            # مطمئن بشیم محتوا مناسبه یا نه، و ریسک نقض قوانین روبیکا مهم‌تر
            # از تاخیر یا رد یه پسته، fail-closed عمل می‌کنیم.
            logger.warning("Gemini NSFW check: بعد از retry هم 429 - fail-closed, NSFW در نظر گرفته میشه")
            return True

        r.raise_for_status()
        data = r.json()

        # اگه خودِ Gemini به‌خاطر سیاست‌های داخلیش (safety) از جواب‌دادن به این
        # عکس امتناع کرده باشه (promptFeedback.blockReason یا finishReason
        # برابر SAFETY/BLOCKLIST/... باشه)، این خودش قوی‌ترین نشونه‌ست که عکس
        # حساسه - پس به‌جای fail-open (که قبلاً باعث رد شدن عکس‌های واقعاً
        # نامناسب می‌شد چون جواب خالی = "NSFW" پیدا نمی‌شد)، اینجا fail-closed
        # عمل می‌کنیم و مستقیم NSFW برمی‌گردونیم.
        block_reason = (data.get("promptFeedback") or {}).get("blockReason")
        if block_reason:
            logger.warning(f"Gemini NSFW check: promptFeedback بلاک شد ({block_reason}) - fail-closed, NSFW در نظر گرفته میشه")
            return True

        candidates = data.get("candidates", [])
        if not candidates:
            logger.warning("Gemini NSFW check: هیچ candidate ای برنگشت - fail-closed, NSFW در نظر گرفته میشه")
            return True

        finish_reason = candidates[0].get("finishReason", "")
        parts_out = candidates[0].get("content", {}).get("parts", [])
        answer = "".join(p.get("text", "") for p in parts_out).strip().upper()

        if not answer and finish_reason and finish_reason != "STOP":
            logger.warning(f"Gemini NSFW check: پاسخ خالی با finishReason={finish_reason} - fail-closed, NSFW در نظر گرفته میشه")
            return True
        return "NSFW" in answer
    except Exception as e:
        logger.warning(f"چک NSFW خطا داد (fail-open، اجازه‌ی ارسال داده میشه): {e}")
        return False


def check_media_nsfw_blocking(file_path: str) -> bool:
    """ورودی می‌تونه عکس یا ویدیو باشه. اگه فیلتر NSFW از پنل ادمین خاموش
    باشه یا GEMINI_KEY تنظیم نشده باشه، همیشه False (یعنی مجاز) برمی‌گردونه.
    برای ویدیو، به‌جای یه فریم ثابت (که ممکنه لحظه‌ی نامناسب رو جا بندازه)،
    چند فریم از نقاط مختلف طول ویدیو استخراج می‌کنه و همه رو با هم توی یه
    درخواست واحد به Gemini می‌ده (به‌جای چند درخواست جدا) تا هم سریع‌تر باشه
    هم کمتر rate limit بخوره."""
    if get_setting("nsfw_filter_enabled", "1") != "1" or not GEMINI_KEY:
        return False
...
def admin_main_text():
    ...
    nsfw_note = "" if GEMINI_KEY else " (⚠️ GEMINI_KEY تنظیم نشده، عملاً غیرفعاله)"
    return (
        f"🔧 پنل ادمین\n\nحالت تعمیر: {maintenance}\nمحدودیت دانلود: {limit_st}\n"
        f"فیلتر NSFW: {nsfw_st}{nsfw_note}"
    )
