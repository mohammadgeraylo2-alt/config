...
async def check_membership_live(user_guid) -> bool:
    """چک زنده‌ی عضویت با سلف‌بات pyrubi. اگه چک زنده موفق اجرا بشه، نتیجه‌ش
    قطعیه (چه True چه False) - یعنی اگه کاربر لف داده باشه، همینجا دسترسی
    واقعاً پس گرفته می‌شه. فقط وقتی خودِ چک زنده خطا بده (سلف‌بات لاگین
    نیست/قطعه/API خطا داد)، به‌عنوان fallback از فلگ قدیمی has_joined
    استفاده می‌کنیم تا ربات کلاً قفل نشه."""
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
    ...
    if get_setting("realcheck_enabled", "0") == "1" and os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi"):
        try:
            live_result = await asyncio.to_thread(_pyrubi_is_member_blocking, channel_guid, user_guid)
        except Exception:
            logger.exception("❌ چک زنده‌ی عضویت با pyrubi خطا داد - fallback به فلگ قدیمی")
            return has_joined(user_guid)

        if live_result:
            if mark_joined(user_guid):
                _spawn(notify_referrer_on_join(user_guid))
            return True
        return False  # چک زنده قطعاً میگه عضو نیست -> دسترسی رد میشه، حتی اگه قبلاً بوده

    return has_joined(user_guid)


async def is_member(user_guid, force_refresh: bool = False) -> bool:
    """نقطه‌ی ورودی اصلی برای چک عضویت. force_refresh=True یعنی کش لیست
    اعضا رو نادیده بگیر و دوباره از pyrubi بگیر (برای دکمه‌ی «بررسی مجدد»)."""
    if force_refresh:
        with _members_cache_lock:
            _members_cache["guids"] = None
...
async def send_not_joined(message):
    ...
    text = "🔒 دسترسی قفله\n\n"
    if link:
        text += f"اول عضو کانال شو:\n🔗 {link}\n\n"
    else:
        text += "اول عضو کانال شو.\n\n"
    text += "بعدش دکمه‌ی «✅ بررسی عضویت» رو بزن تا خودم چک کنم واقعاً عضو شدی یا نه."
    await _rx(message.reply_inline(text, not_joined_keypad()))


async def handle_check_join_callback(message):
    """دکمه‌ی «بررسی عضویت»: اگه realcheck_enabled روشن باشه، همینجا یه چک
    زنده (force_refresh) با سلف‌بات pyrubi می‌زنه تا مطمئن بشیم کاربر واقعاً
    الان عضو کانال هست، نه فقط قبلاً یه‌بار فوروارد کرده."""
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
