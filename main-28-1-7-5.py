...
NSFW_CHECK_PROMPT = (
    "به این تصویر با دقت نگاه کن. اگه هر کدوم از این موارد رو داشت، فقط "
    "بنویس: NSFW\n"
    ...
    "ترقوه) نمایان باشه\n"
    "در غیر این صورت (لباس اسپرت/شنا/تاپ/یقه‌باز معمولی با ژست عادی و "
    "روزمره)، فقط بنویس: SAFE\n"
...
def _check_image_nsfw_blocking(image_path: str) -> bool:
    """یه عکس رو به مدل بینایی gemini-flash-latest می‌ده و True برمی‌گردونه
    اگه تشخیص داد محتوای نامناسب (پورنوگرافی/خیلی جنسی) داره."""
    if not GEMINI_KEY:
        return False
    try:
        ...
        # اگه Gemini با 429 (rate limit) جواب بده - که با چک چندفریمیِ ویدیوها
        # زیاد پیش میاد - به‌جای تسلیم فوری، چندبار با فاصله دوباره امتحان
        # می‌کنیم چون معمولاً موقتیه.
        r = None
        ...
        if r.status_code == 429:
            # بعد از ۳ تلاش هم rate limit بود - نتونستیم واقعاً چک کنیم. قبلاً
            # اینجا fail-open بود که باعث رد شدن محتوای نامناسب می‌شد؛ چون
            # نمی‌تونیم مطمئن بشیم مناسبه یا نه، fail-closed عمل می‌کنیم.
...
def check_media_nsfw_blocking(file_path: str) -> bool:
    ...
    چند فریم از نقاط مختلف طول ویدیو استخراج می‌کنه و اگه هر کدوم NSFW
    تشخیص داده بشن، کل ویدیو رد میشه."""
    ...
    if ext in ("mp4", "mov", "webm", "mkv"):
        ...
        for i, ss in enumerate(offsets):
            ...
            try:
                if not _extract_video_frame_blocking(file_path, frame_path, at_seconds=ss):
                    continue
                checked_any = True
                if _check_image_nsfw_blocking(frame_path):
                    return True
            finally:
                if os.path.exists(frame_path):
                    try:
                        ...
                        pass
        return False  # هیچ فریمی NSFW نبود (یا هیچ فریمی قابل‌استخراج نبود - fail-open)
    else:
        return _check_image_nsfw_blocking(file_path)
    ...
    try:
        ...
        if downloaded:
            ...
            if is_nsfw:
                ...
                await _rx(status.edit("🚫 این پست به‌خاطر محتوای نامناسب (پورنوگرافی/خیلی جنسی) ارسال نشد."))
                return
