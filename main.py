...
async def find_guid_cmd(bot: Robot, message: Message):
    ...
    if not (os.path.exists(PYRUBI_SESSION_NAME + ".db") or glob.glob(PYRUBI_SESSION_NAME + "*")):
...
async def pyrubi_filecheck_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: چک می‌کنه فایل session پیرابی سالمه یا
    موقع restore از PYRUBI_SESSION_B64 خراب شده."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    out = []
    session_path = PYRUBI_SESSION_NAME + ".db"
    ...
    try:
        ...
        result = cur.fetchone()
        out.append(f"🔍 sqlite integrity_check: {result}")
        cur2 = con.execute("SELECT name FROM sqlite_master WHERE type='table'")
        tables = [r[0] for r in cur2.fetchall()]
        out.append(f"📋 جدول‌های داخل فایل: {tables}")
        con.close()
    except Exception as e:
        out.append(f"❌ فایل sqlite معتبر نیست: {type(e).__name__}: {e}")
    await _rx(message.reply("\n".join(out)))
