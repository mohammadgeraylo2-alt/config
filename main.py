...
async def _watch_rubino_login(chat_id, result_queue):
    ...
    if status == "ok":
        try:
            ...
            await _rx(bot.send_message(chat_id, f"✅ لاگین موفق شد (فایل: {session_path}). مقدار زیره طولانیه و توی چند پیام میاد؛ همه‌ی تیکه‌ها رو پشت سر هم (بدون فاصله) بچسبون و به‌عنوان RUBINO_SESSION_B64 توی Railway ذخیره کن، بعد همه‌ی این پیام‌ها رو از چت پاک کن:"))
            # پیام‌های روبیکا حداکثر ۵۰۰۰ کاراکتر و ۱۲۸ خط رو قبول می‌کنن؛ برای
            # اطمینان تیکه‌های ۳۵۰۰ تایی می‌فرستیم (بدون خط جدید داخلشون، پس با
            # همین سایز مشکلی با محدودیت خط هم پیش نمیاد).
            chunk_size = 3500
            chunks = [b64[i:i + chunk_size] for i in range(0, len(b64), chunk_size)]
            for idx, chunk in enumerate(chunks, start=1):
                await _rx(bot.send_message(chat_id, f"[{idx}/{len(chunks)}]\n{chunk}"))
        except Exception as e:
...
def _download_rubino_blocking(chat_id, text: str):
    ...
    try:
        ...
        if not media_url:
            logger.warning(f"rubino: هیچ الگویی توی HTML پیدا نشد. نمونه: {html[:2000]}")
    except Exception as e:
