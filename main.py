...
# ─── فیلتر محتوای نامناسب (NSFW) با NudeNet (لوکال، رایگان، بدون rate limit) ──
# قبلاً این چک با فرستادن عکس به Gemini انجام می‌شد که هم به کلید API نیاز
# داشت هم دائم به rate limit/سهمیه‌ی روزانه می‌خورد و باعث می‌شد یا محتوای
# نامناسب رد بشه (fail-open) یا کل ارسال فلج بشه (fail-closed روی هر پست).
# الان به‌جاش از NudeNet استفاده می‌کنیم: یه مدل تشخیص تصویر که کاملاً روی
# خود سرور (بدون اینترنت/API خارجی) اجرا میشه، رایگانه، و هیچ محدودیت تعداد
# درخواستی نداره. اولین بار که استفاده بشه، مدل (چند مگابایتی) دانلود میشه؛
# بعدش توی حافظه می‌مونه و برای همه‌ی چک‌های بعدی استفاده میشه.
#
# نکته: باید پکیج nudenet به requirements.txt اضافه بشه:
#   nudenet
#
# NudeNet نمی‌تونه مثل Gemini «ژست نامناسب» یا «دکلته‌ی زیاد» رو تشخیص بده؛
# فقط اندام‌های صریح نمایان (سینه/اندام تناسلی/باسن بدون پوشش) رو تشخیص
# می‌ده. برای اکثر موارد واقعی (پورن، برهنگی) همین کافیه.
_NUDENET_DETECTOR = None
_nudenet_init_lock = threading.Lock()

# لیبل‌هایی که NudeNet برمی‌گردونه و به معنای نمایان‌بودن صریح اندام هستن.
# اگه امتیاز تشخیص برای هرکدوم از این‌ها از آستانه بیشتر باشه، NSFW در نظر
# گرفته میشه.
_NUDENET_BLOCK_LABELS = {
...
# لیبل‌های «پوشیده ولی حساس» (بیکینی/لباس شنا/زیرپوش و امثالش). این‌ها
# برهنگی صریح نیستن، ولی خیلی از ادمین‌ها می‌خوان محتوای این‌شکلی (مثل
# ویدیوهای شنای با بیکینی، حالت‌های تحریک‌آمیز و ...) هم فیلتر بشه. آستانه‌ش
# رو جدا و کمی بالاتر گذاشتیم چون این کلاس‌ها معمولاً false-positive بیشتری
# دارن (مثلاً لباس ورزشی/ساحلی معمولی).
_NUDENET_SUGGESTIVE_LABELS = {
    "FEMALE_BREAST_COVERED",
    "BUTTOCKS_COVERED",
}
_NUDENET_SUGGESTIVE_THRESHOLD = 0.4


def _get_nudenet_detector():
    """detector رو فقط یه‌بار می‌سازه (lazy singleton) چون ساختنش کمی طول
    می‌کشه (بارگذاری مدل). thread-safe هست تا دو ریکوئست هم‌زمان دوبار
    نسازنش."""
    global _NUDENET_DETECTOR
    if _NUDENET_DETECTOR is None:
        with _nudenet_init_lock:
            if _NUDENET_DETECTOR is None:
...
def _check_images_nsfw_blocking(image_paths: list) -> bool:
    """یک یا چند عکس (مثلاً چند فریم از یک ویدیو) رو با NudeNet چک می‌کنه و
    True برمی‌گردونه اگه توی هر کدوم از تصویرها اندام صریح نمایان تشخیص داده
    بشه. چون کاملاً لوکاله، نه rate limit داره نه هزینه‌ای."""
    if not image_paths:
        return False
    try:
        detector = _get_nudenet_detector()
    except Exception as e:
        logger.warning(f"NudeNet لود نشد (fail-open، اجازه‌ی ارسال داده میشه): {e}")
        return False
    ...
    for image_path in image_paths:
        try:
            ...
            logger.warning(f"NudeNet روی {image_path} خطا داد، این فریم رد میشه: {e}")
            continue
        # لاگ دیباگ: همه‌ی لیبل‌های تشخیص‌داده‌شده رو با امتیازشون چاپ می‌کنه،
        # حتی اگه هیچ‌کدوم بلاک نشن. برای تنظیم آستانه‌ها لازمه بدونیم روی
        # محتوای واقعی چه امتیازی داده میشه؛ اگه چیزی تشخیص داده نشه هم یه
        # خط می‌زنه که معلوم بشه دیتکتور اصلاً چیزی روی این فریم پیدا نکرده.
        if detections:
            summary = ", ".join(
                f"{d.get('class') or d.get('label')}:{d.get('score', 0):.2f}" for d in detections
            )
            logger.info(f"NudeNet debug [{os.path.basename(image_path)}]: {summary}")
        else:
            logger.info(f"NudeNet debug [{os.path.basename(image_path)}]: هیچ لیبلی تشخیص داده نشد")
        for det in detections:
            label = det.get("class") or det.get("label")
            score = det.get("score", 0)
            if label in _NUDENET_BLOCK_LABELS and score >= _NUDENET_THRESHOLD:
                logger.warning(f"NudeNet: {label} با امتیاز {score:.2f} تشخیص داده شد - NSFW (صریح)")
                return True
            if (
                get_setting("nsfw_filter_suggestive_enabled", "1") == "1"
                and label in _NUDENET_SUGGESTIVE_LABELS
                and score >= _NUDENET_SUGGESTIVE_THRESHOLD
            ):
                logger.warning(f"NudeNet: {label} با امتیاز {score:.2f} تشخیص داده شد - NSFW (پوشیده/حساس)")
                return True
    return False
...
def check_media_nsfw_blocking(file_path: str) -> bool:
    ...
    ویدیو استخراج می‌کنه و همه رو با NudeNet چک می‌کنه."""
    ...
    if ext in ("mp4", "mov", "webm", "mkv"):
        ...
        if duration <= 0:
            ...
            logger.warning(f"NudeNet: مدت‌زمان ویدیو {file_path} تشخیص داده نشد (ffprobe خطا داد)، از افست‌های ثابت استفاده میشه")
            offsets = [0.5, 3.0, 7.0]
        elif duration <= 4:
            offsets = [max(duration * 0.3, 0.3)]
        else:
            offsets = [1.0, duration * 0.5, max(duration - 1.0, 1.0)]
        logger.info(f"NudeNet debug: duration={duration:.2f}s offsets={[round(o, 2) for o in offsets]}")
        ...
        try:
            for i, ss in enumerate(offsets):
                ...
                if _extract_video_frame_blocking(file_path, frame_path, at_seconds=ss):
                    ...
                    logger.warning(f"NudeNet: استخراج فریم در ثانیه {ss:.2f} از {file_path} شکست خورد")
            if not frame_paths:
                logger.warning(f"NudeNet: هیچ فریمی از {file_path} قابل‌استخراج نبود - fail-open")
