...
GEMINI_KEY = os.environ.get("GEMINI_KEY", "")          # برای ساخت/ادیت عکس با AI (رایگان)
GROQ_KEY = os.environ.get("GROQ_KEY", "")               # برای چت متنی سریع با AI (رایگان)
...
def search_songs(query):
    ...
    try:
        r = HTTP_SESSION.get("https://api.deezer.com/search", params={"q": query, "limit": 10}, timeout=8)
...
# ─── ساخت و ادیت عکس با هوش مصنوعی ───────────────────────────────────────────
# فقط از Gemini استفاده می‌کنیم (نه ModelsLab/HuggingFace/Pollinations که توی
# ربات تلگرام استفاده شده بودن) چون: ۱) رایگانه - کلید پولی لازم نداره،
# ۲) خودش مستقیم فارسی رو می‌فهمه (نیازی به سرویس ترجمه‌ی جدا نیست)،
# ۳) کیفیت مدل gemini-2.5-flash-image (معروف به Nano Banana) بالاست.
user_image_state = {}  # chat_id -> {"mode": "await_prompt"/"await_edit_photo"/"await_edit_text", ...}
...
def _guess_image_mime(b: bytes) -> str:
    ...
    if b[:4] == b"RIFF" and b[8:12] == b"WEBP":
        return "image/webp"
    return "image/jpeg"  # حدس منطقی، بیشتر عکس‌های ارسالی توی چت jpeg هستن


def _enhance_image_prompt_blocking(text: str) -> str:
    """توضیح کاربر (حتی فارسی) رو با Gemini غنی‌سازی می‌کنه: خودش می‌فهمه
    فارسیه، به یه پرامپت انگلیسیِ دقیق و فتورئالیستیک تبدیلش می‌کنه که
    کیفیت خروجی عکس خیلی بهتر بشه. اگه به هر دلیلی این مرحله خطا بده، همون
    متن خام کاربر مستقیم به مدل عکس‌ساز داده میشه (بازم کار می‌کنه، فقط
    کیفیتش ممکنه کمتر باشه) - یعنی کل قابلیت گیر نمی‌کنه."""
    if not GEMINI_KEY:
        return text
    try:
        msg = (
            "You are a world-class AI image prompt engineer. "
            "The user's idea may be written in Persian or English - understand it fully "
            "either way (translate mentally if it's Persian). "
            "Expand it into a rich, detailed, photorealistic English prompt with strong "
            "camera/lighting/composition detail, UNLESS the user clearly asked for a "
            "non-photorealistic style (cartoon, anime, painting, etc.) - in that case "
            "respect that requested style instead of forcing photorealism. "
            "Return ONLY the final English prompt, nothing else, max 200 words.\n\n"
            f"User idea: {text}"
        )
        payload = {
        ...
        logger.warning(f"enhance_image_prompt خطا داد، از متن خام استفاده میشه: {e}")
    return text
...
def _gemini_image_call_blocking(parts: list) -> bytes:
    """یه فراخوانی مشترک به gemini-2.5-flash-image - هم برای ساخت عکس هم
    برای ادیت استفاده میشه؛ فرقشون فقط توی parts ورودیه (ادیت یه inline_data
    عکس هم به پارت‌ها اضافه می‌کنه)."""
    payload = {
    ...
    if not candidates:
        ...
        raise RuntimeError(f"جوابی از Gemini برنگشت{f' (دلیل: {reason})' if reason else ''}")
    for part in candidates[0].get("content", {}).get("parts", []):
        inline = part.get("inlineData") or part.get("inline_data")
        if inline and str(inline.get("mimeType") or inline.get("mime_type") or "").startswith("image"):
            return base64.b64decode(inline["data"])
    raise RuntimeError("عکسی توی جواب Gemini پیدا نشد")


async def generate_image_ai(prompt: str) -> bytes:
    if not GEMINI_KEY:
        raise RuntimeError("کلید GEMINI_KEY تنظیم نشده")
    enhanced = await asyncio.to_thread(_enhance_image_prompt_blocking, prompt)
    return await asyncio.to_thread(_gemini_image_call_blocking, [{"text": enhanced}])


async def edit_image_ai(img_bytes: bytes, instruction: str) -> bytes:
    if not GEMINI_KEY:
        raise RuntimeError("کلید GEMINI_KEY تنظیم نشده")
    enhanced = await asyncio.to_thread(_enhance_image_prompt_blocking, instruction)
    ...
    return await asyncio.to_thread(_gemini_image_call_blocking, parts)


async def handle_image_generate(message: "Message", prompt: str):
    try:
        await _rx(message.reply("🎨 دارم عکس رو می‌سازم... (چند ثانیه طول می‌کشه)"))
        img_bytes = await generate_image_ai(prompt)
        tmp_path = f"/tmp/gen_{message.chat_id}_{int(time.time())}.png"
        with open(tmp_path, "wb") as f:
            f.write(img_bytes)
        try:
            await _rx(message.reply_image(path=tmp_path, text="🎨 آماده شد!"))
        finally:
            if os.path.exists(tmp_path):
                os.remove(tmp_path)
    except Exception as e:
        logger.warning(f"generate_image_ai خطا داد: {e}")
        try:
            await _rx(message.reply(f"❌ ساخت عکس ناموفق بود: {e}"))
        except Exception:
            pass


async def handle_image_edit(message: "Message", img_bytes: bytes, instruction: str):
    try:
        await _rx(message.reply("🖌 دارم عکس رو ادیت می‌کنم..."))
        result = await edit_image_ai(img_bytes, instruction)
        tmp_path = f"/tmp/edit_{message.chat_id}_{int(time.time())}.png"
        with open(tmp_path, "wb") as f:
            f.write(result)
        try:
            await _rx(message.reply_image(path=tmp_path, text="🖌 آماده شد!"))
        ...
        logger.warning(f"edit_image_ai خطا داد: {e}")
        try:
            await _rx(message.reply(f"❌ ادیت عکس ناموفق بود: {e}"))
        except Exception:
            pass


async def handle_image_edit_upload(message: "Message", file_meta: dict, caption_text: str):
    """کاربر یه عکس فرستاده در حالی که منتظر عکس برای ادیت بودیم - دانلودش
    می‌کنیم؛ اگه کپشن (توضیح ادیت) هم داشت مستقیم ادیت می‌کنیم، وگرنه عکس رو
    نگه می‌داریم و منتظر پیام بعدی (توضیح ادیت) می‌مونیم."""
    tmp_path = f"/tmp/editsrc_{message.chat_id}_{int(time.time())}.bin"
    try:
        ok = await asyncio.to_thread(_download_rubika_file_blocking, file_meta["file_id"], tmp_path)
        if not ok:
            await _rx(message.reply("❌ نتونستم عکس رو دانلود کنم، دوباره امتحان کن."))
            if os.path.exists(tmp_path):
                os.remove(tmp_path)
            return
        instruction = (caption_text or "").strip()
        if instruction:
            with open(tmp_path, "rb") as f:
                img_bytes = f.read()
            await handle_image_edit(message, img_bytes, instruction)
            if os.path.exists(tmp_path):
                os.remove(tmp_path)
        else:
            user_image_state[message.chat_id] = {"mode": "await_edit_text", "image_path": tmp_path}
            await _rx(message.reply("✍️ حالا بگو چه تغییری روی این عکس بدم؟"))
    except Exception as e:
        logger.warning(f"handle_image_edit_upload خطا داد: {e}")
        try:
            await _rx(message.reply(f"❌ خطا: {e}"))
        except Exception:
            pass
...
async def handle_image_edit_from_path(message: "Message", img_path: str, instruction: str):
    try:
        if not img_path or not os.path.exists(img_path):
            await _rx(message.reply("❌ عکس قبلی پیدا نشد، لطفاً دوباره از /edit شروع کن."))
            return
...
async def image_cmd(bot: Robot, message: Message):
    try:
        ...
        if not await is_member(message.sender_id):
            ...
            return
        if not GEMINI_KEY:
            await _rx(message.reply("❌ قابلیت ساخت عکس فعلاً غیرفعاله (کلید GEMINI_KEY تنظیم نشده)."))
            return
        parts = (message.text or "").split(maxsplit=1)
        prompt = parts[1].strip() if len(parts) > 1 else ""
        chat_mode_users.discard(message.chat_id)
        if prompt:
            user_image_state.pop(message.chat_id, None)
            _spawn(handle_image_generate(message, prompt))
        else:
            user_image_state[message.chat_id] = {"mode": "await_prompt"}
            await _rx(message.reply(
                "🎨 چه عکسی بسازم؟ توضیح بده (فارسی هم می‌تونی بنویسی)\n\n"
                "مثال: «یه گربه فضانورد روی ماه، غروب آفتاب»"
            ))
    except Exception:
        logger.exception("خطا در /image")


@bot.on_message(commands=["edit"])
async def edit_cmd(bot: Robot, message: Message):
    try:
        register_user(message.chat_id)
        if is_banned(message.chat_id):
            await _rx(message.reply("⛔️ شما مسدود شده‌اید."))
        ...
        if not await is_member(message.sender_id):
            ...
            return
        if not GEMINI_KEY:
            await _rx(message.reply("❌ قابلیت ادیت عکس فعلاً غیرفعاله (کلید GEMINI_KEY تنظیم نشده)."))
            return
        chat_mode_users.discard(message.chat_id)
        user_image_state[message.chat_id] = {"mode": "await_edit_photo"}
        await _rx(message.reply(
            "🖌 عکسی که می‌خوای ادیت بشه رو بفرست.\n"
            "اگه توضیح ادیت رو هم همون‌جا (کپشن عکس) بنویسی، سریع‌تر انجام میشه."
        ))
    except Exception:
        logger.exception("خطا در /edit")
...
def start_menu_keypad():
    """کیبورد شیشه‌ای زیر پیام /start - فقط قابلیت‌های هوش مصنوعی، بدون
    کانفیگ رایگان و بدون دکمه‌ی «چقدر منو می‌شناسن»."""
    b = InlineBuilder()
    b = b.row(b.button_simple("start_image", "✨ ساخت عکس با هوش مصنوعی"))
    b = b.row(b.button_simple("start_edit", "🖼 ادیت عکس با هوش مصنوعی"))
    b = b.row(b.button_simple("start_chat", "🤖 چت با هوش مصنوعی"))
    return b.build()


async def start_menu_image_callback(message):
    """کلیک روی دکمه‌ی «ساخت عکس با هوش مصنوعی» توی منوی /start - همون
    مسیر حالت بدون‌پرامپت /image رو طی می‌کنه."""
    register_user(message.chat_id)
    if is_banned(message.chat_id):
        await _rx(message.reply("⛔️ شما مسدود شده‌اید."))
        return
...
async def start_menu_edit_callback(message):
    """کلیک روی دکمه‌ی «ادیت عکس با هوش مصنوعی» توی منوی /start."""
    register_user(message.chat_id)
    if is_banned(message.chat_id):
        await _rx(message.reply("⛔️ شما مسدود شده‌اید."))
        return
    ...
    try:
        ...
        await _rx(message.reply_inline(
            welcome or (
                ...
                "🔗 لینک پست/ریل اینستاگرام، پینترست یا پست روبینو بفرست تا دانلودش کنم.\n"
                "🎵 یا اسم یک آهنگ بنویس تا برات پیدا و دانلودش کنم.\n"
                "یا از منوی زیر یکی از قابلیت‌های هوش مصنوعی رو انتخاب کن 🎨\n\n"
                "/help — راهنما"
            ),
            start_menu_keypad(),
        ))
...
async def help_cmd(bot: Robot, message: Message):
    ...
    try:
        await _rx(message.reply(
            ...
            "• /image — ساخت عکس با هوش مصنوعی (فارسی هم می‌فهمه)\n"
...
@bot.on_message(commands=["test_join4b"])
async def test_join4b_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: سورس واقعی check_join رو از خودِ پکیج
    rubka نصب‌شده در میاره، تا ببینیم کجا داره input() صدا می‌زنه و
    EOFError می‌ده.
    (قبلاً اسمش هم test_join4 بود که با تابع پایین‌تر تداخل داشت و
    overwrite می‌شد؛ برای همین به test_join4b تغییر کرد.)"""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    try:
        import rubka
        out = [f"📦 مسیر پکیج rubka: {rubka.__file__}"]
        try:
            out.append(f"📦 نسخه: {getattr(rubka, '__version__', 'نامشخص')}")
        except Exception:
            pass
        try:
            src = inspect.getsource(bot.check_join)
            out.append("--- سورس check_join ---\n" + src)
        except Exception as e:
            out.append(f"❌ نتونستم سورس check_join رو بگیرم: {type(e).__name__}: {e}")
        try:
            src2 = inspect.getsource(bot._get_client)
            out.append("--- سورس _get_client ---\n" + src2)
        except Exception as e:
            out.append(f"❌ نتونستم سورس _get_client رو بگیرم: {type(e).__name__}: {e}")
        try:
            src3 = inspect.getsource(bot.get_all_member)
            out.append("--- سورس get_all_member ---\n" + src3)
        except Exception as e:
            out.append(f"❌ نتونستم سورس get_all_member رو بگیرم: {type(e).__name__}: {e}")
        try:
            out.append(f"bot.session_name = {getattr(bot, 'session_name', '(نداره)')!r}")
        except Exception as e:
            out.append(f"❌ session_name: {e}")
        try:
            client_get_cls = bot._get_client.__func__.__globals__.get("Client_get")
            out.append(f"Client_get کلاس: {client_get_cls!r}")
            out.append(f"Client_get ماژول: {getattr(client_get_cls, '__module__', '؟')}")
            try:
                src4 = inspect.getsource(client_get_cls.__init__)
                out.append("--- سورس Client_get.__init__ ---\n" + src4)
            except Exception as e:
                out.append(f"❌ سورس __init__: {type(e).__name__}: {e}")
        except Exception as e:
            out.append(f"❌ پیدا کردن Client_get از globals شکست خورد: {type(e).__name__}: {e}")
        text = "\n\n".join(out)
        for i in range(0, len(text), 3500):
            await _rx(message.reply(text[i:i + 3500]))
    except Exception as e:
        logger.exception("خطا در test_join4")
        await _rx(message.reply(f"❌ خطای کلی: {type(e).__name__}: {e}"))


def _pyrubi_signature_probe_blocking(channel_guid: str) -> str:
    """امضای واقعی get_all_members توی pyrubi رو در میاره و چند شکل
    فراخوانی رو امتحان می‌کنه (دقیقاً همون تستی که برای rubpy جواب داد) -
    تا مشخص بشه InvalidInput به‌خاطر اسم/شکل اشتباه پارامتره یا نه."""
    from pyrubi import Client
    client = Client(PYRUBI_SESSION_NAME)
    out = []
    try:
        sig = str(inspect.signature(client.get_all_members))
        out.append(f"امضای واقعی get_all_members{sig}")
    except Exception as e:
        out.append(f"❌ نتونستم امضا رو بگیرم: {type(e).__name__}: {e}")
    try:
        src = inspect.getsource(client.get_all_members)
        out.append("--- سورس get_all_members ---\n" + src[:2500])
    except Exception as e:
        out.append(f"❌ نتونستم سورس رو بگیرم: {type(e).__name__}: {e}")

    try:
        src5 = inspect.getsource(client.methods.getChatAllMembers)
        out.append("--- سورس methods.getChatAllMembers ---\n" + src5[:3000])
    except Exception as e:
        out.append(f"❌ نتونستم سورس getChatAllMembers رو بگیرم: {type(e).__name__}: {e}")

    try:
        from pyrubi.utils import Utils
        chat_type = Utils.getChatTypeByGuid(objectGuid=channel_guid)
        out.append(f"chatType تشخیص‌داده‌شده برای این GUID: {chat_type!r}")
    except Exception as e:
        out.append(f"❌ نتونستم chatType رو بگیرم: {type(e).__name__}: {e}")

    try:
        src6 = inspect.getsource(client.methods.network.request)
        out.append("--- سورس network.request ---\n" + src6[:3000])
    except Exception as e:
        out.append(f"❌ نتونستم سورس network.request رو بگیرم: {type(e).__name__}: {e}")

    raw_attempts = [
        ("فقط channel_guid (بدون search_text/start_id)", lambda: client.methods.network.request(
            method="getChannelAllMembers", input={"channel_guid": channel_guid})),
        ("channel_guid + search_text='' + start_id=''", lambda: client.methods.network.request(
            method="getChannelAllMembers", input={"channel_guid": channel_guid, "search_text": "", "start_id": ""})),
        ("object_guid به‌جای channel_guid", lambda: client.methods.network.request(
            method="getChannelAllMembers", input={"object_guid": channel_guid})),
    ]
    for name, fn in raw_attempts:
        try:
            result = fn()
            out.append(f"• RAW {name}: ✅ {result!r}"[:600])
        except Exception as e:
            out.append(f"• RAW {name}: ❌ {type(e).__name__}: {e}")

    attempts = [
        ("search_text='' (رشته‌ی خالی به‌جای None)", lambda: client.get_all_members(channel_guid, search_text="")),
        ("positional فقط guid", lambda: client.get_all_members(channel_guid)),
        ("object_guid=", lambda: client.get_all_members(object_guid=channel_guid)),
        ("channel_guid=", lambda: client.get_all_members(channel_guid=channel_guid)),
        ("guid=", lambda: client.get_all_members(guid=channel_guid)),
    ]
    for name, fn in attempts:
        try:
            result = fn()
            out.append(f"• {name}: ✅ {result!r}"[:600])
        except TypeError as e:
            out.append(f"• {name}: ⚪️ پارامتر نامعتبره: {e}")
        except Exception as e:
            out.append(f"• {name}: ❌ {type(e).__name__}: {e}")
    return "\n\n".join(out)


@bot.on_message(commands=["test_pyrubi2"])
async def test_pyrubi2_cmd(bot: Robot, message: Message):
    ...
    if not channel_guid:
        ...
        return
    if not (os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi") or glob.glob(PYRUBI_SESSION_NAME + "*")):
        await _rx(message.reply("❌ اکانت pyrubi هنوز لاگین نشده."))
        return
    await _rx(message.reply("🔎 دارم امضای واقعی get_all_members رو در میارم و چند شکل فراخوانی رو تست میکنم..."))
    try:
        text = await asyncio.to_thread(_pyrubi_signature_probe_blocking, channel_guid)
        for i in range(0, len(text), 3500):
            await _rx(message.reply(text[i:i + 3500]))
    except Exception as e:
        logger.exception("خطا در test_pyrubi2")
        await _rx(message.reply(f"❌ خطای کلی: {type(e).__name__}: {e}"))


def _pyrubi_find_single_check_blocking(channel_guid: str, user_guid: str) -> str:
    """دنبال متدی می‌گردیم که وضعیت عضویتِ یه کاربر مشخص رو مستقیم بپرسه
    (نه کل لیست اعضا رو بگیره) - چون get_all_members/getChannelAllMembers
    حتی برای سازنده‌ی کانال InvalidInput می‌ده، احتمالاً روبیکا این
    قابلیت رو برای کانال کلاً باز نکرده، و باید دنبال یه API سبک‌تر شبیه
    'وضعیت این یه کاربر توی این کانال چیه' بگردیم."""
    from pyrubi import Client
    client = Client(PYRUBI_SESSION_NAME)
    out = []

    all_names = [n for n in dir(client) if not n.startswith("_") and callable(getattr(client, n, None))]
    keywords = ["member", "participant", "subscrib", "status", "invite"]
    candidates = [n for n in all_names if any(k in n.lower() for k in keywords)]
    out.append("همه‌ی متدهای مرتبط پیدا‌شده: " + ", ".join(candidates))
...
@bot.on_message(commands=["test_pyrubi3"])
async def test_pyrubi3_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: دنبال متد چک عضویت تک‌کاربره می‌گرده
    (به‌جای گرفتن کل لیست اعضا)."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
    ...
    if not (os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi") or glob.glob(PYRUBI_SESSION_NAME + "*")):
        await _rx(message.reply("❌ اکانت pyrubi هنوز لاگین نشده."))
        return
    await _rx(message.reply("🔎 دارم دنبال متد چک عضویت تک‌کاربره می‌گردم..."))
    try:
        text = await asyncio.to_thread(_pyrubi_find_single_check_blocking, channel_guid, message.sender_id)
        for i in range(0, len(text), 3500):
            await _rx(message.reply(text[i:i + 3500]))
    except Exception as e:
        logger.exception("خطا در test_pyrubi3")
        await _rx(message.reply(f"❌ خطای کلی: {type(e).__name__}: {e}"))


def _pyrubi_probe_chatallmembers_blocking(channel_guid: str) -> str:
    """getChatAllMembers رو (به‌جای getChannelAllMembers) با چند ترکیب
    پارامتر تست می‌کنه - چون توی سورس pyrubi این متد جداگونه از
    get_all_members وجود داره و ممکنه اسم متد درست همینه، نه اون یکی."""
    from pyrubi import Client
    client = Client(PYRUBI_SESSION_NAME)
    out = []

    raw_attempts = [
        ("object_guid + type=Channel",
         lambda: client.methods.network.request(
             method="getChatAllMembers",
             input={"object_guid": channel_guid, "type": "Channel", "search_text": "", "start_id": None})),
        ("object_guid + type=Channel (بدون start_id)",
         lambda: client.methods.network.request(
             method="getChatAllMembers",
             input={"object_guid": channel_guid, "type": "Channel", "search_text": ""})),
        ("channel_guid + type=Channel",
         lambda: client.methods.network.request(
             method="getChatAllMembers",
             input={"channel_guid": channel_guid, "type": "Channel", "search_text": "", "start_id": None})),
        ("فقط object_guid (بدون type)",
         lambda: client.methods.network.request(
             method="getChatAllMembers",
             input={"object_guid": channel_guid, "search_text": "", "start_id": None})),
    ]
    for name, fn in raw_attempts:
        try:
            result = fn()
            out.append(f"• {name}: ✅ {result!r}"[:700])
        except Exception as e:
            out.append(f"• {name}: ❌ {type(e).__name__}: {e}")

    for attr_name in dir(client):
        if "chat_all_member" in attr_name.lower() or "chatallmember" in attr_name.lower():
            try:
                sig = str(inspect.signature(getattr(client, attr_name)))
                out.append(f"متد کلاینت پیدا شد: {attr_name}{sig}")
            except Exception as e:
                out.append(f"⚪️ {attr_name} پیدا شد ولی امضاش نگرفتم: {e}")

    return "\n\n".join(out)


@bot.on_message(commands=["pyrubi_filecheck"])
async def pyrubi_filecheck_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: به‌جای فرض کردن فرمت فایل (که اشتباه از آب
    دراومد - فایل .pyrubi لزوماً sqlite خام نیست)، مستقیم با خودِ pyrubi
    یه get_me() تست می‌کنه تا ببینه سشن واقعاً معتبره یا نه."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    out = []
    session_path = PYRUBI_SESSION_NAME + ".pyrubi"
    if not os.path.exists(session_path):
        await _rx(message.reply("❌ فایل session پیرابی روی دیسک نیست."))
        return
    size = os.path.getsize(session_path)
    out.append(f"📦 حجم فایل: {size} بایت")

    def _test_blocking():
        import builtins

        def _blocked_input(prompt=""):
            raise RuntimeError(f"سشن نامعتبره - pyrubi خواست لاگین اینتراکتیو کنه (prompt={prompt!r})")
        ...
        try:
            ...
            client = Client(PYRUBI_SESSION_NAME)
            me = client.get_me()
            return f"✅ سشن معتبره - get_me(): {me!r}"[:600]
        except Exception as e:
            return f"❌ سشن نامعتبره یا فایل خرابه: {type(e).__name__}: {e}"
        finally:
            builtins.input = original_input

    result = await asyncio.to_thread(_test_blocking)
    out.append(result)
    await _rx(message.reply("\n".join(out)))
...
def _pyrubi_check_join_blocking(channel_guid: str, user_guid: str) -> str:
    """تست متد آماده‌ی check_join که با /pyrubi_handlers پیدا شد.
    اگه INVALID_INPUT بده، سورس واقعی متد رو هم در میاریم تا ببینیم دقیقاً
    چه درخواستی می‌فرسته (شاید فقط برای گروه کار کنه، نه کانال)."""
    from pyrubi import Client
    client = Client(PYRUBI_SESSION_NAME)
    out = []
    try:
        result = client.check_join(channel_guid, user_guid)
        out.append(f"✅ check_join({channel_guid!r}, {user_guid!r}) = {result!r}")
        return "\n".join(out)
    except Exception as e:
        out.append(f"❌ خطا: {type(e).__name__}: {e}")

    try:
        src = inspect.getsource(client.check_join)
        out.append("--- سورس check_join ---\n" + src[:2500])
    except Exception as e:
        out.append(f"❌ نتونستم سورس رو بگیرم: {type(e).__name__}: {e}")
...
@bot.on_message(commands=["test_pyrubi5"])
async def test_pyrubi5_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: تست check_join برای خودِ ادمین روی کانال
    تنظیم‌شده. اگه جواب درست بده، این متد جایگزین منطق فعلی is_member میشه."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
    ...
    if not os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi"):
        await _rx(message.reply("❌ سشن پیرابی موجود نیست."))
        return
    await _rx(message.reply("🔎 دارم check_join رو تست میکنم..."))
    try:
        text = await asyncio.to_thread(_pyrubi_check_join_blocking, channel_guid, message.sender_id)
        for i in range(0, len(text), 3500):
            await _rx(message.reply(text[i:i + 3500]))
    except Exception as e:
        logger.exception("خطا در test_pyrubi5")
        await _rx(message.reply(f"❌ خطای کلی: {type(e).__name__}: {e}"))
...
async def test_pyrubi4_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: تست getChatAllMembers."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
    ...
    if not (os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi") or glob.glob(PYRUBI_SESSION_NAME + "*")):
        await _rx(message.reply("❌ اکانت pyrubi هنوز لاگین نشده."))
        return
    await _rx(message.reply("🔎 دارم getChatAllMembers رو تست میکنم..."))
    try:
        text = await asyncio.to_thread(_pyrubi_probe_chatallmembers_blocking, channel_guid)
        for i in range(0, len(text), 3500):
            await _rx(message.reply(text[i:i + 3500]))
    except Exception as e:
        logger.exception("خطا در test_pyrubi4")
        await _rx(message.reply(f"❌ خطای کلی: {type(e).__name__}: {e}"))


def _pyrubi_probe_checkjoin_blocking(channel_guid: str, user_guid: str) -> str:
    """چون client.check_join(object_guid, user_guid) با InvalidInput خطا
    می‌ده، اول سورس واقعیِ client.methods.checkJoin (نه خودِ wrapper) رو در
    میاریم تا ببینیم دقیقاً چه method و input خامی به سرور روبیکا می‌فرسته.
    بعد چند شکلِ مختلف پارامتر رو مستقیم با client.methods.network.request
    امتحان می‌کنیم - دقیقاً همون الگویی که برای getChatAllMembers جواب داد."""
    from pyrubi import Client
    client = Client(PYRUBI_SESSION_NAME)
    out = []

    try:
        sig = str(inspect.signature(client.methods.checkJoin))
        out.append(f"امضای واقعی methods.checkJoin{sig}")
    except Exception as e:
        out.append(f"❌ نتونستم امضا رو بگیرم: {type(e).__name__}: {e}")

    try:
        src = inspect.getsource(client.methods.checkJoin)
        out.append("--- سورس methods.checkJoin ---\n" + src[:3000])
    except Exception as e:
        out.append(f"❌ نتونستم سورس methods.checkJoin رو بگیرم: {type(e).__name__}: {e}")

    raw_attempts = [
        ("objectGuid + userGuid (دقیقاً مثل check_join wrapper)",
         lambda: client.methods.network.request(
             method="checkJoin",
             input={"objectGuid": channel_guid, "userGuid": user_guid})),
        ("object_guid + user_guid (snake_case)",
         lambda: client.methods.network.request(
             method="checkJoin",
             input={"object_guid": channel_guid, "user_guid": user_guid})),
        ("objectGuid + userGuid + type=Channel",
         lambda: client.methods.network.request(
             method="checkJoin",
             input={"objectGuid": channel_guid, "userGuid": user_guid, "type": "Channel"})),
        ("channel_guid + user_guid",
         lambda: client.methods.network.request(
             method="checkJoin",
             input={"channel_guid": channel_guid, "user_guid": user_guid})),
        ("objectGuid + memberGuid",
         lambda: client.methods.network.request(
             method="checkJoin",
             input={"objectGuid": channel_guid, "memberGuid": user_guid})),
    ]
    for name, fn in raw_attempts:
        try:
            result = fn()
            out.append(f"• RAW {name}: ✅ {result!r}"[:700])
        except Exception as e:
            out.append(f"• RAW {name}: ❌ {type(e).__name__}: {e}")

    try:
        candidates = [n for n in dir(client.methods)
                      if not n.startswith("_")
                      and ("join" in n.lower() or ("check" in n.lower() and "member" in n.lower()))]
        out.append("متدهای مرتبطِ دیگه توی client.methods: " + ", ".join(candidates))
        for name in candidates:
            try:
                sig = str(inspect.signature(getattr(client.methods, name)))
                out.append(f"• methods.{name}{sig}")
            except Exception as e:
                out.append(f"⚪️ methods.{name}: امضا نگرفتم: {e}")
    except Exception as e:
        out.append(f"❌ نتونستم دنبال متدهای مشابه بگردم: {type(e).__name__}: {e}")

    return "\n\n".join(out)


@bot.on_message(commands=["test_pyrubi6"])
async def test_pyrubi6_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: سورس خامِ methods.checkJoin رو در میاره و
    چند شکل ورودی رو مستقیم روی سرور تست می‌کنه."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
    ...
    if not (os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi") or glob.glob(PYRUBI_SESSION_NAME + "*")):
        await _rx(message.reply("❌ اکانت pyrubi هنوز لاگین نشده."))
        return
    await _rx(message.reply("🔎 دارم سورس خامِ checkJoin رو در میارم و چند شکل ورودی رو تست میکنم..."))
    try:
        text = await asyncio.to_thread(_pyrubi_probe_checkjoin_blocking, channel_guid, message.sender_id)
        for i in range(0, len(text), 3500):
            await _rx(message.reply(text[i:i + 3500]))
    except Exception as e:
        logger.exception("خطا در test_pyrubi6")
        await _rx(message.reply(f"❌ خطای کلی: {type(e).__name__}: {e}"))


def _pyrubi_probe_checkjoin2_blocking(channel_guid: str, user_guid: str) -> str:
    """checkJoin خودش یه متد سرور نیست - فقط getChatInfo + getChatAllMembers
    رو صدا می‌زنه. اگه کاربر یوزرنیم عمومی نداشته باشه، userUsername می‌شه
    None و همون None به‌عنوان searchText به getChatAllMembers پاس داده
    می‌شه که احتمالاً باعث InvalidInput روی سرور می‌شه. اینجا اول یوزرنیم
    رو چک می‌کنیم، بعد getChatAllMembers رو مستقیم (نه از راه checkJoin)
    با searchText='' (رشته‌ی خالی، نه None) و justGetGuids=True امتحان
    می‌کنیم تا ببینیم مشکل واقعاً همینه یا نه."""
    from pyrubi import Client
    client = Client(PYRUBI_SESSION_NAME)
    out = []
    ...
    user_username = None
    try:
        chat_info = client.methods.getChatInfo(user_guid)
        user_username = chat_info.get("user", {}).get("username")
        out.append(f"✅ getChatInfo({user_guid!r}) کار کرد. username کاربر: {user_username!r}")
    except Exception as e:
        out.append(f"❌ getChatInfo({user_guid!r}) خطا داد: {type(e).__name__}: {e}")

    try:
        src = inspect.getsource(client.methods.getChatAllMembers)
        out.append("--- سورس methods.getChatAllMembers ---\n" + src[:3000])
    except Exception as e:
        out.append(f"❌ نتونستم سورس getChatAllMembers رو بگیرم: {type(e).__name__}: {e}")

    wrapper_attempts = [
        ("searchText='' (رشته‌ی خالی) + justGetGuids=True",
         lambda: client.methods.getChatAllMembers(
             objectGuid=channel_guid, searchText="", startId=None, justGetGuids=True)),
        ("searchText=None (دقیقاً مثل مسیر داخلی checkJoin وقتی یوزرنیم نداره)",
         lambda: client.methods.getChatAllMembers(
             objectGuid=channel_guid, searchText=None, startId=None, justGetGuids=True)),
    ]
    if user_username:
        wrapper_attempts.append((
            f"searchText={user_username!r} (یوزرنیم واقعی کاربر) + justGetGuids=True",
            lambda: client.methods.getChatAllMembers(
                objectGuid=channel_guid, searchText=user_username, startId=None, justGetGuids=True)))

    for name, fn in wrapper_attempts:
        try:
            result = fn()
            is_member = (user_guid in result) if isinstance(result, (list, set, tuple)) else None
            out.append(f"• {name}: ✅ نتیجه: {result!r}"[:600] + f"\n  کاربر توی لیست هست؟ {is_member}")
        except Exception as e:
            out.append(f"• {name}: ❌ {type(e).__name__}: {e}")
...
@bot.on_message(commands=["test_pyrubi7"])
async def test_pyrubi7_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: فرضیه‌ی «یوزرنیم خالی → searchText=None →
    InvalidInput» رو تست می‌کنه و getChatAllMembers رو مستقیم (بدون رد شدن
    از checkJoin) با searchText='' امتحان می‌کنه."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
    ...
    if not (os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi") or glob.glob(PYRUBI_SESSION_NAME + "*")):
        await _rx(message.reply("❌ اکانت pyrubi هنوز لاگین نشده."))
        return
    await _rx(message.reply("🔎 دارم فرضیه‌ی یوزرنیم/searchText رو تست میکنم..."))
    try:
        text = await asyncio.to_thread(_pyrubi_probe_checkjoin2_blocking, channel_guid, message.sender_id)
        for i in range(0, len(text), 3500):
            await _rx(message.reply(text[i:i + 3500]))
    except Exception as e:
        logger.exception("خطا در test_pyrubi7")
        await _rx(message.reply(f"❌ خطای کلی: {type(e).__name__}: {e}"))


def _pyrubi_sanity_check_blocking(channel_guid: str) -> str:
    """دیگه دنبال حدس‌زدن پارامتر نمی‌گردیم - این یه تست پایه‌ایه برای اینکه
    ببینیم اصلاً خودِ سشن pyrubi و تشخیص نوع چت سالمه یا نه. اگه حتی
    getChatInfo روی GUID خودِ اکانت هم InvalidInput بده، یعنی مشکل از
    سشن/اکانته، نه از فرمت پارامترهای checkJoin/getChatAllMembers."""
    from pyrubi import Client
    from pyrubi.utils import Utils
    client = Client(PYRUBI_SESSION_NAME)
    out = []
    ...
    try:
        me = client.get_me()
        my_guid = me.get("user", {}).get("user_guid") or me.get("user_guid")
        out.append(f"✅ get_me() کار کرد. GUID خودت: {my_guid!r}\nخروجی کامل: {me!r}"[:800])
    except Exception as e:
        out.append(f"❌ get_me() خطا داد: {type(e).__name__}: {e}")

    if my_guid:
        try:
            info = client.methods.getChatInfo(my_guid)
            out.append(f"✅ getChatInfo(GUID خودت) کار کرد: {info!r}"[:600])
        except Exception as e:
            out.append(f"❌ getChatInfo(GUID خودت) خطا داد: {type(e).__name__}: {e}")

    try:
        info2 = client.methods.getChatInfo(channel_guid)
        out.append(f"✅ getChatInfo(GUID کانال) کار کرد: {info2!r}"[:600])
    except Exception as e:
        out.append(f"❌ getChatInfo(GUID کانال) خطا داد: {type(e).__name__}: {e}")

    try:
        chat_type_channel = Utils.getChatTypeByGuid(objectGuid=channel_guid)
        out.append(f"نوع چتِ تشخیص‌داده‌شده برای کانال: {chat_type_channel!r}")
    except Exception as e:
        out.append(f"❌ getChatTypeByGuid(کانال) خطا داد: {type(e).__name__}: {e}")

    if my_guid:
        try:
            chat_type_me = Utils.getChatTypeByGuid(objectGuid=my_guid)
            out.append(f"نوع چتِ تشخیص‌داده‌شده برای GUID خودت: {chat_type_me!r}")
        except Exception as e:
            out.append(f"❌ getChatTypeByGuid(خودت) خطا داد: {type(e).__name__}: {e}")

    return "\n\n".join(out)


@bot.on_message(commands=["test_pyrubi8"])
async def test_pyrubi8_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: تست پایه‌ای سلامت سشن (نه حدس پارامتر) -
    آخرین قدم قبل از تصمیم برای ادامه یا کنار گذاشتنِ این مسیر."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
    ...
    if not (os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi") or glob.glob(PYRUBI_SESSION_NAME + "*")):
        await _rx(message.reply("❌ اکانت pyrubi هنوز لاگین نشده."))
        return
    await _rx(message.reply("🔎 دارم سلامت سشن رو چک میکنم (get_me، getChatInfo، تشخیص نوع چت)..."))
    try:
        text = await asyncio.to_thread(_pyrubi_sanity_check_blocking, channel_guid)
        for i in range(0, len(text), 3500):
            await _rx(message.reply(text[i:i + 3500]))
    except Exception as e:
        logger.exception("خطا در test_pyrubi8")
        await _rx(message.reply(f"❌ خطای کلی: {type(e).__name__}: {e}"))


def _pyrubi_probe_channelinfo_blocking(channel_guid: str) -> str:
    """جالبه که getChatTypeByGuid درست 'Channel' رو تشخیص داد، ولی
    getChatInfo روی همون GUID InvalidInput داد - حتی با اینکه اکانت ادمین
    کاناله. پس دنبال این می‌گردیم که getChatInfo برای نوع Channel دقیقاً
    چه متد خامی صدا می‌زنه (شاید اسم پارامتر یا متد سرور برای کانال با
    گروه/یوزر فرق داره)، و چند حالت خام رو مستقیم امتحان می‌کنیم."""
    from pyrubi import Client
    client = Client(PYRUBI_SESSION_NAME)
    out = []

    try:
        src = inspect.getsource(client.methods.getChatInfo)
        out.append("--- سورس methods.getChatInfo ---\n" + src[:3000])
    except Exception as e:
        out.append(f"❌ نتونستم سورس getChatInfo رو بگیرم: {type(e).__name__}: {e}")

    raw_attempts = [
        ("method=getChannelInfo, channel_guid=",
         lambda: client.methods.network.request(
             method="getChannelInfo", input={"channel_guid": channel_guid})),
        ("method=getObjectInfo, object_guid=",
         lambda: client.methods.network.request(
             method="getObjectInfo", input={"object_guid": channel_guid})),
        ("method=getChannelInfo, object_guid=",
         lambda: client.methods.network.request(
             method="getChannelInfo", input={"object_guid": channel_guid})),
    ]
    for name, fn in raw_attempts:
        try:
            result = fn()
    ...
    # این متد نیازی به عضویت نداره - فقط با لینک عمومی کانال کار می‌کنه.
    # اگه این جواب داد یعنی شبکه/سشن سالمه و مشکل واقعاً مختص GUID/دسترسیه.
    channel_username = get_setting("channel_username") or get_setting("channel_link")
    if channel_username:
        try:
            preview = client.methods.chatPreviewByJoinLink(channel_username)
            out.append(f"✅ chatPreviewByJoinLink({channel_username!r}) کار کرد: {preview!r}"[:600])
        except Exception as e:
            out.append(f"❌ chatPreviewByJoinLink({channel_username!r}) خطا داد: {type(e).__name__}: {e}")
    else:
        out.append("⚪️ یوزرنیم/لینک کانال توی تنظیمات پیدا نشد، chatPreviewByJoinLink تست نشد.")

    out.append(f"GUID کانالِ استفاده‌شده در این تست: {channel_guid!r} (طول: {len(channel_guid)})")

    return "\n\n".join(out)


@bot.on_message(commands=["test_pyrubi9"])
async def test_pyrubi9_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: چرا getChatInfo روی کانال InvalidInput
    می‌ده با وجود اینکه اکانت ادمین کاناله و GUID درسته."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
    ...
    if not (os.path.exists(PYRUBI_SESSION_NAME + ".pyrubi") or glob.glob(PYRUBI_SESSION_NAME + "*")):
        await _rx(message.reply("❌ اکانت pyrubi هنوز لاگین نشده."))
        return
    await _rx(message.reply("🔎 دارم دنبال دلیل InvalidInput روی getChatInfo کانال می‌گردم..."))
    try:
        text = await asyncio.to_thread(_pyrubi_probe_channelinfo_blocking, channel_guid)
        for i in range(0, len(text), 3500):
            await _rx(message.reply(text[i:i + 3500]))
    except Exception as e:
        logger.exception("خطا در test_pyrubi9")
        await _rx(message.reply(f"❌ خطای کلی: {type(e).__name__}: {e}"))


# ─── لیسنر واقعیِ رویداد جوین کانال ─────────────────────────────────────────
# این ترد یه سشن pyrubi رو زنده نگه می‌داره و منفعلانه به آپدیت‌هایی که سرور
# روبیکا پوش می‌کنه گوش می‌ده (نه پرس‌وجو با پارامتر مثل checkJoin که با
# InvalidInput خراب می‌شد). وقتی یه آپدیت مربوط به عضو جدیدِ کانالِ
# تنظیم‌شده تشخیص داده بشه، مستقیم mark_joined صدا زده می‌شه - یعنی تاییدِ
# عضویت کاملاً خودکار و بدون نیاز به فوروارد دستیه. چون فرمت دقیق این
# آپدیت‌ها مستند نیست، همه‌شون رو هم کامل لاگ می‌کنیم تا اگه تشخیص هیوریستیک
# چیزی رو جا انداخت، از لاگ Railway بشه دقیقش کرد.
...
async def test_join_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: check_join خودِ rubka رو با هر دو نوع GUID
    (chat_id پیام‌خصوصی و sender_id واقعی کاربر) تست می‌کنه تا معلوم بشه بعد
    از آپدیت نسخه‌ی کتابخونه، باگ قدیمی EOFError حل شده یا نه - و اگه حل
    شده، کدوم GUID نتیجه‌ی درست می‌ده."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
    ...
    try:
        import rubka
        version = getattr(rubka, "__version__", "نامشخص")
    except Exception:
        version = "نامشخص"
    await _rx(message.reply(f"🔎 نسخه‌ی نصب‌شده‌ی rubka: {version}\n📣 GUID کانال: {channel_guid}"))

    async def _try(label: str, user_guid: str):
        await _rx(message.reply(f"⏳ در حال تست check_join با {label} = {user_guid} ..."))
        try:
            result = await _rx(bot.check_join(channel_guid, user_guid))
            await _rx(message.reply(f"✅ {label}: بدون خطا اجرا شد.\nنتیجه‌ی خام: {result!r}"))
        except AttributeError as e:
            await _rx(message.reply(f"❌ {label}: متد check_join روی این نسخه اصلاً وجود نداره:\n{e}"))
        except Exception as e:
            await _rx(message.reply(f"❌ {label}: خطا داد:\n{type(e).__name__}: {e}"))

    await _try("chat_id (پیام‌خصوصی، پیشوند b0)", message.chat_id)
    await _try("sender_id (GUID واقعی کاربر، پیشوند u0)", message.sender_id)

    await _rx(message.reply(
        "📌 اگه هر دو تست بالا ❌ خطا دادن (خصوصاً EOFError) → این نسخه هم هنوز "
        "همون مشکل قدیمی رو داره؛ باید بریم سراغ برگردوندن pyrubi.\n"
        "📌 اگه یکی از دو تا ✅ بدون خطا نتیجه‌ی True/False داد → همون نتیجه رو "
        "برام بفرست (کدوم GUID بود) تا is_member/handle_check_join_callback رو "
        "بر همون اساس بازنویسی کنم."
    ))
...
@bot.on_message(commands=["rubino_lasterror"])
async def rubino_lasterror_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: traceback کامل آخرین خطای subprocess دانلود روبینو رو نشون میده."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    if not _LAST_RUBINO_ERROR:
        await _rx(message.reply("هنوز خطایی از subprocess دانلود روبینو ثبت نشده."))
        return
    text = (
        f"❌ {_LAST_RUBINO_ERROR.get('error_type')}: {_LAST_RUBINO_ERROR.get('error')}\n\n"
        f"--- traceback ---\n{_LAST_RUBINO_ERROR.get('traceback')}"
    )
    for i in range(0, len(text), 3500):
        await _rx(message.reply(text[i:i + 3500]))
...
async def handle_text(bot: Robot, message: Message):
    ...
    try:
        ...
        # ─── چت با هوش مصنوعی: اگه کاربر توی حالت چته و فایلی نفرستاده ───
        if message.chat_id in chat_mode_users and text and not file_meta:
            _spawn(handle_chat_message(message, text))
            return

        # ─── ساخت/ادیت عکس با AI: اگه کاربر توی جریان /image یا /edit هست ───
        img_state = user_image_state.get(message.chat_id)
        if img_state:
            mode = img_state.get("mode")
            if mode == "await_edit_photo" and file_meta:
                user_image_state.pop(message.chat_id, None)
                _spawn(handle_image_edit_upload(message, file_meta, text))
                return
            if mode == "await_prompt" and text and not file_meta:
                user_image_state.pop(message.chat_id, None)
                _spawn(handle_image_generate(message, text))
                return
            if mode == "await_edit_text" and text and not file_meta:
                img_path = img_state.get("image_path")
                user_image_state.pop(message.chat_id, None)
                _spawn(handle_image_edit_from_path(message, img_path, text))
                return
...
async def on_callback(bot: Robot, message: Message):
    ...
    try:
        ...
        elif btn_id == "start_image":
            _spawn(start_menu_image_callback(message))
        elif btn_id == "start_edit":
            _spawn(start_menu_edit_callback(message))
...
async def _handle_inline_click(inline: dict):
    ...
    try:
        if btn_id.startswith("dl_"):
            ...
            await start_menu_image_callback(msg)
        elif btn_id == "start_edit":
            await start_menu_edit_callback(msg)
        elif btn_id == "start_chat":
