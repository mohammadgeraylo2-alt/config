...
async def test_link_button_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: چک می‌کنه دکمه‌ی نوع Link (با ساختار دستیِ
    درست‌شده‌ی make_link_button، نه متد بگ‌دار builder.button_link) کار
    می‌کنه یا نه."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    try:
        kp = make_link_keypad(
            make_link_button("test_google", "🔗 تست گوگل", "https://google.com"),
            make_link_button("test_rubika", "🔗 تست کانال", "https://rubika.ir/instasavexx"),
        )
        raw = json.dumps(kp, ensure_ascii=False, indent=2)
        await _rx(message.reply_inline(
            f"این دو تا دکمه رو بزن، ببین کدوم باز میشه:\n\n📦 ساختار خام کی‌پد:\n{raw}",
            kp,
        ))
    except Exception as e:
        tb = traceback.format_exc()
        await _rx(message.reply(f"❌ خطا توی ساخت/ارسال تست:\n{type(e).__name__}: {e}\n\n{tb[-1500:]}"))
