...
# ─── تنظیمات واترمارک روی ویدیو ──────────────────────────────────────────────
WATERMARK_ENABLED = os.environ.get("WATERMARK_ENABLED", "1") == "1"
WATERMARK_TEXT = os.environ.get("WATERMARK_TEXT", "InstaSaveXX@")
# مسیر یه فونت ttf (اختیاری). اگه ست نشه، ffmpeg سعی میکنه از فونت پیش‌فرض
# سیستم (fontconfig) استفاده کنه. برای متن فارسی/راست‌به‌چپ لازمه ffmpeg با
# libfribidi/fontconfig بیلد شده باشه وگرنه حروف بهم‌ریخته نشون داده میشن -
# برای همین پیش‌فرض روی متن انگلیسی (InstaSaveXX@) گذاشته شده که همیشه درست نشون داده بشه.
WATERMARK_FONT = os.environ.get("WATERMARK_FONT", "")
for _fp in (
    WATERMARK_FONT,
...
def _escape_drawtext(text: str) -> str:
    """کاراکترهای خاص فیلتر drawtext رو escape میکنه (: ' \\ %)."""
    return (
        text.replace("\\", "\\\\\\\\")
        .replace(":", "\\:")
        .replace("'", "\u2019")   # آپاستروف با کوتیشن تایپوگرافیک جایگزین میشه چون توی drawtext مشکل‌ساز میشه
        .replace("%", "\\%")
    )


def _apply_watermark_blocking(input_path: str, output_path: str) -> bool:
    """یه واترمارک متنی گوشه‌ی پایین-راست ویدیو با ffmpeg (drawtext) اضافه میکنه.
    خروجی رو توی output_path میذاره. اگه شکست بخوره False برمیگردونه و صدا زننده
    باید از فایل اصلی (بدون واترمارک) استفاده کنه تا کل فرآیند ارسال fail نشه."""
    text = _escape_drawtext(WATERMARK_TEXT)
    font_part = f"fontfile='{WATERMARK_FONT}':" if WATERMARK_FONT else ""
    drawtext = (
    ...
    try:
        ...
        if result.returncode != 0:
            logger.warning(f"واترمارک ffmpeg شکست خورد: {result.stderr.decode(errors='ignore')[-500:]}")
            return False
        return os.path.exists(output_path) and os.path.getsize(output_path) > 0
    except Exception as e:
        logger.warning(f"واترمارک ffmpeg با خطا مواجه شد: {e}")
        return False
...
async def send_video_native(message: "Message", file_path: str, caption: str = "", file_name: str = None):
    ...
    if WATERMARK_ENABLED:
        candidate = f"{file_path}_wm.mp4"
        ok = await asyncio.to_thread(_apply_watermark_blocking, file_path, candidate)
        if ok:
            send_path = candidate
            watermarked_path = candidate
        else:
            logger.warning("واترمارک زده نشد، ویدیوی اصلی بدون واترمارک ارسال میشه.")

    try:
        return await asyncio.to_thread(
            _rubika_send_video_blocking, message.chat_id, send_path, caption, file_name, keypad
        )
    except Exception as e:
        logger.warning(f"ارسال ویدیو با type=Video خام شکست خورد، fallback به reply_document: {e}")
        result = await _rx(message.reply_document(path=send_path, text=caption))
        if keypad:
            try:
