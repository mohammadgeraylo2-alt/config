...
async def send_not_joined(message):
    ...
    text = "⛔️ برای استفاده از ربات، اول باید عضو کانال بشی:\n"
    if link:
        text += f"\n1️⃣ روی این لینک بزن و عضو شو:\n🔗 {link}\n"
    text += (
        "\n2️⃣ بعد از اینکه عضو شدی، برگرد و یکی از پست‌های همون کانال رو "
        "برای من فوروارد کن (تا بفهمم واقعاً عضو شدی)\n\n"
        "⚠️ فقط فوروارد کردن کافی نیست، اول باید واقعاً عضو کانال شده باشی."
    )
...
@bot.on_message(commands=["test_join4"])
async def test_join4_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: get_channel_info/get_channel_all_members رو
    روی یه channel_guid دلخواه (که خودت به‌عنوان آرگومان می‌فرستی) تست می‌کنه.
    هدف: فهمیدن اینکه INVALID_INPUT مال خودِ کانال هدفه (مثلاً چون اکانت
    لاگین‌شده عضوش نیست) یا یه مشکل عمومی‌تره - با تست روی کانالی که خودِ
    همین اکانت مطمئناً توش عضو/ادمینه (مثلاً یه کانال تستی که خودت باهاش
    ساختی)."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) < 2:
        await _rx(message.reply(
            "فرمت: /test_join4 <channel_guid>\n"
            "یه کانالی رو بده که مطمئنی اکانت لاگین‌شده (rubino_acc) توشه/ادمینشه."
        ))
        return
    test_guid = parts[1].strip()
    if not (os.path.exists("rubino_acc.rp") or glob.glob("rubino_acc*")):
        await _rx(message.reply("❌ اکانت روبینو هنوز لاگین نشده."))
        return
    await _rx(message.reply(f"🔎 در حال تست روی channel_guid = {test_guid!r} ..."))
    try:
        attempts = await asyncio.to_thread(_rubpy_check_join_blocking, test_guid, message.sender_id)
        chunk = ""
        for n, r in attempts:
            line = f"• {n}:\n{r}\n\n"
        ...
        if chunk:
            await _rx(message.reply(chunk))
        await _rx(message.reply(
            "📌 اگه اینجا هم INVALID_INPUT گرفتی → مشکل عمومیه (نه مال کانال "
            "هدف)، احتمالاً از خودِ سشن/اکانت.\n"
            "📌 اگه اینجا ✅ جواب داد ولی روی کانال هدف INVALID_INPUT می‌گیری → "
            "یعنی اکانت لاگین‌شده اصلاً عضو کانال هدف نیست (باید اول join بشه)."
        ))
    except Exception as e:
        logger.exception("خطا در test_join4")
        await _rx(message.reply(f"❌ خطای کلی: {type(e).__name__}: {e}"))
