...
# این تابع عمداً async هست و مستقیم await میشه، نه با asyncio.to_thread.
# دلیلش: نسخه‌ی sync کلاینت rubpy (`with Client(...)`) برای گرفتن/ساختن event
# loop از asyncio.get_event_loop() توی همون تردی که صداش می‌زنیم استفاده
# می‌کنه. asyncio.to_thread تردهاش رو از یه ThreadPoolExecutor مشترک برمی‌داره
# و همون تردها رو دوباره برای فراخوانی‌های بعدی استفاده می‌کنه؛ اگه یه بار
# event loop توی اون ترد بسته بشه (که rubpy موقع خروج از with می‌بنده)، دفعه‌ی
# بعد که همون ترد دوباره استفاده بشه get_event_loop() همون loop بسته‌شده رو
# برمی‌گردونه و باعث میشه اتصال قطع بشه و خطاهای عجیب مثل
# "'NoneType' object is not callable" بیفته. تست دستی توی کنسول همیشه توی یه
# پروسه/ترد تازه اجرا میشه، برای همین اونجا مشکلی دیده نمیشه ولی توی ربات که
# چند بار پشت‌سرهم صدا زده میشه، مشکل خودشو نشون میده.
async def _download_rubino_async(chat_id, text: str):
    ...
    try:
        ...
        async with Client("rubino_acc") as c:
            rubino = Rubino(c)
            result = await rubino.get_post_by_share_link(post_url)
