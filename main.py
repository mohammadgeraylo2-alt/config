...
async def _rx(value):
    """کتابخونه‌ی rubka (نسخه‌ی نصب‌شده روی Railway) متدهاش رو به‌صورت async
    ساخته (مثل message.reply / reply_document / status.edit / bot.check_join)
    و اگه await نشن، یا کرش می‌کنن (RuntimeError/TypeError) یا اصلاً ارسال
    نمیشن (coroutine was never awaited). این تابع کمکی، هم برای متدهای async
    و هم برای متدهای عادی sync درست کار می‌کنه، پس همه‌جا با await صداش می‌زنیم."""
...
async def handle_instagram(message: Message, text: str):
    ...
    try:
        ...
        if downloaded:
            await _rx(message.reply_document(path=video_path, text=CAPTION))
            ...
            try:
                ...
                if result and result.get("title"):
                    ...
                    if song_path and os.path.exists(song_path):
                        await _rx(message.reply_music(path=song_path, text=CAPTION, file_name=f"{title} - {artist}.mp3"))
    ...
    try:
        ...
        if file_path:
            ...
            if ext in ("mp4", "mov", "webm"):
                await _rx(message.reply_document(path=file_path, text=CAPTION))
            else:
...
async def handle_download_callback(message: Message, index: int):
    ...
    if not path or not os.path.exists(path):
        ...
        return
    try:
        await _rx(message.reply_music(path=path, text=CAPTION, file_name=f"{title} - {artist}.mp3"))
