...
async def _watch_pyrubi_login(chat_id, result_queue):
    ...
    _pyrubi_login_state["in_progress"] = False
    if status == "ok":
        await _rx(bot.send_message(chat_id, "✅ لاگین pyrubi موفق شد (فایل سشن روی دیسک ذخیره شد)."))
    else:
...
def _pyrubi_find_guid_blocking(username: str) -> str:
    ...
    for name in ("get_object_by_username", "get_chat_by_username", "get_object"):
        ...
        try:
            ...
            out.append(f"{name} خطا داد: {type(e).__name__}: {e}")
