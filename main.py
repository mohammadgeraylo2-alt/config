...
async def test_join4_cmd(bot: Robot, message: Message):
    ...
    try:
        ...
        try:
            from rubka.client import Client_get
            src4 = inspect.getsource(Client_get.__init__)
            out.append("--- سورس Client_get.__init__ ---\n" + src4)
        except Exception as e:
            out.append(f"❌ نتونستم Client_get.__init__ رو بگیرم (شاید مسیر ماژول فرق داره): {type(e).__name__}: {e}")
            try:
                out.append(f"کلاس Client_get از کجا میاد: {inspect.getmodule(type(bot._get_client()))}")
            except Exception as e2:
                out.append(f"❌ پیدا کردن ماژول Client_get هم شکست خورد: {e2}")
        text = "\n\n".join(out)
        for i in range(0, len(text), 3500):
            await _rx(message.reply(text[i:i + 3500]))
    except Exception as e:
