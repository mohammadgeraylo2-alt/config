...
def _pyrubi_is_member_blocking(channel_guid: str, user_guid: str) -> bool:
    ...
    with _members_cache_lock:
        ...
        if cache_valid:
            return str(user_guid) in _members_cache["guids"]
...
async def send_not_joined(message):
    ...
    text = "🔒 دسترسی قفله\n\n"
    if link:
        text += f"۱. اول عضو کانال شو:\n🔗 {link}\n\n"
    else:
        text += "۱. اول عضو کانال شو.\n\n"
    text += (
        "۲. بعدش یکی از پست‌های همون کانال رو مستقیم از داخل کانال به همین چت "
        "فوروارد کن (نه فقط دکمه‌ی زیر رو بزن) تا عضویتت تایید بشه."
    )
