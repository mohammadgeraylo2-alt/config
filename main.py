...
async def check_membership_live(user_guid) -> bool:
    """نکته‌ی مهم: user_guid اینجا در واقع message.sender_id هست که از
    بات‌پلتفرم rubka میاد - این GUID در یه فضای شناسه‌ی کاملاً جدا از GUID
    واقعی حسابیه که pyrubi (سلف‌بات، وصل به API پیام‌رسان) می‌بینه. برای
    همین دیگه اینجا چک زنده‌ی لیست اعضا با pyrubi رو روی user_guid انجام
    نمی‌دیم (چون هیچ‌وقت match نمی‌شه، صرف‌نظر از هر باگ دیگه). به‌جاش فقط
    فلگ has_joined رو چک می‌کنیم که با تایید فوروارد پست کانال (در
    handle_text) و کلیدشده بر همون sender_id/بات‌پلتفرم ثبت می‌شه - یعنی
    مقایسه‌ی «سیب با سیب»، نه «سیب با پرتقال»."""
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
...
async def is_member(user_guid, force_refresh: bool = False) -> bool:
    """نقطه‌ی ورودی اصلی برای چک عضویت. force_refresh دیگه اثری نداره (چون
    دیگه کشی برای لیست اعضا اینجا استفاده نمی‌شه) - فقط برای سازگاری با
    call siteهای قدیمی نگه داشته شده."""
    return await check_membership_live(user_guid)


# ─── تایید عضویت با فوروارد پست کانال ────────────────────────────────────────
# چون sender_id (فضای بات‌پلتفرم) و GUID واقعی حساب (فضای pyrubi) قابل
# مقایسه‌ی مستقیم نیستن، عضویت رو از راه دیگه‌ای تایید می‌کنیم: از کاربر
# می‌خوایم یه پست از خودِ کانال رو به این چت فوروارد کنه. GUID مبدأ فوروارد
# (که توی خودِ پیام دریافتی از بات‌پلتفرم میاد) با channel_guid تنظیم‌شده
# مقایسه می‌شه - این مقایسه چون هر دو طرف از فضای بات‌پلتفرم/API عمومی
# کانال هستن (نه GUID شخصی کاربر)، قابل اعتماده.
...
def _dig_guid(value):
    """از یه دیکشنری/شیء یا خودِ رشته، دنبال چیزی که شبیه GUID کانال باشه
    می‌گرده (پیشوند c0/g0/b0 برای کانال/گروه/بات)."""
    if value is None:
        return None
...
def _extract_forward_channel_guid(message: "Message"):
    ...
    for attr in _FORWARD_ATTR_CANDIDATES:
        guid = _dig_guid(getattr(message, attr, None))
        if guid:
            return guid
    try:
...
def _debug_forward_dump(message: "Message") -> str:
    ...
    بشه اسم واقعی فیلد رو پیدا کرد."""
    found = {}
    try:
...
async def handle_check_join_callback(message):
    """دکمه‌ی «بررسی عضویت»: چون sender_id که بات‌پلتفرم می‌ده با GUID واقعی
    حساب (که pyrubi می‌بینه) قابل مقایسه نیست، اینجا دیگه چک زنده‌ی
    عضویت با pyrubi انجام نمی‌شه - فقط فلگ has_joined (که با فوروارد پست
    کانال ثبت می‌شه، توی handle_text) رو چک می‌کنه."""
    channel_guid = get_setting("channel_guid")
    if not channel_guid:
    ...
    member = await is_member(message.sender_id, force_refresh=True)
...
async def whoami_cmd(bot: Robot, message: Message):
    ...
    می‌ده و می‌گه از نظر جوین اجباری تاییدشده حساب می‌شه یا نه (چک زنده)."""
    channel_guid = get_setting("channel_guid")
