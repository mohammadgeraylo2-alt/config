...
def _pyrubi_is_member_blocking(channel_guid: str, user_guid: str) -> bool:
    ...
    start_id = None
    while True:
        result = client.get_all_members(channel_guid, start_id=start_id, just_get_guids=True)
        guids = result.get("in_chat_members") or result.get("member_guids") or []
        # بسته به نسخه‌ی pyrubi، ممکنه لیست دیکشنری باشه یا مستقیم لیست guid
        for item in guids:
            g = item.get("member_guid") if isinstance(item, dict) else item
            if g == user_guid:
                return True
        if not result.get("has_continue"):
            return False
        start_id = result.get("next_start_id")
        if not start_id:
            return False
...
async def is_member(chat_id):
    ...
    try:
        return await asyncio.to_thread(_pyrubi_is_member_blocking, channel_guid, chat_id)
    except Exception as e:
        logger.error(f"❌ چک عضویت با pyrubi خطا داد (GUID={channel_guid}, chat_id={chat_id}): {e}", exc_info=True)
...
async def test_member_cmd(bot: Robot, message: Message):
    ...
    await _rx(message.reply(
        ...
        f"🔎 chat_id (خودت)={message.chat_id}\n"
    ...
    if not has_session:
        ...
        return
    try:
        result = await asyncio.to_thread(_pyrubi_is_member_blocking, channel_guid, message.chat_id)
        await _rx(message.reply(f"✅ نتیجه‌ی is_member: {result}"))
    except Exception as e:
