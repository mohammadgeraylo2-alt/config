...
async def handle_music_search(message: Message, query: str):
    ...
    if not results:
        # ممکنه کاربر به‌جای اسم آهنگ، یه تیکه از شعرشو فرستاده باشه. کاتالوگ
        # Deezer/YouTube Music فقط رو عنوان/آرتیست سرچ می‌کنه، نه متن ترانه؛
        # برای همین با گوگل شعر رو سرچ می‌کنیم تا اسم واقعی آهنگ رو پیدا کنیم
        # و دوباره تو همون کاتالوگ بگردیم.
        ...
        if lyric_query and lyric_query.strip().lower() != query.strip().lower():
            results, artist_id, artist_name, artist_source = await asyncio.to_thread(search_songs, lyric_query)
    if not results:
