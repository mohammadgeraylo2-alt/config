            ...
            try:
                ...
                if result and result.get("title"):
                    ...
                    if song_path and os.path.exists(song_path):
                        ...
                        await send_music_native(message, song_path, CAPTION)
...
# صفحه‌ی عمومی پست (rubika.ir/post/...) رو مثل روش پینترست اسکرپ می‌کنیم و
# لینک ویدیو/عکس رو از og:video یا الگوهای mp4 داخل HTML پیدا می‌کنیم. این
# روش نیاز به لاگین یا اکانت جدا نداره. اگه صفحه سمت کلاینت (JS) رندر بشه و
# این الگوها چیزی پیدا نکنن، توی لاگ یه نمونه از HTML چاپ میشه تا بشه رجکس رو
# اصلاح کرد.
def _download_rubino_blocking(chat_id, text: str):
    headers_scrape = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36",
    ...
    # این‌ها آدرس‌های پیش‌فرض/لوگوی خودِ سایت روبیکان، نه محتوای واقعی پست -
    # اگه رجکس به‌جای مدیای واقعی به یکی از این‌ها برخورد کنه، باید رد بشه
    # (وگرنه دانلودش هم شکست می‌خوره، هم گاهی با خطای SSL عجیب کرش می‌کنه).
    def _is_placeholder(url: str) -> bool:
        bad_markers = ("/assets/logo", "/assets/default", "favicon", "og-image-default")
        return any(m in url.lower() for m in bad_markers)

    try:
        html = requests.get(post_url, headers=headers_scrape, timeout=20).text
        ...
        if not media_url:
            logger.warning(f"rubino: هیچ الگویی توی HTML پیدا نشد (یا فقط لینک پیش‌فرض بود). نمونه: {html[:2000]}")
    except Exception as e:
        logger.warning(f"rubino scrape error: {e}")

    if not media_url:
        return None

    try:
        # بعضی CDNهای روبیکا بدون هدر Referer درخواست رو رد می‌کنن یا ریدایرکت
        # می‌کنن به یه آدرس دیگه با گواهی نامعتبر - برای همین Referer اصلی رو
        # هم می‌فرستیم.
        dl_headers = dict(headers_scrape)
        dl_headers["Referer"] = post_url
        content = requests.get(media_url, headers=dl_headers, timeout=40).content
...
async def handle_rubino(message: Message, text: str):
    ...
    try:
        file_path = await asyncio.to_thread(_download_rubino_blocking, chat_id, text)
        if file_path:
            ext = file_path.rsplit(".", 1)[-1].lower()
            if ext in ("mp4", "mov", "webm"):
                await send_video_native(message, file_path, CAPTION)
...
async def handle_download_callback(message: Message, index: int):
    ...
    try:
        ...
        await send_music_native(message, path, CAPTION)
