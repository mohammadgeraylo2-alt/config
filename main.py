...
async def _resolve_real_sender_guid(message: "Message"):
    """لاگ ریلویی که فرستادی نشون داد rubka چند تا پراپرتی async روی Message
    داره: author_info, author_name, name, username (کوروتین‌هایی که باید
    await بشن). این دقیقاً همون چیزیه که لازم داشتیم: به‌جای فرض‌کردن اینکه
    sender_id همون GUID واقعیه، این تابع سعی می‌کنه با await‌کردن این
    پراپرتی‌ها GUID واقعی حساب رو در بیاره. اگه پیدا شد، caller می‌تونه
    مستقیم با pyrubi (که با GUID واقعی کار می‌کنه) چک عضویت واقعی بزنه."""
