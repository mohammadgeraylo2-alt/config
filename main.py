...
async def is_member(chat_id):
    ...
    try:
        ...
        # قبلاً اینجا فقط یه خط کوتاه لاگ می‌شد که مشخص نمی‌کرد دقیقاً چرا
        # check_join شکست خورده (متد وجود نداره؟ ربات ادمین کانال نیست؟ GUID
        # اشتباهه؟) - الان traceback کامل رو لاگ می‌کنیم تا علتش مشخص بشه.
        logger.error(f"❌ check_join خطا داد (GUID={channel_guid}, chat_id={chat_id}): {e}", exc_info=True)
...
def build_caption(with_tag: bool = False) -> str:
    """کپشن اصلی فایل‌ها؛ اگه with_tag=True باشه، تگ کانال (@InstaSaveXX یا هرچی
    ادمین توی تنظیمات گذاشته) هم زیرش اضافه میشه - برای آهنگ‌ها استفاده میشه."""
...
def build_channel_keypad():
    """دکمه‌ی شیشه‌ای «کانال ما» که زیر ویدیوها می‌ذاریم - اگه لینک کانال توی
    تنظیمات ادمین ست نشده باشه، دکمه‌ای برنمی‌گردونه."""
    link = get_setting("channel_link")
    if not link:
        return None
    b = InlineBuilder()
    b = b.row(b.button_link("open_channel", "📢 کانال ما", link))
    return b.build()
...
def _rubpy_check_join_blocking(channel_guid: str, user_guid: str) -> list:
    ...
    نیازی به input() نداره. مستندات rubpy مشخص نکرده دقیقاً اسم متدِ «گرفتن
    اعضای کانال» چیه، برای همین چند تا اسم رایج رو امتحان می‌کنیم و نتیجه‌ی
    خامِ هرکدوم رو برمی‌گردونیم تا از روی خروجی واقعی بشه فهمید کدوم درسته."""
    from rubpy import Client
    attempts = []
    with Client("rubino_acc") as client:
        candidates = [
            "get_channel_all_members",
            "get_channel_members",
            "channel_members",
            "get_all_members",
            "get_channel_info",
        ...
        for name in candidates:
            ...
            try:
                ...
                attempts.append((name, f"✅ {result!r}"[:900]))
            except Exception as e:
...
async def test_join2_cmd(bot: Robot, message: Message):
    """دستور دیباگ - فقط ادمین: چند اسم متد مختلف رو روی اکانت روبینوی
    لاگین‌شده امتحان می‌کنه تا ببینیم کدومش واقعاً لیست/وضعیت اعضای کانال رو
    برمی‌گردونه. بعد از دیدن نتیجه، همون متدِ درست رو دائمی توی is_member()
    می‌ذاریم."""
    if not ADMIN_CHAT_ID or message.sender_id != ADMIN_CHAT_ID:
        return
    channel_guid = get_setting("channel_guid")
    ...
    if not (os.path.exists("rubino_acc.rp") or glob.glob("rubino_acc*")):
        ...
        return
    await _rx(message.reply("🔎 دارم چند روش مختلف رو با اکانت روبینو امتحان میکنم، چند ثانیه صبر کن..."))
    try:
        attempts = await asyncio.to_thread(_rubpy_check_join_blocking, channel_guid, message.sender_id)
        text = "📋 نتیجه‌ی تست‌ها:\n\n" + "\n\n".join(f"• {n}:\n{r}" for n, r in attempts)
        await _rx(message.reply(text[:4000]))
    except Exception as e:
