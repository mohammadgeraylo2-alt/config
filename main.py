...
# ─── جوین اجباری کانال ────────────────────────────────────────────────────────
# کش لیست اعضای کانال: قبلاً هر بار که یه پیام می‌اومد، کل لیست اعضای کانال
# صفحه‌به‌صفحه از اول گرفته می‌شد که هم کند بود هم برای کانال‌های بزرگ ممکن بود
# تایم‌اوت بده یا ریت‌لیمیت بخوره - همین باعث می‌شد چک عضویت گاهی درست کار نکنه.
# حالا کل لیست رو یک‌بار می‌گیریم و MEMBER_CACHE_TTL ثانیه نگه می‌داریم؛ فقط
# موقع زدن دکمه‌ی «عضو شدم» (handle_check_join_callback) کش رو دور می‌زنیم و
# یه فچ تازه می‌زنیم، چون دقیقاً همون لحظه‌ست که باید مطمئن باشیم.
MEMBER_CACHE_TTL = 120  # ثانیه
_member_cache = {"guid": None, "members": set(), "ts": 0.0}
_member_cache_lock = threading.Lock()


def _pyrubi_paginate_guids_blocking(client, method_name: str, channel_guid: str) -> set:
    """صفحه‌بندی مشترک برای متدهایی مثل get_all_members و get_admin_members که
    فرمت خروجی یکسانی دارن (دیکشنری با in_chat_members/has_continue/next_start_id)."""
...
def _pyrubi_fetch_all_members_blocking(channel_guid: str) -> set:
    """bot.check_join (rubka) با EOFError کرش می‌کنه (ظاهراً می‌خواد لاگین
    تعاملی بزنه). به‌جاش با pyrubi (که مطمئنیم کار می‌کنه) اعضا رو می‌گیریم.
    نکته‌ی مهم (با /test_pyrubi تأیید شد): get_all_members فقط اعضای عادی
    (join_type='Member') رو برمی‌گردونه و ادمین‌ها/سازنده‌ی کانال
    (join_type='Admin'/'Creator') رو کلاً از این لیست حذف می‌کنه - برای
    همین جدا با get_admin_members هم ادمین‌ها/سازنده رو می‌گیریم و با هم
    ترکیب می‌کنیم، وگرنه ادمین‌های خودِ کانال (و حتی خودِ ربات) به‌عنوان
    «عضو نیستی» شناخته می‌شدن."""
    from pyrubi import Client
    client = Client("pyrubi_acc")
    guids = _pyrubi_paginate_guids_blocking(client, "get_all_members", channel_guid)
    try:
        guids |= _pyrubi_paginate_guids_blocking(client, "get_admin_members", channel_guid)
    except Exception as e:
        logger.warning(f"⚠️ گرفتن لیست ادمین‌های کانال با get_admin_members ناموفق بود (چک فقط با اعضای عادی انجام میشه): {e}")
    return guids


def _get_cached_members_blocking(channel_guid: str, force_refresh: bool = False) -> set:
    now = time.time()
    with _member_cache_lock:
        fresh_enough = (
            not force_refresh
            and _member_cache["guid"] == channel_guid
            and (now - _member_cache["ts"]) < MEMBER_CACHE_TTL
        )
        if fresh_enough:
            return _member_cache["members"]
    # فچ خارج از لاک انجام میشه تا در طول یه ریکوئست کند (شبکه)، بقیه‌ی
    # پیام‌ها/چک‌ها رو بلاک نکنه.
    members = _pyrubi_fetch_all_members_blocking(channel_guid)
    with _member_cache_lock:
        _member_cache["guid"] = channel_guid
        _member_cache["members"] = members
        _member_cache["ts"] = time.time()
    return members
...
async def is_member(user_guid, force_refresh: bool = False) -> bool:
    ...
    try:
        members = await asyncio.to_thread(_get_cached_members_blocking, channel_guid, force_refresh)
        return user_guid in members
    except Exception as e:
        logger.error(f"❌ چک عضویت با pyrubi خطا داد (GUID={channel_guid}, user_guid={user_guid}): {e}", exc_info=True)
        # اگه کش قدیمی از یه فچ موفق قبلی داریم، به‌جای باز کردن بی‌قیدوشرط
        # دسترسی، از همون کش (حتی منقضی‌شده) استفاده می‌کنیم - نتیجه‌ی
        # دقیق‌تری از "همیشه True برگردون" میده.
        with _member_cache_lock:
            if _member_cache["guid"] == channel_guid and _member_cache["members"]:
                return user_guid in _member_cache["members"]
        return True  # هیچ کشی هم نداریم -> جلوی کاربر رو نگیر
...
async def handle_check_join_callback(message):
    ...
    # force_refresh=True: دقیقاً همین لحظه که کاربر روی «عضو شدم» زده باید
    # آخرین وضعیت رو بگیریم، نه یه کش تا ۲ دقیقه قدیمی.
    if await is_member(message.sender_id, force_refresh=True):
...
async def whoami_cmd(bot: Robot, message: Message):
    ...
    می‌ده و می‌گه همین الان توی لیست اعضا/ادمین‌های کانال هست یا نه - برای
    اینکه هر اکانتی بتونه خودش مستقیم چک کنه، بدون نیاز به دسترسی ادمین."""
    channel_guid = get_setting("channel_guid")
    text = f"🆔 GUID شما: {message.sender_id}"
    if channel_guid and (os.path.exists("pyrubi_acc.db") or glob.glob("pyrubi_acc*")):
        try:
            found = await is_member(message.sender_id, force_refresh=True)
            text += f"\n📣 عضویت در کانال: {'✅ عضو هستید' if found else '❌ در لیست اعضا/ادمین‌های کانال نیستید'}"
        except Exception as e:
            text += f"\n⚠️ چک عضویت خطا داد: {type(e).__name__}: {e}"
...
async def test_member_cmd(bot: Robot, message: Message):
    ...
    if not has_session:
        ...
        return
    try:
        members = await asyncio.to_thread(_get_cached_members_blocking, channel_guid, True)
        found = message.sender_id in members
        await _rx(message.reply(
            f"✅ فچ موفق - تعداد کل اعضای برگشتی از get_all_members: {len(members)}\n"
            f"🔎 GUID تو ({message.sender_id}) توی این لیست {'✅ هست' if found else '❌ نیست'}"
        ))
        if not found:
            sample = list(members)[:5]
            await _rx(message.reply(
                "⚠️ اگه مطمئنی عضو/ادمین کانالی ولی توی لیست نیستی، احتمالش اینه که "
                "get_all_members توی pyrubi ادمین‌ها/سازنده‌ی کانال رو توی لیست برنمی‌گردونه "
                "(فقط اعضای عادی). چند نمونه GUID از لیست برگشتی:\n" + "\n".join(sample or ["(لیست خالیه)"])
            ))
    except Exception as e:
