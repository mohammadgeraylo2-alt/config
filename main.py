...
def google_lyrics_search(query: str):
    ...
    اولویت با تیکه‌ی لاتین/فینگلیش (اگه تو تیتر/توضیحات نتایج پیدا بشه) چون
    خیلی دقیق‌تر از تیتر خام فارسی با کاتالوگ مچ میشه؛ اگه هیچ نتیجه‌ای لاتین
    نداشت، به تیتر فارسیِ پاک‌سازی‌شده برمی‌گردیم."""
    try:
