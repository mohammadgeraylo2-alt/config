...
def get_referral_link(chat_id) -> str:
    """لینک اختصاصی دعوت کاربر (https://rubika.ir/<username>?start=<chat_id>) رو
    برمی‌گردونه. چون این یه لینک ساده‌ی متنیه (نه دکمه‌ی شیشه‌ای)، روبیکا خودش
    ...
    if not bot_username:
        return ""
    return f"https://rubika.ir/{bot_username}?start={chat_id}"
...
async def check_chat_limit(message) -> bool:
    ...
    if used_today >= chat_free_limit:
        ...
        link = get_referral_link(message.chat_id)
        text = (
            f"⛔️ سقف رایگان چت با هوش مصنوعی امروزت ({chat_free_limit} پیام) تموم شده. فردا دوباره امتحان کن.\n\n"
            f"💎 یا همین الان {target} نفر رو دعوت کن و {reward_days} روز اشتراک VIP رایگان بگیر:\n"
        )
        if link:
            text += f"🔗 {link}"
        else:
            text += f"کد دعوت: `{message.chat_id}`\n(دوستت باید بعد از /start این کد رو بفرسته)"
...
def invite_text(chat_id) -> str:
    """متن مشترک /invite برای استفاده هم توی دستور و هم توی دکمه‌ی منو."""
    target = int(get_setting("referral_target", "2") or "2")
    reward_days = int(get_setting("referral_reward_days", "30") or "30")
...
async def start_menu_invite_callback(message):
    ...
    if is_banned(message.chat_id):
        await _rx(message.reply("⛔️ شما مسدود شده‌اید."))
        return
    await _rx(message.reply(invite_text(message.chat_id)))
...
async def start(bot: Robot, message: Message):
    ...
    try:
        register_user(message.chat_id)
        # ─── دریافت کد دعوت از /start <referrer_id> (سیستم دعوت دونفره) ───
        parts = (message.text or "").split(maxsplit=1)
        if len(parts) > 1 and parts[1].strip():
            set_invited_by(message.chat_id, parts[1].strip())
        if is_banned(message.chat_id):
            await _rx(message.reply("⛔️ شما مسدود شده‌اید."))
            return
        if not await is_member(message.sender_id):
