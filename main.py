...
async def handle_text(bot: Robot, message: Message):
    ...
    # ─── پست‌های خودِ کانال (که چون ربات ادمین/سازنده‌ی کاناله، به‌عنوان
    # پیام ورودی عادی هم به این‌جا می‌رسن) رو کلاً نادیده می‌گیریم - وگرنه
    # ربات سعی می‌کنه متن پستِ خودش رو مثل یه پیام کاربر پردازش کنه (که یا
    # بی‌فایده‌ست یا اگه خطا بده، ریپلای «خطای غیرمنتظره» رو توی خودِ کانال
    # منتشر می‌کنه - دقیقاً همون چیزی که دیده شد).
    if str(getattr(message, "chat_id", "")).startswith("c0"):
        return
