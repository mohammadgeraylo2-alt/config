...
async def test_link_button_cmd(bot: Robot, message: Message):
    ...
    kp = builder.build()
    # خروجی خام JSON کی‌پد رو هم می‌فرستیم تا ساختار واقعی دکمه‌ی Link رو ببینیم
    try:
        raw = json.dumps(kp, ensure_ascii=False, indent=2)
    except Exception as e:
        raw = f"(خطا در dump کردن: {e})"
    await _rx(message.reply_inline("این دو تا دکمه رو بزن، ببین کدوم باز میشه:", kp))
    await _rx(bot.send_message(message.chat_id, f"📦 ساختار خام کی‌پد:\n```\n{raw}\n```"))
...
def build_invite_message(chat_id):
    ...
    if link:
        ...
        text += f"\n\n🔗 {link}"
        builder = InlineBuilder()
        builder = builder.row(builder.button_link("do_invite", "🤝 دعوت کردن", link))
