...
# توجه: قبلاً اینجا از یک Thread جدا برای اجرای دانلودها استفاده می‌شد تا ربات
# موقع دانلود یک نفر، برای بقیه بلاک نشه. اما rubka فقط توی همون event loop اصلی
# (رشته‌ی اصلی) اجازه‌ی ارسال پیام/فایل می‌ده؛ صدا زدن message.reply/reply_document
# از یک Thread جدا با RuntimeError («no running event loop») و قطع شدن SSL برخورد
# می‌کرد. برای رفع قطعی مشکل، این دکوریتور الان هندلر رو مستقیم و همون‌جا (بدون
# Thread) اجرا می‌کنه؛ یعنی موقع دانلود یک نفر، بقیه چند ثانیه صبر می‌کنن، ولی
# دیگه کرش/قطع ارسال فایل پیش نمیاد.
...
def handle_text(bot: Robot, message: Message):
    ...
    try:
        ...
        try:
            message.reply(f"❌ یه خطای غیرمنتظره پیش اومد.")
        except Exception:
            pass
