...
async def is_member(chat_id):
    ...
    try:
        return bool(await _rx(bot.check_join(channel_guid, chat_id)))
...
def _rubika_send_video_blocking(chat_id: str, file_path: str, caption: str = "", file_name: str = None) -> dict:
    ...
    # ۳) ارسال واقعیِ فایل (با file_id) به چت
...
async def send_video_native(message: "Message", file_path: str, caption: str = "", file_name: str = None):
    ...
    try:
        return await asyncio.to_thread(
            _rubika_send_video_blocking, message.chat_id, file_path, caption, file_name
        )
    except Exception as e:
        logger.warning(f"ارسال ویدیو با type=Video خام شکست خورد، fallback به reply_document: {e}")
        return await _rx(message.reply_document(path=file_path, text=caption))
...
async def handle_instagram(message: Message, text: str):
    ...
    try:
        ...
        if downloaded:
            await send_video_native(message, video_path, CAPTION)
            add_download(chat_id, "instagram")
            await _rx(status.edit("🎵 دارم آهنگش رو شناسایی میکنم..."))
            try:
                result = await asyncio.to_thread(detect_song_sync, video_path)
                if result and result.get("title"):
                    ...
                    if song_path and os.path.exists(song_path):
                        ...
                        await _rx(message.reply_music(path=song_path, text=CAPTION))
    ...
    try:
        ...
        if file_path:
            ...
            if ext in ("mp4", "mov", "webm"):
                await send_video_native(message, file_path, CAPTION)
            else:
                await _rx(message.reply_image(path=file_path, text=CAPTION))
...
async def handle_download_callback(message: Message, index: int):
    ...
    try:
        ...
        await _rx(message.reply_music(path=path, text=CAPTION))
...
async def handle_admin_text(message: Message, text: str):
    ...
    if action == "adm_give_vip":
        ...
        await _rx(message.reply(f"✅ GUID کانال تنظیم شد: {text.strip()}"))
...
async def handle_text(bot: Robot, message: Message):
    ...
    logger.info(f"📩 پیام متنی از {message.sender_id}: {text[:60]!r}")
...
# ═══════════════════════════════════════════════════════════════════════════
# سیستم مستقلِ دریافت کلیک دکمه‌های شیشه‌ای (InlineKeypad)
# ═══════════════════════════════════════════════════════════════════════════
# مشکل: کتابخونه‌ی rubka موقع polling (getUpdates)، کلیک روی دکمه‌های شیشه‌ای
# (که طبق مستندات رسمی روبیکا به‌شکل یه شیء جدا به اسم inline_message میان، نه
# یه پیام معمولی) رو تشخیص نمیده و on_callback اصلاً صدا زده نمیشه - بدون هیچ
# لاگ یا خطایی. برای همین یه حلقه‌ی polling کاملاً مستقل و موازی (توی ترد جدا،
# مستقیم با requests) می‌سازیم که فقط دنبال inline_message می‌گرده و پردازشش
# می‌کنه - بدون اینکه به مکانیزم پردازش پیام‌های عادی rubka دست بزنیم (اونا
# همچنان از همون مسیر قبلی و درست کار می‌کنن).

def _rubika_get_updates_blocking(offset_id: str = None, limit: int = 100) -> dict:
    payload = {"limit": limit}
    if offset_id:
        payload["offset_id"] = offset_id
    r = requests.post(f"{RUBIKA_API_BASE}/getUpdates", json=payload, timeout=40)
    r.raise_for_status()
    j = r.json()
...
class RawMessage:
    ...
    و هم برای کلیک دکمه‌های شیشه‌ای (از مسیر این پولینگ مستقل) استفاده کرد."""
...
async def _handle_inline_click(inline: dict):
    ...
    logger.info(f"📩 کلیک دکمه‌ی شیشه‌ای (مسیر مستقل) از {sender_id}: {btn_id}")
    ...
    try:
        ...
        logger.exception("خطا در پردازش کلیک دکمه‌ی شیشه‌ای (مسیر مستقل)")


async def _inline_poll_loop():
    offset_id = None
    logger.info("🔘 حلقه‌ی مستقل polling برای دکمه‌های شیشه‌ای (inline) استارت شد.")
    while True:
        try:
            data = await asyncio.to_thread(_rubika_get_updates_blocking, offset_id, 100)
            updates = data.get("updates") or []
            next_offset = data.get("next_offset_id")
            if next_offset:
                offset_id = next_offset
            for upd in updates:
                inline = upd.get("inline_message") if isinstance(upd, dict) else None
                if inline:
                    await _handle_inline_click(inline)
        except Exception as e:
            logger.warning(f"خطا در حلقه‌ی polling دکمه‌های شیشه‌ای: {e}")
        await asyncio.sleep(1.5)


def start_inline_poll_thread():
    """این حلقه رو توی یه ترد و event loop کاملاً جدا اجرا می‌کنه، مستقل از
    حلقه‌ی داخلی خودِ rubka، تا هیچ تداخلی با پردازش پیام‌های عادی نداشته باشه."""
    def _runner():
        asyncio.run(_inline_poll_loop())
    t = threading.Thread(target=_runner, daemon=True, name="inline-poll")
    t.start()
