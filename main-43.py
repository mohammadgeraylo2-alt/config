...
def _rubika_send_music_blocking(chat_id: str, file_path: str, caption: str = "") -> dict:
    ...
    # مثل _rubika_send_video_blocking: اگه اتصالِ keep-alive استفاده‌شده برای
    # آپلود قطع/timeout بشه (خطای «Connection aborted / write operation timed
    # out» که قبلاً بدون هیچ retry مستقیم بالا می‌رفت)، یه بار با upload_url
    # تازه و timeout بزرگتر دوباره امتحان می‌کنیم تا کل ارسال fail نشه.
    try:
        with open(file_path, "rb") as f:
            r2 = HTTP_SESSION.post(upload_url, files={"file": (name, f, "audio/mpeg")}, timeout=(20, 180))
        r2.raise_for_status()
    except Exception as e:
        logger.warning(f"آپلود آهنگ بار اول شکست خورد ({e})، با upload_url تازه دوباره امتحان میشه")
        upload_url = _rubika_request_upload_url_blocking("Music")
        with open(file_path, "rb") as f:
            r2 = HTTP_SESSION.post(upload_url, files={"file": (name, f, "audio/mpeg")}, timeout=(20, 240))
        r2.raise_for_status()
