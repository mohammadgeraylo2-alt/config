...
if __name__ == "__main__":
    ...
    try:
        # bot.set_commands یه coroutine هست، نه تابع معمولی - قبلاً بدون await
        # صدا زده می‌شد که یعنی اصلاً اجرا نمی‌شد (فقط یه coroutine object
        # بی‌مصرف ساخته می‌شد و پایتون یه RuntimeWarning تو لاگ می‌ذاشت)،
        # برای همین با اینکه لاگ «✅ تنظیم شد» رو می‌دیدیم، منوی دستورات
        # هیچ‌وقت واقعاً روی روبیکا ثبت نمی‌شد. با asyncio.run واقعاً
        # اجراش می‌کنیم.
        asyncio.run(bot.set_commands([
            {"command": "start", "description": "شروع و منوی اصلی"},
            {"command": "music", "description": "🎵 جستجو و دانلود آهنگ"},
            ...
            {"command": "vip", "description": "💎 وضعیت اشتراک VIP"},
    ...
    # ─── اجرای مقاوم در برابر کرش ────────────────────────────────────────────
    # کتابخانه‌ی rubka یه باگ داخلی داره: وقتی توی حلقه‌ی run_progelry یه
    # RuntimeError موقتی پیش میاد (مثلاً یه قطعی کوتاه شبکه)، تلاشِ ریکاوری
    # خودش با یه asyncio.run() جدید انجام میشه که چون توی همون فراخوانیِ
    # bot.run() و روی همون event loop در حال بسته‌شدنه، با خطای
    # "Event loop is closed" شکست می‌خوره؛ این خطا بعدش به‌غلط به‌صورت
    # InvalidTokenError گزارش میشه و کل پروسه می‌میره (با اینکه توکن کاملاً
    # سالمه). به‌جای اینکه بذاریم Railway هر بار کل کانتینر رو کامل ری‌استارت
    # کنه (کند و پرهزینه)، خودمون این حلقه رو نگه می‌داریم: هر بار bot.run()
    # کرش کرد، چند ثانیه صبر می‌کنیم و با یه event loop کاملاً تازه (چون
    # bot.run() خودش asyncio.run جدید می‌سازه) دوباره وصل می‌شیم.
    # نکته‌ی مهم: بعد از اولین کرش، تلاش‌های بعدی همه‌شون فوری (بدون حتی
    # وصل‌شدن دوباره به وب‌ساکت) شکست می‌خورن. یعنی مشکل فقط یه قطعی گذرای
    # شبکه نیست؛ یه چیزی توی حافظه‌ی همون پروسه (مثلاً event loop قبلی یا
    # state داخلی rubka) بعد از اولین کرش آلوده/خراب می‌مونه و دیگه با
    # ساختن یه event loop جدید تویِ همون پروسه هم درست نمی‌شه. برای همین
    # به‌جای ریترای داخل پروسه، کل پروسه‌ی پایتون رو با os.execv از نو
    # می‌سازیم (جایگزینیِ کامل ایمیج پروسه با حافظه‌ی تمیز)، که خیلی سریع‌تر
    # از منتظرِ ری‌استارت کامل کانتینر توسط Railway هست.
    RETRY_DELAY_SECONDS = 5
    try:
        bot.run()
        logger.info("ℹ️ bot.run() بدون خطا خاتمه یافت.")
    except Exception as e:
        logger.error(
            f"❌ ربات کرش کرد ({type(e).__name__}: {e}). "
            f"{RETRY_DELAY_SECONDS} ثانیه صبر و بازسازی کامل پروسه با حافظه‌ی تازه..."
        )
        logger.exception("جزئیات کرش:")
