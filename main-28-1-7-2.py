...
NSFW_CHECK_PROMPT = (
    ...
    "بنویس: NSFW\n"
    "- برهنگی یا پورنوگرافی\n"
    "- ژست یا حالت بدنی که آشکارا جنسی‌نما (sexually suggestive) باشه\n"
    "- دکلته‌ی خیلی زیاد که بخش قابل‌توجهی از سینه نمایان باشه\n"
    "در غیر این صورت (لباس اسپرت/شنا/تاپ/یقه‌باز معمولی که دکلته‌ی زیادی "
    "نداره و ژست عادی و غیرجنسی‌نما داره)، فقط بنویس: SAFE\n"
...
def check_media_nsfw_blocking(file_path: str) -> bool:
    ...
    برای ویدیو، اول یه فریم ازش استخراج می‌کنه و همون فریم رو چک می‌کنه."""
    if get_setting("nsfw_filter_enabled", "1") != "1" or not GEMINI_KEY:
        return False
    ext = file_path.rsplit(".", 1)[-1].lower()
    if ext in ("mp4", "mov", "webm", "mkv"):
        frame_path = f"{file_path}.nsfwframe.jpg"
        try:
            if not _extract_video_frame_blocking(file_path, frame_path):
                return False  # نتونستیم فریم بگیریم - fail-open
            return _check_image_nsfw_blocking(frame_path)
        finally:
